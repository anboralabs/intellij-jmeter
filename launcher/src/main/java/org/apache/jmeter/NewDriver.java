...
public final class NewDriver {
  ...
  private static final String OS_NAME_LC =
      OS_NAME.toLowerCase(java.util.Locale.ENGLISH);
  ...
  static {
    ...
    String tmpDir;
    StringTokenizer tok =
        new StringTokenizer(initiaClasspath, File.pathSeparator);
    if (tok.countTokens() == 1 ||
        (tok.countTokens() ==
             2 // Java on Mac OS can add a second entry to the initial classpath
      ...
      tmpDir = System.getProperty(
          "jmeter.home",
          System.getenv(
              "JMETER_HOME")); // Allow override $NON-NLS-1$ $NON-NLS-2$
    ...
    File[] libDirs = new File[] {
        ...
        new File(JMETER_INSTALLATION_DIRECTORY + File.separator + "lib" +
                 File.separator + "ext"), // $NON-NLS-1$ $NON-NLS-2$
        new File(JMETER_INSTALLATION_DIRECTORY + File.separator + "lib" +
                 File.separator + "junit")}; // $NON-NLS-1$ $NON-NLS-2$
    for (File libDir : libDirs) {
      File[] libJars = libDir.listFiles((dir, name) -> name.endsWith(".jar"));
      if (libJars == null) {
        new Throwable("Could not access " + libDir)
            .printStackTrace(); // NOSONAR No logging here
        continue;
      }
      ...
      for (File libJar : libJars) {
        try {
          ...
          if (usesUNC) {
            if (s.startsWith("\\\\") &&
                !s.startsWith("\\\\\\")) { // $NON-NLS-1$ $NON-NLS-2$
              s = "\\\\" + s;              // $NON-NLS-1$
  ...
  createClassLoader(List<URL> jars, ClassLoader pluginClassLoader) {
    return java.security.AccessController.doPrivileged(
        (java.security.PrivilegedAction<DynamicClassLoader>)()
            -> new DynamicClassLoader(jars.toArray(new URL[jars.size()])));
  }
   ...
   * @throws MalformedURLException when <code>path</code> points to an invalid
   *     url
   */
   ...
   */
  public static void addURL(URL url) { loader.addURL(url); }
   ...
   */
  public static String getJMeterDir() { return JMETER_INSTALLATION_DIRECTORY; }
  ...
  public static void main(String[] args) {
    if (!EXCEPTIONS_IN_INIT.isEmpty()) {
      System.err.println(
          "Configuration error during init, see exceptions:" +
          exceptionsToString(
              EXCEPTIONS_IN_INIT)); // NOSONAR Intentional System.err use
    } else {
      ...
      try {
        ...
        Method startup = initialClass.getMethod(
            "start", new Class[] {new String[0].getClass()}); // $NON-NLS-1$
        startup.invoke(instance, new Object[] {args});
      } catch (Throwable e) { // NOSONAR We want to log home directory in case
                              // of exception
        e.printStackTrace();  // NOSONAR No logger at this step
        System.err.println(
            "JMeter home directory was detected as: " +
            JMETER_INSTALLATION_DIRECTORY); // NOSONAR Intentional System.err
                                            // use
      }
    }
  }
  ...
  exceptionsToString(List<? extends Exception> exceptionsInInit) {
  ...
  private static void setLoggingProperties(String[] args) {
    String jmLogFile = getCommandLineArgument(
        args, 'j', "jmeterlogfile"); // $NON-NLS-1$ $NON-NLS-2$

    if (jmLogFile != null && !jmLogFile.isEmpty()) {
      jmLogFile = replaceDateFormatInFileName(jmLogFile);
      System.setProperty(JMETER_LOGFILE_SYSTEM_PROPERTY,
                         jmLogFile); // $NON-NLS-1$
    } else if (System.getProperty(JMETER_LOGFILE_SYSTEM_PROPERTY) ==
               null) { // $NON-NLS-1$
      System.setProperty(JMETER_LOGFILE_SYSTEM_PROPERTY,
                         "jmeter.log"); // $NON-NLS-1$ $NON-NLS-2$
    }

    String jmLogConf = getCommandLineArgument(
        args, 'i', "jmeterlogconf"); // $NON-NLS-1$ $NON-NLS-2$
    File logConfFile = null;

    if (jmLogConf != null && !jmLogConf.isEmpty()) {
      logConfFile = new File(jmLogConf);
    } else if (System.getProperty("log4j.configurationFile") ==
               null) {                      // $NON-NLS-1$
      logConfFile = new File("log4j2.xml"); // $NON-NLS-1$
      if (!logConfFile.isFile()) {
        logConfFile = new File(JMETER_INSTALLATION_DIRECTORY,
                               "bin" + File.separator +
                                   "log4j2.xml"); // $NON-NLS-1$ $NON-NLS-2$
      }
    }
  ...
  private static String getCommandLineArgument(String[] args, int id,
    ...
    for (int i = 0; i < args.length; i++) {
      if ((shortArgName.equals(args[i]) && i < args.length - 1) ||
        ...
        break;
      } else if (!shortArgName.equals(args[i]) &&
                 args[i].startsWith(shortArgName)) {
  ...
  /*
   * If the fileName contains at least one set of paired single-quotes, reformat
   * using DateFormat
   */
  private static String replaceDateFormatInFileName(String fileName) {
    try {
      ...
      while (begin != -1) {
        ...
        if (end == -1) {
          throw new IllegalArgumentException(
              "Invalid pairs of single-quotes in the file name: " +
              fileName); // $NON-NLS-1$
        }

        format = fileName.substring(begin + 1, end);
        dateFormat = DateTimeFormatter.ofPattern(format).withZone(
      ...
      System.err.println("Error replacing date format in file name:" +
                         fileName + ", error:" + ex.getMessage()); // NOSONAR
    }
