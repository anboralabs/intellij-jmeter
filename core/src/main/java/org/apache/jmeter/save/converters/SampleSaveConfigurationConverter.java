...
public class SampleSaveConfigurationConverter extends ReflectionConverter {
  ...
  static class MyWrapper extends MapperWrapper {
    ...
    public MyWrapper(Mapper wrapped) { super(wrapped); }
  ...
  private static void createNode(HierarchicalStreamWriter writer, boolean save,
                                 String node) {
    if (!save) {
      return;
    }
  ...
  public Object unmarshal(HierarchicalStreamReader reader,
    ...
    final Class<SampleSaveConfiguration> thisClass =
    ...
    // The default for missing tags is false, so preset all the fields
    // accordingly
    ...
    while (reader.hasMoreChildren()) {
      ...
      if (!"formatter".equals(
              nn)) { // Skip formatter (if present) bug 42674 $NON-NLS-1$
        String fieldName = mapper.realMember(thisClass, nn);
        java.lang.reflect.Field field =
            reflectionProvider.getField(thisClass, fieldName);
