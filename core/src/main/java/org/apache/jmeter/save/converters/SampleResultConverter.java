...
public class SampleResultConverter extends AbstractCollectionConverter {
  private static final Logger log =
      LoggerFactory.getLogger(SampleResultConverter.class);
    ...
    if (save.saveResponseData(res)) {
      ...
      try {
        if (SampleResult.TEXT.equals(res.getDataType())) {
          writer.setValue(new String(res.getResponseData(),
                                     res.getDataEncodingWithDefault()));
        } else {
          writer.setValue("Non-TEXT response data, cannot record: (" +
                          res.getDataType() + ")");
        }
        // Otherwise don't save anything - no point
      } catch (UnsupportedEncodingException e) {
        writer.setValue(
            "Unsupported encoding in response data, cannot record: " + e);
      }
      writer.endNode();
    }
  ...
  protected void setAttributes(HierarchicalStreamWriter writer,
    ...
    if (save.saveLabel()) {
      writer.addAttribute(ATT_LABEL,
                          ConversionHelp.encode(res.getSampleLabel()));
    }
    if (save.saveCode()) {
      writer.addAttribute(ATT_RESPONSE_CODE,
                          ConversionHelp.encode(res.getResponseCode()));
    }
    if (save.saveMessage()) {
      writer.addAttribute(ATT_RESPONSE_MESSAGE,
                          ConversionHelp.encode(res.getResponseMessage()));
    }
    if (save.saveThreadName()) {
      writer.addAttribute(ATT_THREADNAME,
                          ConversionHelp.encode(res.getThreadName()));
    }
    ...
    SampleEvent event =
        (SampleEvent)context.get(SaveService.SAMPLE_EVENT_OBJECT);
    if (event != null) {
  ...
  public Object unmarshal(HierarchicalStreamReader reader,
    ...
    SampleResult res =
        (SampleResult)createCollection(context.getRequiredType());
   ...
   * @return <code>true</code> if the item was processed (for
   *     HTTPResultConverter)
   */
    ...
    if (subItem instanceof AssertionResult) {
      ...
      if (responseData.length() > 0) {
        ...
        try {
          ...
          res.setResponseData("Can't support the char set: " + dataEncoding,
                              null);
  ...
  protected void retrieveAttributes(HierarchicalStreamReader reader,
    ...
    res.setDataEncoding(
        ConversionHelp.decode(reader.getAttribute(ATT_DATA_ENCODING)));
    ...
    if (oldrc != null) {
      ...
      res.setResponseCode(
          ConversionHelp.decode(reader.getAttribute(ATT_RESPONSE_CODE)));
    }
    res.setResponseMessage(
        ConversionHelp.decode(reader.getAttribute(ATT_RESPONSE_MESSAGE)));
    res.setSuccessful(
        Converter.getBoolean(reader.getAttribute(ATT_SUCCESS), true));
    res.setThreadName(
        ConversionHelp.decode(reader.getAttribute(ATT_THREADNAME)));
    ...
    res.setConnectTime(
        Converter.getLong(reader.getAttribute(ATT_CONNECT_TIME)));
    ...
    res.setSampleCount(Converter.getInt(reader.getAttribute(ATT_SAMPLE_COUNT),
                                        1)); // default is 1
    res.setErrorCount(Converter.getInt(reader.getAttribute(ATT_ERROR_COUNT),
                                       0)); // default is 0
   ...
   */
  public SampleResultConverter(Mapper arg0) { super(arg0); }
}
