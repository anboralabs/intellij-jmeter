...
public class ConversionHelp {
  private static final Logger log =
      LoggerFactory.getLogger(ConversionHelp.class);
   ...
   *
   * AFAIK the variables should only be accessed from one thread, so no need to
   * synchronize.
   */
  ...
  public static void setInVersion(String v) { inVersion = v; }

  public static void setOutVersion(String v) { outVersion = v; }
  ...
  public static String encode(String p) {
    if (p == null) { // Nulls cannot be written using PrettyPrintWriter - they
                     // cause an NPE
      return "";     // $NON-NLS-1$
    }
   ...
   */
  public static String cdata(byte[] chars, String encoding)
      throws UnsupportedEncodingException {
  ...
  private static void saveClass(TestElement el, HierarchicalStreamWriter writer,
                                String prop) {
    String clazz = el.getPropertyAsString(prop);
    if (clazz.length() > 0) {
      writer.addAttribute(propertyToAttribute.get(prop),
                          SaveService.classToAlias(clazz));
    }
  }

  private static void
  restoreClass(TestElement el, HierarchicalStreamReader reader, String prop) {
    String att = propertyToAttribute.get(prop);
    String alias = reader.getAttribute(att);
    if (alias != null) {
      alias = SaveService.aliasToClass(alias);
      if (TestElement.GUI_CLASS.equals(
              prop)) { // mainly for TestElementConverter
  ...
  private static void saveItem(TestElement el, HierarchicalStreamWriter writer,
                               String prop, boolean encode) {
  ...
  private static void restoreItem(TestElement el,
                                  HierarchicalStreamReader reader, String prop,
                                  boolean decode) {
    String att = propertyToAttribute.get(prop);
   ...
   */
  public static String getPropertyName(HierarchicalStreamReader reader,
   ...
   */
  public static String getPropertyValue(HierarchicalStreamReader reader,
                                        UnmarshallingContext context,
                                        String name) {
  ...
  /**
   * Update a property value using {@link NameUpdater#getCurrentName(String,
   * String, String)}.
   *
   * Do not use for GUI_CLASS or TEST_CLASS.
   *
   ...
   * @return the property value, updated if necessary
   */
  public static String getUpgradePropertyValue(String name, String value,
                                               UnmarshallingContext context) {
    String testClass = (String)context.get(SaveService.TEST_CLASS_NAME);
