...
public class SaveService {
        ...
        public String serializedClass(
          ...
          if (type == null) {
            return super.serializedClass(
                null); // was type, but that caused FindBugs warning
          }
  ...
  private static final XStream JMXSAVER =
      new XStreamWrapper(new PureJavaReflectionProvider());
  private static final XStream JTLSAVER =
      new XStreamWrapper(new PureJavaReflectionProvider());
  static {
  ...
  // The XML header, with placeholder for encoding, since that is controlled by
  // property
  private static final String XML_HEADER =
      "<?xml version=\"1.0\" encoding=\"<ph>\"?>"; // $NON-NLS-1$

  // Default file name
  private static final String SAVESERVICE_PROPERTIES_FILE =
      "saveservice.properties"; // $NON-NLS-1$

  // Property name used to define file name
  private static final String SAVESERVICE_PROPERTIES =
      "saveservice_properties"; // $NON-NLS-1$
  ...
  private static String propertiesVersion =
      ""; // read from properties file; written to JMX files
  ...
  private static String fileVersion =
      ""; // computed from saveservice.properties file// $NON-NLS-1$

  private static String fileEncoding =
      ""; // read from properties file// $NON-NLS-1$

  static {
    log.info("Testplan (JMX) version: {}. Testlog (JTL) version: {}",
  ...
  private static void makeAlias(String aliasList, String clazz) {
    ...
    for (String a : aliases) {
      ...
      if (old != null) {
        log.error("Duplicate class detected for {}: {} & {}", alias, clazz,
                  old);
      }
    }
  ...
  private static File getSaveServiceFile() {
    String saveServiceProps = JMeterUtils.getPropDefault(
        SAVESERVICE_PROPERTIES, SAVESERVICE_PROPERTIES_FILE); //$NON-NLS-1$
    if (saveServiceProps.length() > 0) {                      //$NON-NLS-1$
      return JMeterUtils.findFile(saveServiceProps);
    }
    throw new IllegalStateException(
        "Could not find file configured in saveservice_properties property set to:" +
        saveServiceProps);
  }
  ...
  private static String checksum(Properties nameMap)
      throws NoSuchAlgorithmException {
    MessageDigest md = MessageDigest.getInstance("SHA-1");
    // This checksums the actual entries, and it ignores comments and blank
    // lines
    nameMap.entrySet()
        .stream()
        .sorted(Comparator
                    .comparing(
                        (Map.Entry<Object, Object> e) -> e.getKey().toString())
    ...
    try {
      ...
      try {
        ...
        throw new JMeterError(
            "JMeter requires the checksum of saveservice properties file to continue",
            e);
      }
      ...
      for (Map.Entry<Object, Object> me : nameMap.entrySet()) {
        ...
        if (!key.startsWith("_")) { // $NON-NLS-1$
          ...
          if (key.equalsIgnoreCase("_version")) { // $NON-NLS-1$
            ...
            log.info("Using SaveService properties version {}",
                     propertiesVersion);
          } else if (key.equalsIgnoreCase("_file_version")) { // $NON-NLS-1$
            log.info(
                "SaveService properties file version is now computed by a checksum,"
                +
      ...
      throw new JMeterError(
          "JMeter requires the saveservice properties file to continue");
    }
  }
  ...
  private static void registerConverter(String key, String val) {
    try {
      ...
      boolean useMapper =
          "collection".equals(trimmedValue) ||
          "mapping".equals(trimmedValue); // $NON-NLS-1$ $NON-NLS-2$
  ...
  private static void registerConverter(String key, XStream jmxsaver,
    ...
    final Class<? extends Converter> clazz =
        Class.forName(key).asSubclass(Converter.class);
    if (useMapper) {
      jmxsaver.registerConverter(
          clazz.getConstructor(Mapper.class).newInstance(jmxsaver.getMapper()));
    } else {
  ...
  public static void saveTree(HashTree tree, OutputStream out)
      throws IOException {
  ...
  // Used by Test code
  public static void saveElement(Object el, OutputStream out)
      throws IOException {
    // Get the OutputWriter to use
    OutputStreamWriter outputStreamWriter = getOutputStreamWriter(out);
    writeXmlHeader(outputStreamWriter);
   ...
   * @param writer output stream which must be created using {@link
  ...
  public static synchronized void saveSampleResult(SampleEvent evt,
                                                   Writer writer)
      throws IOException {
    DataHolder dh = JTLSAVER.newDataHolder();
    dh.put(SAMPLE_EVENT_OBJECT, evt);
    // This is effectively the same as saver.toXML(Object, Writer) except we get
    // to provide the DataHolder Don't know why there is no method for this in
    // the XStream class
    try {
      JTLSAVER.marshal(evt.getResult(), new XppDriver().createWriter(writer),
                       dh);
    } catch (RuntimeException e) {
      throw new IllegalArgumentException(
          "Failed marshalling:" + (evt.getResult() != null
                                       ? showDebuggingInfo(evt.getResult())
                                       : "null"),
          e);
    }
    writer.write('\n');
  ...
  private static String showDebuggingInfo(SampleResult result) {
    try {
      return "class:" + result.getClass() +
          ",content:" + ToStringBuilder.reflectionToString(result);
    } catch (Exception e) {
      return "Exception occurred creating debug from event, message:" +
          e.getMessage();
    }
  }

  // Routines for TestSaveService
  static String getPropertyVersion() { return SaveService.propertiesVersion; }

  static String getFileVersion() { return SaveService.fileVersion; }
  ...
  static List<String> checkClasses() {
    ...
    for (Object clazz : classToAlias.keySet()) {
      ...
      if (!NameUpdater.isMapped(name)) { // don't bother checking class is
                                         // present if it is to be updated
        try {
  ...
  private static void checkVersions() {
    if (!PROPVERSION.equalsIgnoreCase(propertiesVersion)) {
      log.warn("Bad _version - expected {}, found {}.", PROPVERSION,
               propertiesVersion);
    }
  }
  ...
  loadTestResults(InputStream reader,
    ...
    DataHolder dh = JTLSAVER.newDataHolder();
    dh.put(RESULTCOLLECTOR_HELPER_OBJECT,
           resultCollectorHelper); // Allow TestResultWrapper to feed back the
                                   // samples
    // This is effectively the same as saver.fromXML(InputStream) except we get
    // to provide the DataHolder Don't know why there is no method for this in
    // the XStream class
  ...
  private static HashTree readTree(InputStream inputStream, File file)
    ...
    } catch (CannotResolveClassException | ConversionException |
             NoClassDefFoundError e) {
      if (file != null) {
        throw new IllegalArgumentException(
            "Problem loading XML from:'" + file.getAbsolutePath() +
  ...
  private static void writeXmlHeader(OutputStreamWriter writer)
      throws IOException {
  ...
  //  line number         : 929
  //  path                :
  //  /jmeterTestPlan/hashTree/hashTree/hashTree[4]/hashTree[5]/WebServiceSampler
  //  cause-exception     :
  ...
  public static String getPropertiesVersion() { return propertiesVersion; }
