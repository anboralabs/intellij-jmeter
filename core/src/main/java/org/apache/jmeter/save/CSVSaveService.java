...
public final class CSVSaveService {
  private static final java.util.regex.Pattern DELIMITER_PATTERN =
      java.util.regex
          ...
          .compile(
              "\\w+((\\W)\\w+)?(\\2\\w+)*(\\2\"\\w+\")*" // $NON-NLS-1$
                                                         // last entries may be
                                                         // quoted strings
          );
  private static final Logger log =
      LoggerFactory.getLogger(CSVSaveService.class);
  ...
  private static final SampleSaveConfiguration _saveConfig =
  ...
  public static void processSamples(String filename, Visualizer visualizer,
    ...
    try (InputStream inStream = new FileInputStream(filename);
         Reader inReader = new InputStreamReader(
             inStream,
      ...
      if (saveConfig == null) { // not a valid header
        log.info(
            ...
            filename);
        saveConfig = (SampleSaveConfiguration)resultCollector.getSaveConfig()
                         .clone(); // may change the format later
      ...
      while ((parts = csvReadFile(dataReader, delim)).length != 0) {
        lineNumber++;
        SampleEvent event = CSVSaveService.makeResultFromDelimitedString(
            parts, saveConfig, lineNumber);
        if (event != null) {
          final SampleResult result = event.getResult();
          if (ResultCollector.isSampleWanted(result.isSuccessful(), errorsOnly,
                                             successOnly)) {
    ...
    try {
        ...
        if (saveConfig.printMilliseconds()) {
          try {
            timeStamp = Long.parseLong(text); // see if this works
                       ...
                       e) { // it did not, let's try some other formats
            log.warn(
                "Cannot parse timestamp: '{}', will try following formats {}",
                text, Arrays.asList(DATE_FORMAT_STRINGS));
            ...
            for (String fmt : DATE_FORMAT_STRINGS) {
              ...
              try {
                ...
                log.info("{} did not match {}, trying next date format", text,
                         fmt);
              }
            }
            if (!foundMatch) {
              throw new ParseException(
                  "No date-time format found matching " + text, -1);
            }
          }
      ...
      if (saveConfig.saveSampleCount()) {
        ...
        StatisticalSampleResult sampleResult =
            new StatisticalSampleResult(timeStamp, elapsed);
        result = sampleResult;
      } else {
      ...
      if (i + saveConfig.getVarCount() < parts.length) {
        log.warn(
            "Line: {}. Found {} fields, expected {}. Extra fields have been ignored.",
            lineNumber, parts.length, i);
      }

    } catch (NumberFormatException | ParseException e) {
      if (log.isWarnEnabled()) {
        log.warn("Error parsing field '{}' at line {}. {}", field, lineNumber,
                 e.toString());
      }
      throw new JMeterError(e);
    } catch (ArrayIndexOutOfBoundsException e) {
      log.warn("Insufficient columns to parse field '{}' at line {}", field,
               lineNumber);
      throw new JMeterError(e);
    }
  ...
  printableFieldNamesToString(SampleSaveConfiguration saveConfig) {
    ...
    appendFields(saveConfig.saveAssertionResultsFailureMessage(), text, delim,
                 FAILURE_MESSAGE);
    appendFields(saveConfig.saveBytes(), text, delim, CSV_BYTES);
    appendFields(saveConfig.saveSentBytes(), text, delim, CSV_SENT_BYTES);
    appendFields(saveConfig.saveThreadCounts(), text, delim, CSV_THREAD_COUNT1,
                 CSV_THREAD_COUNT2);
    ...
    appendFields(saveConfig.saveSampleCount(), text, delim, CSV_SAMPLE_COUNT,
                 CSV_ERROR_COUNT);
  ...
  private static void appendFields(final boolean condition,
                                   StringBuilder textBuffer, String delim,
                                   String... fieldNames) {
    if (condition) {
  ...
  static {
    ...
    headerLabelMethods.put(FAILURE_MESSAGE,
                           new Functor("setAssertionResultsFailureMessage"));
   ...
   *         header line
   */
  public static SampleSaveConfiguration
  getSampleSaveConfiguration(String headerLine, String filename) {
    String[] parts =
        splitHeader(headerLine, _saveConfig.getDelimiter()); // Try
                                                             // default
                                                             // delimiter
    ...
    if (delim != null) {
      if (log.isWarnEnabled()) {
        log.warn(
            "Default delimiter '{}' did not work; using alternate '{}' for reading {}",
  ...
  private static String[] splitHeader(String headerLine, String delim) {
    ...
    for (int i = 0; i < parts.length; i++) {
      ...
      if (current <= previous) {
        log.warn("Column header number {} name {} is out of order.", i + 1,
                 label);
   ...
   *             when writing to <code>writer</code> fails
   */
  public static void saveCSVStats(List<?> data, Writer writer, String[] headers)
      throws IOException {
    final char DELIM = ',';
  ...
  public static String resultToDelimitedString(SampleEvent event) {
    return resultToDelimitedString(
        event, event.getResult().getSaveConfig().getDelimiter());
  }
  ...
  public static String resultToDelimitedString(SampleEvent event,
    ...
    return resultToDelimitedString(
        event, event.getResult(), event.getResult().getSaveConfig(), delimiter);
  }
  ...
  @SuppressWarnings("JavaUtilDate")
  public static String
  resultToDelimitedString(SampleEvent event, SampleResult sample,
                          SampleSaveConfiguration saveConfig,
                          final String delimiter) {
  ...
  public static String[] csvReadFile(BufferedReader infile, char delim)
    ...
    while (-1 != (ch = infile.read())) {
      ...
      case PLAIN:
        if (ch == QUOTING_CHAR) {
          baos.write(ch);
          throw new IOException("Cannot have quote-char in plain field:[" +
                                baos.toString() + "]");
        } else if (isDelimOrEOL(delim, ch)) {
          push = true;
  ...
  public static void saveSampleResult(SampleEvent event, PrintWriter out) {
    SampleSaveConfiguration saveConfiguration =
        event.getResult().getSaveConfig();
    String delimiter = saveConfiguration.getDelimiter();
    String formattedResult = resultToDelimitedString(
        event, event.getResult(), saveConfiguration, delimiter);
  ...
  private static void saveSubResults(SampleEvent event, PrintWriter out,
                                     SampleSaveConfiguration saveConfiguration,
                                     String delimiter, SampleResult result,
                                     int recursionLevel) {
    ...
    for (SampleResult subResult : result.getSubResults()) {
      ...
      out.println(formattedResult);
      saveSubResults(event, out, saveConfiguration, delimiter, subResult,
                     recursionLevel + 1);
    }
  }
}
