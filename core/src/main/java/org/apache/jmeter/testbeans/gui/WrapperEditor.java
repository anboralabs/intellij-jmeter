 ...
 * also provides optional support for JMeter 'expressions' (you can choose
 * whether they make valid property values).
 *
 */
class WrapperEditor
    extends PropertyEditorSupport implements PropertyChangeListener {
  private static final Logger log =
      LoggerFactory.getLogger(WrapperEditor.class);
   ...
   */
  WrapperEditor(Object source, PropertyEditor typeEditor,
                PropertyEditor guiEditor, boolean acceptsNull,
                boolean acceptsExpressions, boolean acceptsOther,
                Object defaultValue) {
    super();
    if (source != null) {
   ...
   */
  WrapperEditor(PropertyEditor typeEditor, PropertyEditor guiEditor,
    ...
    if (text == null) {
      ...
      if (acceptsExpressions && isExpression(text)) {
        ...
        if (!acceptsOther && !isATag(text)) {
          shouldNeverHappen(
              "Text is not a tag but other entries are not allowed");
        }
  ...
  public String getAsText() {
    ...
    if (text == null) {
      ...
      if (!acceptsOther && !isATag(text)) {
        shouldNeverHappen(
            "Text is not a tag, but other values are not allowed");
      }
    }
      ...
      if (GuiPackage.getInstance() == null) {
        ...
        JOptionPane.showMessageDialog(
            ...
            JMeterUtils.getResString(
                "property_editor.value_is_invalid_message"), //$NON-NLS-1$
            JMeterUtils.getResString(
                "property_editor.value_is_invalid_title"), //$NON-NLS-1$
