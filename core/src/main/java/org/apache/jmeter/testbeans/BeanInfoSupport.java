 ...
 * <b><i>propertyName</i>.displayName</b>. <li>Properties' short descriptions --
 * from properties <b><i>propertyName</i>.shortDescription</b>.
 * </ul>
 * <p>
...
public abstract class BeanInfoSupport extends SimpleBeanInfo {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(BeanInfoSupport.class);

  // Some known attribute names, just for convenience:
  public static final String TAGS = GenericTestBeanCustomizer.TAGS;

  /**
   * Whether the field must be defined (i.e. is required);  Boolean, defaults
   * to FALSE
   */
  ...
  public static final String TEXT_LANGUAGE =
      GenericTestBeanCustomizer.TEXT_LANGUAGE;
  ...
  protected BeanInfoSupport(Class<? extends TestBean> beanClass) {
    ...
    try {
      rootBeanInfo = Introspector.getBeanInfo(
          beanClass, Introspector.IGNORE_IMMEDIATE_BEANINFO);
    } catch (IntrospectionException e) {
      throw new Error("Can't introspect " + beanClass,
    ...
    try {
      ...
      if (resourceBundle.containsKey(dnKey)) { // $NON-NLS-1$
        getBeanDescriptor().setDisplayName(
            resourceBundle.getString(dnKey)); // $NON-NLS-1$
      } else {
        log.debug("Localized display name not available for bean {}",
                  beanClass);
      }
      ...
      for (PropertyDescriptor property : properties) {
        ...
        if (resourceBundle.containsKey(propDnKey)) {
          property.setDisplayName(
              resourceBundle.getString(propDnKey)); // $NON-NLS-1$
        } else {
          log.debug(
              "Localized display name not available for property {} in {}",
              name, beanClass);
        }
        ...
        if (resourceBundle.containsKey(propSdKey)) {
          ...
          log.debug(
              "Localized short description not available for property {} in {}",
              name, beanClass);
        }
      }
      ...
      log.warn("Something bad happened when loading bean info for bean {}",
               beanClass, e);
    }
  }
  ...
  property(final String name, final Class<? extends Enum<?>> enumClass) {
    PropertyDescriptor property = property(name);
    if (property != null) {
      property.setValue(GenericTestBeanCustomizer.GUITYPE, enumClass);
      // we also provide the resource bundle
      property.setValue(GenericTestBeanCustomizer.RESOURCE_BUNDLE,
