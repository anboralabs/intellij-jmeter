...
public class TestBeanHelper {
  protected static final Logger log =
      LoggerFactory.getLogger(TestBeanHelper.class);

  /**
   * This class extracts information from {@link CachedPropertyDescriptor} in
   * such a way so it does not need to synchronize.
   */
  ...
  /**
   * Cache property information, so preparing test elements does not need to
   * perform reflective and synchronization again.
   */
  private static final ClassValue<List<CachedPropertyDescriptor>> GOOD_PROPS =
      new ClassValue<List<CachedPropertyDescriptor>>() {
        @Override
        protected List<CachedPropertyDescriptor> computeValue(Class type) {
          ...
          try {
            ...
            throw new IllegalArgumentException(
                "Couldn't set properties for " + type, e);
          }
          ...
          List<CachedPropertyDescriptor> descriptors =
              new ArrayList<>(descs.length);
          for (PropertyDescriptor desc : descs) {
            if (isDescriptorIgnored(desc)) {
              if (log.isDebugEnabled()) {
                log.debug("Ignoring property '{}' in {}", desc.getName(),
    ...
    try {
      for (CachedPropertyDescriptor desc : GOOD_PROPS.get(el.getClass())) {
        ...
        try {
          ...
          throw new IllegalStateException("Can't retrieve property '" +
        ...
        UnsatisfiedLinkError ule) { // Can occur running headless on Jenkins
  ...
  private static Object unwrapProperty(PropertyDescriptor desc,
    ...
    if (jprop instanceof TestElementProperty) {
      ...
      value = unwrapCollection((MultiProperty)jprop,
                               (String)desc.getValue(TableEditor.CLASSNAME));
    }
    // value was not provided, and this is allowed
    else if (jprop instanceof NullProperty &&
             // use negative condition so missing (null) value is treated as
             // FALSE
             !Boolean.TRUE.equals(
                 desc.getValue(GenericTestBeanCustomizer.NOT_UNDEFINED))) {
      value = null;
    } else {
    ...
    if (prop instanceof CollectionProperty) {
      ...
      for (JMeterProperty jMeterProperty : prop) {
        try {
          ...
          log.error("Couldn't convert object: {} to {}", prop.getObjectValue(),
                    type, e);
        }
      }
      return values;
