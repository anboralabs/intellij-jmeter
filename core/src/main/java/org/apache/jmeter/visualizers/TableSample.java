...
public class TableSample implements Serializable, Comparable<TableSample> {
  ...
  public TableSample(long totalSamples, int sampleCount, long startTime,
                     String threadName, String label, long elapsed,
                     boolean success, long bytes, long sentBytes, long latency,
                     long connect) {
  ...
  // The following getters may appear not to be used - however they are invoked
  // via the Functor class
  ...
  public int compareTo(TableSample o) {
    TableSample oo = o;
    return (totalSamples - oo.totalSamples) < 0
        ? -1
        : (totalSamples == oo.totalSamples ? 0 : 1);
  }
   ...
   */
  public long getConnectTime() { return connect; }
