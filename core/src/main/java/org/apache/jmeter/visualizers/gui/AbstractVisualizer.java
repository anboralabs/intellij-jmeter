 ...
 * <ul>
 * <li>{@link org.apache.jmeter.gui.JMeterGUIComponent#configure(TestElement)
 * configure(TestElement)}. Any additional parameters of your Visualizer need to
 * be handled by you.</li> <li>{@link
 * org.apache.jmeter.gui.JMeterGUIComponent#createTestElement()
 * createTestElement()}. For most purposes, the default
 * {@link org.apache.jmeter.reporters.ResultCollector ResultCollector} created
 * by this method is sufficient.</li>
 * <li>{@link org.apache.jmeter.gui.JMeterGUIComponent#getMenuCategories
 * getMenuCategories()}. To control where in the GUI your visualizer can be
 * added.</li> <li>{@link
 * org.apache.jmeter.gui.JMeterGUIComponent#modifyTestElement(TestElement)
 * modifyTestElement(TestElement)}. Again, additional parameters you require
 * have to be handled by you.</li> <li>{@link
 * org.apache.jmeter.gui.JMeterGUIComponent#createPopupMenu()
 * createPopupMenu()}.</li>
 * </ul>
 * </li>
 ...
 */
public abstract class AbstractVisualizer extends AbstractListenerGui
    implements Visualizer, ChangeListener, UnsharedComponent, Clearable {
  private static final long serialVersionUID = 241L;

  /** Logging. */
  private static final Logger log =
      LoggerFactory.getLogger(AbstractVisualizer.class);

  /** File Extensions */
  private static final String[] EXTS = {
      ".xml", ".jtl", ".csv"}; // $NON-NLS-1$ $NON-NLS-2$ $NON-NLS-3$
  ...
  protected AbstractVisualizer() {
    ...
    errorLogging = new JCheckBox(
        JMeterUtils.getResString("log_errors_only")); // $NON-NLS-1$
    ...
    successOnlyLogging = new JCheckBox(
        JMeterUtils.getResString("log_success_only")); // $NON-NLS-1$
    ...
    $NON-NLS-1$ true, collector.getSaveConfig()); d.pack();
        ComponentUtil.centerComponentInComponent(GuiPackage.getInstance().getMainFrame(),
    d); d.setVisible(true);
    });*/

    filePanel =
        new FilePanel(JMeterUtils.getResString("file_visualizer_output_file"),
                      EXTS); // $NON-NLS-1$
    filePanel.addChangeListener(this);
    filePanel.add(
        new JLabel(JMeterUtils.getResString("log_only"))); // $NON-NLS-1$
   ...
   */
  protected JCheckBox getErrorLoggingCheckbox() { return errorLogging; }
   ...
   */
  protected ResultCollector getModel() { return collector; }
   ...
   */
  protected Component getFilePanel() { return filePanel; }
  ...
  public void configure(TestElement el) {
    ...
    collector.setSaveConfig(
        (SampleSaveConfiguration)rc.getSaveConfig().clone());
  }
