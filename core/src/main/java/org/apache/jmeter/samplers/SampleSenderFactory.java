...
public class SampleSenderFactory {
  ...
  private static final String MODE_STRIPPED_BATCH =
      "StrippedBatch";                                // $NON-NLS-1$
  private static final String MODE_ASYNCH = "Asynch"; // $NON-NLS-1$
  private static final String MODE_STRIPPED_ASYNCH =
      "StrippedAsynch";                                     // $NON-NLS-1$
  private static final String MODE_DISKSTORE = "DiskStore"; // $NON-NLS-1$
  private static final String MODE_STRIPPED_DISKSTORE =
      "StrippedDiskStore"; // $NON-NLS-1$
  ...
  static SampleSender getInstance(RemoteSampleListener listener) {
    ...
    final String type =
        JMeterUtils.getPropDefault("mode", MODE_STRIPPED_BATCH); // $NON-NLS-1$
    ...
    if (type.equalsIgnoreCase(MODE_BATCH)) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Unable to create a sample sender from mode or class:'" + type +
                "', search for mode property in jmeter.properties for correct configuration options, "
