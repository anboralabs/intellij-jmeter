...
public class AsynchSampleSender
  ...
  private static final Logger log =
      LoggerFactory.getLogger(AsynchSampleSender.class);
  ...
  private static final int SERVER_CONFIGURED_CAPACITY =
      JMeterUtils.getPropDefault("asynch.batch.queue.size",
                                 DEFAULT_QUEUE_SIZE); // $NON-NLS-1$
  ...
  private transient BlockingQueue<SampleEvent>
      queue; // created by server in readResolve method

  private transient long
      queueWaits; // how many times we had to wait to queue a sample
  ...
  public void testEnded(String host) {
    ...
    if (queueWaits > 0) {
      log.info("QueueWaits: {}; QueueWaitTime: {} (nanoseconds)", queueWaits,
               queueWaitTime);
    }
  }
