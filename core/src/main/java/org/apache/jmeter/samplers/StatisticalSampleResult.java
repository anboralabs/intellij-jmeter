...
public class StatisticalSampleResult
  ...
  /**
   * Allow CsvSaveService to generate a suitable result when sample/error counts
   * have been saved.
   *
   * @deprecated Needs to be replaced when multiple sample results are sorted
   *     out
   *
  ...
  @Override public int getErrorCount() { // Overrides SampleResult
    return errorCount;
  }

  @Override public void setErrorCount(int e) { // for reading CSV files
    errorCount = e;
  }
   ...
   * @param keyOnThreadName true if key should use thread name, otherwise use
   *     thread group
