...
public class StatisticalSampleSender
  ...
  private static final Logger log =
      LoggerFactory.getLogger(StatisticalSampleSender.class);
  ...
  private static final boolean KEY_ON_THREADNAME =
      JMeterUtils.getPropDefault("key_on_threadname", false);

  // Instance fields are constructed by the client when the instance is create
  // in the test plan and the field values are then transferred to the server
  // copy by RMI serialisation/deserialisation
  ...
  // Settings; readResolve sets these from the server/client values as
  // appropriate
  // TODO would be nice to make these final; not 100% sure volatile is needed as
  ...
  private transient int
      sampleCount; // maintain separate count of samples for speed
  ...
  private Object readResolve() throws ObjectStreamException {
    ...
    if (log.isInfoEnabled()) {
      log.info(
          ...
          isClientConfigured() ? "Client" : "Server", numSamplesThreshold,
          timeThresholdMs, keyOnThreadName);
    }
    return this;
  }
}
