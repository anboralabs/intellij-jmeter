...
public class BatchSampleSender
  ...
  private static final Logger log =
      LoggerFactory.getLogger(BatchSampleSender.class);
  ...
  private static final int NUM_SAMPLES_THRESHOLD = JMeterUtils.getPropDefault(
      "num_sample_threshold", DEFAULT_NUM_SAMPLE_THRESHOLD); // $NON-NLS-1$
  ...
  private final long clientConfiguredTimeThresholdMs =
      JMeterUtils.getPropDefault("time_threshold",
                                 DEFAULT_TIME_THRESHOLD); // $NON-NLS-1$
  ...
  protected BatchSampleSender(RemoteSampleListener listener) {
    ...
    if (isClientConfigured()) {
      log.info(
          "Using batching (client settings) for this run. Thresholds: num={}, time={}",
  ...
  protected RemoteSampleListener getListener() { return listener; }

  /**
   * @return the sampleStore
   */
  // added: Bug 50008 - allow BatchSampleSender to be subclassed
  protected List<SampleEvent> getSampleStore() { return sampleStore; }
  ...
  public void sampleOccurred(SampleEvent e) {
    ...
    synchronized (sampleStore) {
      ...
      if (sendNow) {
        ...
        final ArrayList<SampleEvent> clone =
            (ArrayList<SampleEvent>)((ArrayList<SampleEvent>)sampleStore)
                .clone();
        clonedStore = clone;
        sampleStore.clear();
        if (timeThresholdMs != -1) {
  ...
  private Object readResolve() throws ObjectStreamException {
    ...
    log.info("Using batching for this run. Thresholds: num={}, time={}",
             numSamplesThreshold, timeThresholdMs);
    return this;
  }
}
