...
public class ThreadGroupGui
    extends AbstractThreadGroupGui implements ItemListener {
  ...
  private final JBooleanPropertyEditor scheduler =
      new JBooleanPropertyEditor(ThreadGroupSchema.INSTANCE.getUseScheduler(),
  ...
  public ThreadGroupGui() { this(true); }
  ...
  public ThreadGroupGui(boolean showDelayedStart) {
    ...
    bindingGroup.addAll(Arrays.asList(
        new JTextComponentBinding(
            threadInput, AbstractThreadGroupSchema.INSTANCE.getNumThreads()),
        new JTextComponentBinding(rampInput,
                                  ThreadGroupSchema.INSTANCE.getRampTime()),
        new JTextComponentBinding(duration,
                                  ThreadGroupSchema.INSTANCE.getDuration()),
        new JTextComponentBinding(delay, ThreadGroupSchema.INSTANCE.getDelay()),
        sameUserBox, scheduler));
  }
  ...
  public void assignDefaultValues(TestElement element) {
    ...
    element.set(AbstractThreadGroupSchema.INSTANCE.getSameUserOnNextIteration(),
                true);
    ((AbstractThreadGroup)element)
        .setSamplerController((LoopController)loopPanel.createTestElement());
  }
  ...
  public void configure(TestElement tg) {
    super.configure(tg);
    loopPanel.configure(
        (TestElement)tg.getProperty(AbstractThreadGroup.MAIN_CONTROLLER)
            .getObjectValue());
  ...
  private void toggleSchedulerFields() {
    boolean enable =
        !scheduler.getValue().equals(JEditableCheckBox.Value.of(false));
  ...
  private void init() { // WARNING: called from ctor so must not be overridden
    ...
    // NUMBER OF THREADS
    threadPropsPanel.add(
        labelFor(threadInput, "number_of_threads")); // $NON-NLS-1$
    ...
    scheduler.addPropertyChangeListener(JBooleanPropertyEditor.VALUE_PROPERTY,
                                        (ev) -> toggleSchedulerFields());
