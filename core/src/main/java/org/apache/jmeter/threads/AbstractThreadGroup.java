...
public abstract class AbstractThreadGroup extends AbstractTestElement
  ...
  private final transient IdentityHashMap<TestElement, Object> children =
      new IdentityHashMap<>();

  private static final Object DUMMY = new Object();

  public static final Duration DEFAULT_THREAD_STOP_TIMEOUT = Duration.ofMillis(
  ...
  /**
   * Stop test (all threads) if sampler error occurs, the entire test is
   * stopped at the end of any current samples
   */
  public static final String ON_SAMPLE_ERROR_STOPTEST = "stoptest";

  /**
   * Stop test NOW (all threads) if sampler error occurs, the entire test is
   * stopped abruptly. Any current samplers are interrupted if possible.
   */
  ...
  private final AtomicInteger numberOfThreads =
      new AtomicInteger(0); // Number of active threads in this group
  ...
  @NotNull
  @Override
  public PropertiesAccessor<? extends AbstractThreadGroup, ?
                                extends AbstractThreadGroupSchema>
  getProps() {
  ...
  public void initialize() {
    ...
    property.setObjectValue(
        getName()); // Copy our name into that of the controller
    property.setRunningVersion(
        property.isRunningVersion()); // otherwise name reverts
    c.initialize();
  }
   ...
   */
  void incrNumberOfThreads() { numberOfThreads.incrementAndGet(); }

  /**
   * Decrement the number of active threads
   */
  void decrNumberOfThreads() { numberOfThreads.decrementAndGet(); }
   ...
   */
  public int getNumThreads() { return get(getSchema().getNumThreads()); }
   ...
   */
  public abstract void start(int groupCount, ListenerNotifier notifier,
   ...
   */
  public abstract JMeterThread addNewThread(int delay,
                                            StandardJMeterEngine engine);
  ...
  protected JMeterThread
  makeThread(StandardJMeterEngine engine, JMeterThreadMonitor monitor,
             ListenerNotifier notifier, int groupNumber, int threadNumber,
             ListedHashTree threadGroupTree, JMeterVariables variables) {
    ...
    final JMeterThread jmeterThread = new JMeterThread(
        threadGroupTree, monitor, notifier, isSameUserOnNextIteration());
    ...
    final String threadName =
        distributedPrefix + (distributedPrefix.isEmpty() ? "" : "-") +
        groupName + " " + groupNumber + "-" + (threadNumber + 1);
