...
public final class JMeterContextService {
  ...
  private static final AtomicInteger NUMBER_OF_ACTIVE_THREADS =
      new AtomicInteger();

  private static final AtomicInteger NUMBER_OF_THREADS_STARTED =
      new AtomicInteger();

  private static final AtomicInteger NUMBER_OF_THREADS_FINISHED =
      new AtomicInteger();
   ...
   */
  public static JMeterContext getContext() { return threadContext.get(); }
  ...
  public static synchronized void startTest() {
    if (testStart.get() == 0) {
      ...
      JMeterUtils.setProperty("TESTSTART.MS",
                              Long.toString(testStart.get())); // $NON-NLS-1$
    }
  }
  ...
  public static ThreadCounts getThreadCounts() {
    return new ThreadCounts(NUMBER_OF_ACTIVE_THREADS.get(),
                            NUMBER_OF_THREADS_STARTED.get(),
                            NUMBER_OF_THREADS_FINISHED.get());
  }
  ...
  public static long getTestStartTime() { return testStart.get(); }

  /**
   * Get the total number of threads (&gt;= active)
   * @return total thread count
   */
  public static int getTotalThreads() { return TOTAL_THREADS.get(); }
   ...
   */
  public static JMeterVariables getClientSideVariables() { return variables; }
  ...
  initClientSideVariables(JMeterVariables clientSideVariables) {
    JMeterContextService.variables =
        new UnmodifiableJMeterVariables(clientSideVariables);
  }
