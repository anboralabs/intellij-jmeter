...
public class ThreadGroup extends AbstractThreadGroup {
  ...
  private static final long WAIT_TO_DIE =
      DEFAULT_THREAD_STOP_TIMEOUT.toMillis();
  ...
  private final ConcurrentHashMap<JMeterThread, Thread> allThreads =
  ...
  @Override
  public @NotNull
  PropertiesAccessor<? extends ThreadGroup, ? extends ThreadGroupSchema>
  getProps() {
   ...
   */
  public boolean getScheduler() { return getPropertyAsBoolean(SCHEDULER); }
   ...
   */
  public long getDuration() { return getPropertyAsLong(DURATION); }
   ...
   */
  public long getDelay() { return getPropertyAsLong(DELAY); }
   ...
   */
  public int getRampUp() { return getPropertyAsInt(ThreadGroup.RAMP_TIME); }
  ...
  private void scheduleThread(JMeterThread thread, long now) {
    ...
    if (getDelay() >= 0) { // Duration is in seconds
      ...
      throw new JMeterStopTestException("Invalid delay " + getDelay() +
                                        " set in Thread Group:" + getName());
    }
    ...
    if (getDuration() > 0) { // Duration is in seconds
      ...
      throw new JMeterStopTestException("Invalid duration " + getDuration() +
                                        " set in Thread Group:" + getName());
    }
  ...
  @Override
  public void start(int groupNum, ListenerNotifier notifier,
    ...
    log.info(
        "Starting thread group... number={} threads={} ramp-up={} delayedStart={}",
        groupNumber, numThreads, rampUpPeriodInSeconds, delayedStartup);
    if (delayedStartup) {
      threadStarter =
          new Thread(new ThreadStarter(notifier, threadGroupTree, engine),
                     getName() + "-ThreadStarter");
      threadStarter.setDaemon(true);
      threadStarter.start();
      // N.B. we don't wait for the thread to complete, as that would prevent
      // parallel TGs
    } else {
      ...
      for (int threadNum = 0; running && threadNum < numThreads; threadNum++) {
        ...
        if (log.isDebugEnabled()) {
          log.debug("Computed delayForNextThreadInMillis:{} for thread:{}",
                    delayForNextThreadInMillis, Thread.currentThread().getId());
        }
        lastThreadStartInMillis = nowInMillis;
        startNewThread(notifier, threadGroupTree, engine, threadNum, variables,
                       nowInMillis, Math.max(0, delayForNextThreadInMillis));
      }
    }
   ...
   */
  private JMeterThread startNewThread(ListenerNotifier notifier,
                                      ListedHashTree threadGroupTree,
                                      StandardJMeterEngine engine,
                                      int threadNum, JMeterVariables variables,
                                      long now, int delay) {
    JMeterThread jmThread =
        makeThread(engine, this, notifier, groupNumber, threadNum,
  ...
  public JMeterThread addNewThread(int delay, StandardJMeterEngine engine) {
    ...
    newJmThread = startNewThread(notifier, threadGroupTree, engine, numThreads,
                                 context.getVariables(), now, delay);
   ...
   */
  private static void stopThread(JMeterThread jmeterThread, Thread jvmThread,
                                 boolean interrupt) {
    jmeterThread.stop();
  ...
  class ThreadStarter implements Runnable {
    ...
    public ThreadStarter(ListenerNotifier notifier,
                         ListedHashTree threadGroupTree,
                         StandardJMeterEngine engine) {
      super();
      this.notifier = notifier;
    ...
    public void run() {
      try {
        ...
        if (usingScheduler) {
          ...
          // set the endtime for the Thread
          endtime = getDuration();
          if (endtime > 0) { // Duration is in seconds, starting from when the
                             // threads start
        ...
        for (int threadNumber = 0; running && threadNumber < numThreads;
          ...
          if (threadNumber > 0) {
            ...
            final int perThreadDelayInMillis =
                Math.round((rampUpOriginInMillis - elapsedInMillis) /
                           (float)(numThreads - threadNumber));
            pause(Math.max(
                0, perThreadDelayInMillis)); // ramp-up delay (except first)
          }
          if (usingScheduler && System.currentTimeMillis() > endtime) {
            break; // no point continuing beyond the end time
          }
          JMeterThread jmThread =
              makeThread(engine, ThreadGroup.this, notifier, groupNumber,
                         threadNumber, cloneTree(threadGroupTree), variables);
          ...
          Thread newThread = new Thread(jmThread, jmThread.getThreadName());
          newThread.setDaemon(false); // ThreadStarter is daemon, but we don't
                                      // want sampler threads to be so too
          registerStartedThread(jmThread, newThread);
          newThread.start();
        }
      } catch (Exception ex) {
