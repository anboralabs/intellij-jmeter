...
public class JMeterThread implements Runnable, Interruptible {
  ...
  private static final boolean APPLY_TIMER_FACTOR =
      Float.compare(TIMER_FACTOR, ONE_AS_FLOAT) != 0;
  ...
  // TestIterationListener#testIterationStart Since this is a frequent event, it
  // makes sense to create the list once rather than scanning each time The
  // memory used will be released when the thread finishes
   ...
   * This is done before start() is called, so the values will be published to
   * the thread safely
   * TODO - consider passing them to the constructor, so that they can be made
   * final (to avoid adding lots of parameters, perhaps have a parameter wrapper
   * object.
   */
  ...
  private final ReentrantLock interruptLock =
      new ReentrantLock(); // ensure that interrupt cannot overlap with shutdown
  ...
  public JMeterThread(HashTree test, JMeterThreadMonitor monitor,
    ...
    SearchByClass<TestIterationListener> threadListenerSearcher =
        new SearchByClass<>(TestIterationListener.class); // TL - IS
    test.traverse(threadListenerSearcher);
    testIterationStartListeners = threadListenerSearcher.getSearchResults();
    SearchByClass<SampleMonitor> sampleMonitorSearcher =
        new SearchByClass<>(SampleMonitor.class);
  ...
  /**
   * Updates the variables with all entries found in the variables in {@code
   * vars}
   ...
   */
  public void setScheduled(boolean sche) { this.scheduler = sche; }
   ...
   */
  public void setStartTime(long stime) { startTime = stime; }
   ...
   *            the EndTime value.
   */
  public void setEndTime(long etime) { endTime = etime; }
  ...
  public void run() {
          ...
          if (threadContext.getTestLogicalAction() !=
            ...
            if (log.isDebugEnabled() && onErrorStartNextLoop &&
                threadContext.getTestLogicalAction() !=
                    TestLogicalAction.CONTINUE) {
              log.debug(
                  "Start Next Thread Loop option is on, Last sample failed, starting next thread loop");
            }
            if (onErrorStartNextLoop && !lastSampleOk) {
              triggerLoopLogicalActionOnParentControllers(
                  sam, threadContext, JMeterThread::continueOnThreadLoop);
            } else {
              switch (threadContext.getTestLogicalAction()) {
              case BREAK_CURRENT_LOOP:
                triggerLoopLogicalActionOnParentControllers(
                    sam, threadContext, JMeterThread::breakOnCurrentLoop);
                break;
              ...
              case START_NEXT_ITERATION_OF_CURRENT_LOOP:
                triggerLoopLogicalActionOnParentControllers(
                    sam, threadContext, JMeterThread::continueOnCurrentLoop);
                break;
              default:
                break;
              }
    ...
    // Might be found by controller.next()
      ...
      if (log.isInfoEnabled()) {
        log.info("Stop Thread seen for thread {}, reason: {}", getThreadName(),
                 e.toString());
      }
      ...
      interruptLock.lock(); // make sure current interrupt is finished, prevent
                            // another starting yet
      try {
        threadContext.clear();
        ...
        interruptLock.unlock(); // Allow any pending interrupt to complete (OK
                                // because currentSampler == null)
      }
    }
  }

  /**
   * Trigger break/continue/switch to next thread Loop  depending on consumer
   * implementation
   * @param sampler Sampler Base sampler
   * @param threadContext
   * @param consumer Consumer that will process the tree of elements up to root
   *     node
   */
  private void triggerLoopLogicalActionOnParentControllers(
      Sampler sampler, JMeterContext threadContext,
    ...
    if (realSampler == null) {
      throw new IllegalStateException(
          ...
          (sampler != null ? sampler.getName() : "null") +
          ", sampler:" + sampler);
    }
    // Find parent controllers of current sampler
    FindTestElementsUpToRootTraverser pathToRootTraverser =
        new FindTestElementsUpToRootTraverser(realSampler);
    ...
    // then we still need to report the Transaction in error (and create the
    // sample result)
    if (transactionSampler != null) {
      SamplePackage transactionPack =
          compiler.configureTransactionSampler(transactionSampler);
      doEndTransactionSampler(transactionSampler, null, transactionPack,
                              threadContext);
    }
  }
   ...
   */
  private static void
  continueOnCurrentLoop(FindTestElementsUpToRootTraverser pathToRootTraverser) {
   ...
   */
  private static void
  breakOnCurrentLoop(FindTestElementsUpToRootTraverser pathToRootTraverser) {
    List<Controller> controllersToReinit =
        pathToRootTraverser.getControllersToRoot();
    for (Controller parentController : controllersToReinit) {
      if (parentController instanceof AbstractThreadGroup) {
  ...
  /**
   * Executes a restart of Thread loop, equivalent of "continue" in algorithm
   * but on Thread Loop. As a consequence it ends all loop on the path to root
   * @param pathToRootTraverser {@link FindTestElementsUpToRootTraverser}
   */
  private static void
  continueOnThreadLoop(FindTestElementsUpToRootTraverser pathToRootTraverser) {
    List<Controller> controllersToReinit =
        pathToRootTraverser.getControllersToRoot();
    for (Controller parentController : controllersToReinit) {
      if (parentController instanceof AbstractThreadGroup) {
   ...
   * error The Sampler provided is not always the "real" one, it can be a
   * TransactionSampler, if there are some other controllers (SimpleController
   * or other implementations) between this TransactionSampler and the real
   * sampler, triggerEndOfLoop will not be called for those controllers leaving
   * them in "ugly" state. the following method will try to find the sampler
   * that really generate an error
   * @return {@link Sampler}
   */
   ...
   */
  private SampleResult processSampler(Sampler current, Sampler parent,
    ...
    try {
      if (current instanceof TransactionSampler) {
        ...
        if (transactionSampler.isTransactionDone()) {
          transactionResult = doEndTransactionSampler(
              transactionSampler, parent, transactionPack, threadContext);
          ...
          if (current instanceof TransactionSampler) {
            SampleResult res =
                processSampler(current, prev, threadContext); // recursive call
      ...
      if (current != null) {
        executeSamplePackage(current, transactionSampler, transactionPack,
                             threadContext);
      }
      ...
      if (current != null) {
        log.error("Error while processing sampler: '{}'.", current.getName(),
                  e);
      } else {
        log.error("Error while processing sampler.", e);
      }
    }
    if (!running && transactionResult == null && transactionSampler != null &&
   ...
   * interruption</li> <li>Playing SampleMonitor before and after sampling</li>
   ...
   */
  private SampleResult doSampling(JMeterContext threadContext,
                                  Sampler sampler) {
  ...
  doEndTransactionSampler(TransactionSampler transactionSampler, Sampler parent,
                          SamplePackage transactionPack,
                          JMeterContext threadContext) {
  ...
  /**
   * Get the SampleListeners for the sampler. Listeners who receive transaction
   * sample will not be in this list.
   *
   * @param samplePack
   ...
   */
  private static List<SampleListener>
  getSampleListeners(SamplePackage samplePack, SamplePackage transactionPack,
                     TransactionSampler transactionSampler) {
    ...
    if (transactionSampler != null) {
      ...
      List<SampleListener> transListeners =
          transactionPack.getSampleListeners();
   ...
   */
  private static void setLastSampleOk(JMeterVariables variables,
                                      boolean value) {
  ...
  private IterationListener initRun(JMeterContext threadContext) {
    threadVars.putObject(JMeterVariables.VAR_IS_SAME_USER_KEY,
  ...
  static class ThreadListenerTraverser implements HashTreeTraverser {
    ...
    ThreadListenerTraverser(boolean start) { isStart = start; }
  ...
  /**
   * Set running flag to false which will interrupt JMeterThread on next flag
   * test. This is a clean shutdown.
   */
  public void
  stop() { // Called by StandardJMeterEngine, TestAction and AccessLogSampler
    running = false;
  ...
  public boolean interrupt() {
    ...
    try {
      Sampler samp =
          currentSamplerForInterruption; // fetch once; must be done under lock
  ...
  /**
   * Clean shutdown of test, which means wait for end of current running
   * samplers
   */
  ...
  private static void recurseAssertionChecks(SampleResult parent,
                                             Assertion assertion, int level) {
    if (level < 0) {
      return;
    }
    ...
    // If parent is OK, but child failed, add a message and flag the parent as
    // failed
  ...
  private static void processAssertion(SampleResult result,
                                       Assertion assertion) {
    AssertionResult assertionResult;
    try {
  ...
  private static void
  runPostProcessors(List<? extends PostProcessor> extractors) {
  ...
  private static void
  runPreProcessors(List<? extends PreProcessor> preProcessors) {
  ...
  private void delay(List<? extends Timer> timers) {
    ...
    for (Timer timer : timers) {
      ...
      if (APPLY_TIMER_FACTOR && timer.isModifiable()) {
        if (log.isDebugEnabled()) {
          log.debug("Applying TIMER_FACTOR:{} on timer:{} for thread:{}",
                    TIMER_FACTOR, ((TestElement)timer).getName(),
                    getThreadName());
        }
    ...
    if (totalDelay > 0) {
      try {
        if (scheduler) {
          // We reduce pause to ensure end of test is not delayed by a sleep
          // ending after test scheduled end See Bug 60049
          totalDelay = TIMER_SERVICE.adjustDelay(totalDelay, endTime, false);
          if (totalDelay < 0) {
            log.debug(
                "The delay would be longer than the scheduled period, so stop thread now.");
            running = false;
            return;
          }
        }
        ...
        log.warn(
            "The delay timer was interrupted - probably did not wait as long as intended.");
  ...
  void notifyTestListeners() {
    ...
    for (TestIterationListener listener : testIterationStartListeners) {
      listener.testIterationStart(new LoopIterationEvent(
          threadGroupLoopController, threadVars.getIteration()));
  ...
  private void notifyListeners(List<SampleListener> listeners,
                               SampleResult result) {
    SampleEvent event =
        new SampleEvent(result, threadGroup.getName(), threadVars);
   ...
   */
  private void rampUpDelay() { delayBy(initialDelay, "RampUp"); }
    ...
    if (delay > 0) {
      ...
      while (running && (now = System.currentTimeMillis()) < end) {
        ...
        try {
          ...
          if (running) { // NOSONAR running may have been changed from another
                         // thread
            log.warn(
                "{} delay for {} was interrupted. Waited {} milli-seconds out of {}",
                type, threadName, now - start, delay);
          }
          Thread.currentThread().interrupt();
          break;
        }
   ...
   */
  public void setOnErrorStopTest(boolean b) { onErrorStopTest = b; }
   ...
   * @param b true or false
   */
  public void setOnErrorStopTestNow(boolean b) { onErrorStopTestNow = b; }
   ...
   * @param b true or false
   */
  public void setOnErrorStopThread(boolean b) { onErrorStopThread = b; }
   ...
   * @param b true or false
   */
  public void setOnErrorStartNextLoop(boolean b) { onErrorStartNextLoop = b; }
   ...
   */
  public ListedHashTree getTestTree() { return (ListedHashTree)testTree; }

  /**
   * @return {@link ListenerNotifier}
   */
  public ListenerNotifier getNotifier() { return notifier; }
}
