...
public abstract class AbstractThreadGroupGui
  ...
  public JPopupMenu createPopupMenu() {
    ...
    if (this.isEnabled() && !JMeterUtils.isTestRunning()) {
      ...
      pop.add(createMenuItem("add_think_times",
                             ActionNames.ADD_THINK_TIME_BETWEEN_EACH_STEP));
      pop.add(createMenuItem("run_threadgroup", ActionNames.RUN_TG));
      pop.add(createMenuItem("run_threadgroup_no_timers",
                             ActionNames.RUN_TG_NO_TIMERS));
  ...
  private static JMenuItem createMenuItem(String name, String actionCommand) {
    JMenuItem addThinkTimesToChildren =
        new JMenuItem(JMeterUtils.getResString(name));
  ...
  // WARNING: called from ctor so must not be overridden (i.e. must be private
  // or final)
  private void init() {
  ...
  private void initGui() { continueBox.setSelected(true); }
  ...
  private JPanel createOnErrorPanel() {
    ...
    startNextLoop = new JRadioButton(JMeterUtils.getResString(
        "sampler_on_error_start_next_loop")); // $NON-NLS-1$
    group.add(startNextLoop);
    panel.add(startNextLoop);

    stopThreadBox = new JRadioButton(JMeterUtils.getResString(
    ...
    stopTestNowBox = new JRadioButton(JMeterUtils.getResString(
        "sampler_on_error_stop_test_now")); // $NON-NLS-1$
