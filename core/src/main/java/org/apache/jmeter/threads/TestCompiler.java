 ...
 *  <li>A map with key TransactionController and as value the associated
 * SamplePackage</li>
 * </ul>
 */
...
public class TestCompiler implements HashTreeTraverser {
   ...
   * It seems to be used to prevent adding a child to a parent if the child has
   * already been added. If the ObjectPair (child, parent) is present, then the
   * child has been added. Otherwise, the child is added to the parent and the
  ...
  private final IdentityHashMap<Sampler, SamplePackage> samplerConfigMap =
      new IdentityHashMap<>();

  private final IdentityHashMap<TransactionController, SamplePackage>
      transactionControllerConfigMap = new IdentityHashMap<>();

  private final HashTree testTree;

  public TestCompiler(HashTree testTree) { this.testTree = testTree; }
  ...
  /**
   * Configures sampler from SamplePackage extracted from Test plan and returns
   * it
  ...
  /**
   * Configures Transaction Sampler from SamplePackage extracted from Test plan
   * and returns it
   ...
   */
  public void done(SamplePackage pack) { pack.recoverRunningVersion(); }
    ...
    if (!stack.isEmpty()) {
      ...
      if (duplicate) {
        if (log.isWarnEnabled()) {
          log.warn("Unexpected duplicate for {} and {}", parent.getClass(),
                   child.getClass());
        }
      }
    }
  ...
  private void saveSamplerConfigs(Sampler sam) {
    ...
    SamplePackage pack = new SamplePackage(
        configs, listeners, timers, assertions, posts, pres, controllers);
    pack.setSampler(sam);
   ...
   */
  private static void
  addDirectParentControllers(List<? super Controller> controllers,
                             TestElement maybeController) {
  ...
  private static void
  configureWithConfigElements(Sampler sam,
                              List<? extends ConfigTestElement> configs) {
