...
public abstract class AbstractJMeterGuiComponent
  ...
  private static final Logger log =
      LoggerFactory.getLogger(AbstractJMeterGuiComponent.class);
   ...
   * @deprecated use {@link #getName()} or {@link
   *AbstractJMeterGuiComponent#createTitleLabel()} for better alignment of the
   *fields
   **/
  ...
  private final JTextArea commentField =
      JFactory.tabMovesFocus(new JTextArea());

  /**
   * Stores a collection of property editors, so GuiCompoenent can have default
   * implementations that update the UI fields based on {@link TestElement}
  ...
  /**
   * Provides a default implementation for setting the name property. It's
   ...
   */
  public void setComment(String comment) { commentField.setText(comment); }
   ...
   * @deprecated use {@link #getName()} or {@link
   *     AbstractJMeterGuiComponent#createTitleLabel()} for better alignment of
   *     the fields
   */
  ...
  /**
   * Provides a default implementation that resets the name field to the value
   * of getStaticLabel(), reset comment and sets enabled to true. Your GUI may
   * need more things cleared, in which case you should override, clear the
   * extra fields, and still call super.clearGui().
   */
  @Override
   ...
   * behavior.
   * <p>Deprecation notice: most likely you do not need the method, and you
   * should override {@link #modifyTestElement(TestElement)} instead</p>
   *
   * @param mc
  ...
  /**
   * Assigns basic fields from UI to the test element: name, comments, gui
   * class, and the registered editors.
   *
  ...
  private void modifyTestElementEnabledAndComment(TestElement mc) {
    ...
    // elements with enabled=false (~HTTP Request Defaults) can't unexpectedly
    // override the regular ones like HTTP Request.
    mc.set(TestElementSchema.INSTANCE.getEnabled(),
           enabled ? null : Boolean.FALSE);
    // Note: we can't use editors for "comments" as getComments() is not a final
    // method, so plugins might override it and provide a different
    // implementation.
  ...
  protected Container makeTitlePanel() {
    JPanel titlePanel =
        new JPanel(new MigLayout("fillx, wrap 2, insets 0", "[][fill,grow]"));
    ...
    // Note: VerticalPanel has a workaround for Box layout which aligns
    // elements, so we can't use trivial JPanel. Extra wrapper is often required
    // to ensure that further additions to the panel would be vertical For
    // instance AbstractVisualizer adds "browse file" panel If it calls just
    // ..add(browseFilePanel), then it will go to
   ...
   * @return a JScrollPane containing the specified component
   */
  protected JScrollPane makeScrollPane(Component comp, int verticalPolicy,
  ...
  /**
   * Compute Anchor value to find reference in documentation for a particular
   * component
  ...
  public String getDocAnchor() {
    ...
    String label =
        JMeterUtils.getResString(getLabelResource(), new Locale("", ""));
