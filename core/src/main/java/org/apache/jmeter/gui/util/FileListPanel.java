...
public class FileListPanel extends JPanel implements ActionListener {
  ...
  private final JButton browse =
      new JButton(JMeterUtils.getResString(ACTION_BROWSE));

  private final JButton clear =
      new JButton(JMeterUtils.getResString("clear")); // $NON-NLS-1$

  private final JButton delete =
      new JButton(JMeterUtils.getResString("delete")); // $NON-NLS-1$
   ...
   */
  public void addFilename(String f) { tableModel.addRow(f); }

  /**
   * clear the files from the table
   */
  public void clearFiles() { tableModel.clearData(); }
  ...
  protected void initializeTableModel() {
    tableModel = new ObjectTableModel(
        new String[] {JMeterUtils.getResString(LABEL_LIBRARY)}, new Functor[0],
        new Functor[0], // i.e. bypass the Functors
