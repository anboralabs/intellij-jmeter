 ...
 *      <li>Validate a set of Thread Groups with/without sleeping on the timers
 * depending on jmeter properties</li>
 * </ul>
 */
...
public class Start extends AbstractAction {
  ...
  private enum RunMode { AS_IS, IGNORING_TIMERS, VALIDATION }
  ...
  /**
   * Implementation of {@link TreeCloner} used to clone the tree before running
   * validation
   */
  private static final String CLONER_FOR_VALIDATION_CLASS_NAME =
      JMeterUtils.getPropDefault(
          VALIDATION_CLONER_CLASS_PROPERTY_NAME, //$NON-NLS-1$
   ...
   */
  private static AbstractThreadGroup[]
  keepOnlyThreadGroups(JMeterTreeNode[] currentNodes) {
   ...
   */
  private void startEngine(AbstractThreadGroup[] threadGroupsToRun,
                           RunMode runMode) {
    GuiPackage gui = GuiPackage.getInstance();
    HashTree testTree = gui.getTreeModel().getTestPlan();

    // We need to make this conversion before removing any Thread Group as 1
    // thread Group running may reference another one (not running) using
    // ModuleController We don't clone as we'll be doing it later AND we cannot
    // clone before we have removed the unselected ThreadGroups
   ...
   */
  private static TreeCloner
  createTreeClonerForValidation(boolean honorThreadClone) {
    Class<?> clazz;
    try {
      clazz = Class.forName(CLONER_FOR_VALIDATION_CLASS_NAME, true,
                            Thread.currentThread().getContextClassLoader());
      return (TreeCloner)clazz.getConstructor(boolean.class)
          .newInstance(honorThreadClone);
    } catch (InstantiationException | IllegalAccessException |
             ClassNotFoundException | NoSuchMethodException |
             InvocationTargetException ex) {
      log.error("Error instantiating class:'{}' defined in property:'{}'",
                CLONER_FOR_VALIDATION_CLASS_NAME,
   ...
   */
  private static void keepOnlySelectedThreadGroupsInHashTree(
      HashTree testTree, AbstractThreadGroup[] threadGroupsToKeep) {
    for (Object o : new ArrayList<>(testTree.list())) {
      ...
      if (o instanceof AbstractThreadGroup) {
        if (!isInThreadGroups(item, threadGroupsToKeep)) {
          ...
          // where 2 AbstractTestElement can be equals but have different
          // hashcode
          try {
          ...
          keepOnlySelectedThreadGroupsInHashTree(testTree.getTree(item),
                                                 threadGroupsToKeep);
        }
      } else {
        keepOnlySelectedThreadGroupsInHashTree(testTree.getTree(item),
   ...
   */
  private static boolean isInThreadGroups(TestElement item,
