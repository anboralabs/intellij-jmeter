...
public class JMeterTreeTransferHandler extends TransferHandler {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(JMeterTreeTransferHandler.class);
  ...
  public JMeterTreeTransferHandler() {
    try {
      ...
      String jvmLocalFlavor = DataFlavor.javaJVMLocalObjectMimeType +
                              ";class=\"" + JMeterTreeNode[].class.getName() +
                              "\"";
        ...
        for (int j = 0; j < paths.length; j++) {
          if (i != j &&
              ((JMeterTreeNode)paths[i].getLastPathComponent())
                  .isNodeAncestor(
                      (JMeterTreeNode)paths[j].getLastPathComponent())) {
            toRemove[i] = true;
            size--;
            break;
          }
  ...
  public boolean importData(TransferHandler.TransferSupport support) {
    ...
    for (JMeterTreeNode node : nodes) {
      ...
      // Insert a clone of the node, the original one will be removed by the
      // exportDone method the children are not cloned but moved to the cloned
      // node working on the original node would be harder as
      //    you'll have to deal with the insertion index offset if you re-order
      //    a node inside a parent
      JMeterTreeNode copy = (JMeterTreeNode)node.clone();

      // first copy the children as the call to copy.add will modify the
      ...
      pathsToSelect[pathPosition++] =
          new TreePath(treeModel.getPathToRoot(copy));
    }
  ...
  private class NodesTransferable implements Transferable {
    ...
    public NodesTransferable(JMeterTreeNode[] nodes) { this.nodes = nodes; }
