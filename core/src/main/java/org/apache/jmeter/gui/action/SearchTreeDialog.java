...
public class SearchTreeDialog
    extends JDialog implements ActionListener { // NOSONAR

  private static final long serialVersionUID = -4436834972710248247L;

  private static final Logger logger =
      LoggerFactory.getLogger(SearchTreeDialog.class);
  ...
  public SearchTreeDialog(JFrame parent) {
    super(parent, JMeterUtils.getResString("search_tree_title"),
          false); //$NON-NLS-1$
    init();
  }
  ...
  private void init() { // WARNING: called from ctor so must not be overridden
    ...
    isRegexpCB =
        new JCheckBox(JMeterUtils.getResString("search_text_chkbox_regexp"),
                      false); //$NON-NLS-1$
    isCaseSensitiveCB =
        new JCheckBox(JMeterUtils.getResString("search_text_chkbox_case"),
                      true); //$NON-NLS-1$
    ...
    searchCriterionPanel.setBorder(BorderFactory.createTitledBorder(
        JMeterUtils.getResString("search_matching"))); //$NON-NLS-1$
    ...
    searchAndExpandButton =
        createButton("search_search_all_expand"); //$NON-NLS-1$
    ...
    replaceAndFindButton =
        createButton("search_replace_and_find"); //$NON-NLS-1$
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    } else if (source == searchButton || source == searchAndExpandButton) {
      doSearch(e);
  ...
  private static void doResetSearch(ActionEvent event) {
    ActionRouter.getInstance().doActionNow(new ActionEvent(
        event.getSource(), event.getID(), ActionNames.SEARCH_RESET));
  }
  ...
  private void doReplace() {
    ...
    if (currentSearchIndex >= 0) {
      ...
      if (currentNode != null) {
        ...
        String regex = isRegexpCB.isSelected() ? wordToSearch
                                               : Pattern.quote(wordToSearch);
        boolean caseSensitiveReplacement = isCaseSensitiveCB.isSelected();
        Pair<Integer, JMeterTreeNode> pair = doReplacementInCurrentNode(
            currentNode, regex, wordToReplace, caseSensitiveReplacement);
        if (pair != null) {
  ...
  private JMeterTreeNode doNavigateToSearchResult(boolean isNext) {
    boolean doSearchAgain =
        ...
        !Triple
             .of(searchTF.getText(), isCaseSensitiveCB.isSelected(),
    ...
    if (doSearchAgain) {
      ...
      if (StringUtils.isEmpty(wordToSearch)) {
        ...
        this.lastSearchConditions =
            Triple.of(wordToSearch, isCaseSensitiveCB.isSelected(),
                      isRegexpCB.isSelected());
      }
    ...
    if (!lastSearchResult.isEmpty()) {
      ...
      GuiPackage.getInstance().getMainFrame().getTree().setSelectionPath(
          selection);
      GuiPackage.getInstance().getMainFrame().getTree().scrollPathToVisible(
          selection);
      return selectedNode;
    }
    return null;
  ...
  private void doSearch(ActionEvent e) {
    ...
    try {
      Pair<Integer, Set<JMeterTreeNode>> result =
          searchInTree(guiPackage, searcher, wordToSearch);
  ...
  private Pair<Integer, Set<JMeterTreeNode>>
  searchInTree(GuiPackage guiPackage, Searcher searcher, String wordToSearch) {
    int numberOfMatches = 0;
    ...
    for (JMeterTreeNode jMeterTreeNode :
      ...
      try {
        ...
        logger.error("Error occurred searching for word:{} in node:{}",
                     wordToSearch, jMeterTreeNode.getName(), ex);
      }
    }
   ...
   */
  private static void markConcernedNodes(boolean expand,
                                         Set<? extends JMeterTreeNode> nodes) {
      ...
      if (expand) {
        if (jMeterTreeNode.isLeaf()) {
          jTree.expandPath(new TreePath(
              ((JMeterTreeNode)jMeterTreeNode.getParent()).getPath()));
        } else {
  ...
  /**
   * Replace all occurrences in nodes that contain {@link Replaceable} Test
   * Elements
   * @param e {@link ActionEvent}
   */
  private void doReplaceAll(ActionEvent e) {
    ...
    Searcher searcher = createSearcher(wordToSearch);
    String regex =
        isRegexpCB.isSelected() ? wordToSearch : Pattern.quote(wordToSearch);
    GuiPackage guiPackage = GuiPackage.getInstance();
    boolean caseSensitiveReplacement = isCaseSensitiveCB.isSelected();
    int totalReplaced = 0;
    Pair<Integer, Set<JMeterTreeNode>> result =
    ...
    for (JMeterTreeNode jMeterTreeNode : matchingNodes) {
      Pair<Integer, JMeterTreeNode> pair = doReplacementInCurrentNode(
          jMeterTreeNode, regex, wordToReplace, caseSensitiveReplacement);
      if (pair != null) {
   ...
   * @return null if no replacement occurred or Pair of (number of replacement,
   *     current tree node)
   */
  private static Pair<Integer, JMeterTreeNode>
  doReplacementInCurrentNode(JMeterTreeNode jMeterTreeNode, String regex,
                             String replaceBy,
    ...
    try {
      if (jMeterTreeNode.getUserObject() instanceof Replaceable) {
        ...
        int numberOfReplacements =
            replaceable.replace(regex, replaceBy, caseSensitiveReplacement);
        if (numberOfReplacements > 0) {
          if (logger.isInfoEnabled()) {
            logger.info(
                "Replaced {} in element:{}", numberOfReplacements,
      ...
      logger.error("Error occurred replacing data in node:{}",
                   jMeterTreeNode.getName(), ex);
    }
    return null;
  }
