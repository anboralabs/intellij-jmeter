...
public class JMeterTreeModel extends DefaultTreeModel {
   ...
   * @return a list of tree nodes of the given <code>type</code>, or an empty
   *     list
   */
   ...
   * @param component The {@link TestElement} to be used as data for the newly
   *     created node
   * @param node The {@link JMeterTreeNode} into which the newly created node is
   *     to be inserted
   * @return new {@link JMeterTreeNode} for the given <code>component</code>
   * @throws IllegalUserActionException
   *             when the user object for the <code>node</code> is not an
   * instance of {@link AbstractConfigGui}
   */
  ...
  @SuppressWarnings("JdkObsolete")
  private static JMeterTreeNode traverseAndFind(TestElement userObject,
                                                JMeterTreeNode node) {
   ...
   * @param node The {@link JMeterTreeNode} from which the sub tree is to be
   *     taken
  ...
  private void initTree(TestElement tp) {
    ...
    // This should not be necessary, but without it, nodes are not shown when
    // the user uses the Close menu item
   ...
   */
  private void moveWorkBenchToTestPlan(JMeterTreeNode current,
                                       HashTree workbenchTree)
      throws IllegalUserActionException {
    ...
    if (!workbenchTree.isEmpty()) {
      ...
      TestFragmentController testFragmentController =
          new TestFragmentController();
      testFragmentController.setName("WorkBench Test Fragment");
      testFragmentController.setProperty(
          TestElement.GUI_CLASS, TestFragmentControllerGui.class.getName());
