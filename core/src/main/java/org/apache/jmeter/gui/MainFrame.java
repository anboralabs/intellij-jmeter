 ...
 */
public class MainFrame extends JPanel implements TestStateListener, Remoteable,
                                                 DropTargetListener, Clearable,
                                                 ActionListener {
  ...
  private static final boolean DISPLAY_LOGGER_PANEL =
      JMeterUtils.getPropDefault("jmeter.loggerpanel.display",
                                 false); // $NON-NLS-1$
  ...
  private final ImageIcon runningIcon = JMeterUtils.getImage(
      "status/" + iconSize + "/user-online-2.png"); // $NON-NLS-1$

  /** An image which is displayed when a test is not currently running. */
  private final ImageIcon stoppedIcon = JMeterUtils.getImage(
      "status/" + iconSize + "/user-offline-2.png"); // $NON-NLS-1$

  /** An image which is displayed to indicate FATAL, ERROR or WARNING. */
  private final ImageIcon warningIcon = JMeterUtils.getImage(
      "status/" + iconSize + "/pictogram-din-w000-general.png"); // $NON-NLS-1$
  ...
  private final javax.swing.Timer computeTestDurationTimer =
      new javax.swing.Timer(1000, this::computeTestDuration);

  private final AtomicInteger errorOrFatal = new AtomicInteger(0);

  private final javax.swing.Timer refreshErrorsTimer =
      new javax.swing.Timer(1000, this::refreshErrors);
  ...
  public MainFrame(TreeModel treeModel, JMeterTreeListener treeListener) {
    ...
    testTimeDuration.setToolTipText(
        JMeterUtils.getResString("duration_tooltip")); //$NON-NLS-1$

    activeAndTotalThreads = new JLabel("0/0"); // $NON-NLS-1$
    activeAndTotalThreads.setToolTipText(JMeterUtils.getResString(
        "active_total_threads_tooltip")); // $NON-NLS-1$
    ...
    warnIndicator.setToolTipText(
        JMeterUtils.getResString("error_indicator_tooltip")); // $NON-NLS-1$
    ...
    addMouseWheelListener(e -> {
      if (e.getWheelRotation() == 0) {
        // Nothing to do here. This happens when scroll event is delivered from
        // a touchbar or MagicMouse. There's getPreciseWheelRotation, however it
        // looks like there's no trivial and consistent way to use that See
        // https://github.com/JetBrains/intellij-community/blob/21c99af7c78fc82aefc4d05646389f4991b08b38/bin/idea.properties#L133-L156
        return;
      }
      // Shift down means "horizontal scrolling" on macOS, and we need only
      // vertical one
      if ((e.getModifiersEx() & (ctrlAltMask | InputEvent.SHIFT_DOWN_MASK)) ==
          ctrlAltMask) {
        e.consume();
    ...
    addPropertyChangeListener("graphicsConfiguration", evt -> {
      // Update UI when JMeter window moves to a different monitor as it might
      // have different scaling settings
      JMeterUtils.refreshUI();
    });
  }
  ...
  private void refreshErrors(ActionEvent evt) {
    if (errorOrFatal.get() > 0) {
      warnIndicator.setForeground(
          UIManager.getColor(JMeterUIDefaults.BUTTON_ERROR_FOREGROUND));
   ...
   */
  public void setEditAddMenu(JMenu menu) { menuBar.setEditAddMenu(menu); }
  ...
  public void showStoppingMessage(String host) {
    ...
    JMeterUtils.getResString("stopping_test_title"), true); //$NON-NLS-1$ String
    label = JMeterUtils.getResString("stopping_test"); //$NON-NLS-1 if
    (!StringUtils.isEmpty(host)) { label = label +
    JMeterUtils.getResString("stopping_test_host")+ ": " + host;
    }
  ...
  public void updateCounts() {
    SwingUtilities.invokeLater(
        ()
            -> activeAndTotalThreads.setText(String.format(
                "%d/%d", JMeterContextService.getNumberOfThreads(),
                JMeterContextService.getTotalThreads())));
  }

  public void setMainPanel(JComponent comp) { mainPanel.setViewportView(comp); }
  ...
  private void init() { // WARNING: called from ctor so must not be overridden
    ...
    GuiPackage.getInstance().getLogEventBus().registerEventListener(
        errorsAndFatalsCounterLogTarget);
    ...
    // $NON-NLS-1$ setWindowTitle(); // define AWT WM_CLASS string
   ...
   */
  public void initTopLevelDndHandler() { new DropTarget(this, this); }
  ...
  public void setExtendedFrameTitle(String fname) {
    ...
    String simpleName =
        temp.substring(temp.lastIndexOf('/') + 1); // $NON-NLS-1$
   ...
   */
  private static JScrollPane createMainPanel() { return new JScrollPane(); }
  ...
  private static void addQuickComponentHotkeys(JTree treevar) {
    ...
    for (int n = 0; n < keyStrokes.length; n++) {
      treevar.getActionMap().put(
          ActionNames.QUICK_COMPONENT + String.valueOf(n), quickComponent);
      inputMap.put(keyStrokes[n],
                   ActionNames.QUICK_COMPONENT + String.valueOf(n));
    }
  }
  ...
  private static final class QuickComponent extends AbstractAction {
    ...
    private QuickComponent(String name) { super(name); }
  ...
  public boolean openJmxFilesFromDragAndDrop(Transferable tr)
    ...
    List<File> files =
        (List<File>)tr.getTransferData(DataFlavor.javaFileListFlavor);
    if (files.isEmpty()) {
      return false;
    }
    ...
    if (!file.getName().endsWith(".jmx")) {
      if (log.isWarnEnabled()) {
        log.warn(
            "Importing file, {}, from DnD failed because file extension does not end with .jmx",
            file.getName());
      }
      return false;
    }

    ActionEvent fakeEvent =
        new ActionEvent(this, ActionEvent.ACTION_PERFORMED, ActionNames.OPEN);
  ...
  private static void setWindowTitle() {
    ...
    if (xtoolkit.getName().equals(
            "sun.awt.X11.XToolkit")) { // NOSONAR (we don't want to depend on
                                       // native LAF) $NON-NLS-1$
      try {
        ...
        // By default, strong encapsulation prevents setAccessible on
        // java.desktop
        if ("java.lang.reflect.InaccessibleObjectException".equals(
                e.getClass().getName())) {
          /* ignore */
