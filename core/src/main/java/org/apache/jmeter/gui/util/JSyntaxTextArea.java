...
public class JSyntaxTextArea extends RSyntaxTextArea {
  ...
  private static final Theme DEFAULT_THEME =
      loadTheme(Theme.class, "themes/default.xml");

  private final Properties languageProperties = JMeterUtils.loadProperties(
      "org/apache/jmeter/gui/util/textarea.properties"); //$NON-NLS-1$
  ...
  private static final HierarchyListener GUTTER_THEME_PATCHER = e -> {
    if ((e.getChangeFlags() & HierarchyEvent.PARENT_CHANGED) != 0 &&
      ...
      SwingUtilities.invokeLater(
          () -> applyTheme((JSyntaxTextArea)e.getChanged()));
    }
  };
  ...
  public static JSyntaxTextArea getInstance(int rows, int cols,
    ...
    try {
      JSyntaxTextArea jSyntaxTextArea =
          new JSyntaxTextArea(rows, cols, disableUndo);
      JFactory.withDynamic(jSyntaxTextArea, JSyntaxTextArea::applyTheme);
      // Gutter styling is only applied if the text area is contained in a
      // scroll pane.
  ...
  private static void applyTheme(JSyntaxTextArea jSyntaxTextArea) {
    ...
    final Theme theme = isDarklafTheme
                            ? new DarklafRSyntaxTheme(jSyntaxTextArea)
                            : DEFAULT_THEME;
    if (theme != null) {
    ...
    if (!isDarklafTheme) {
      // Darklaf themes provide a custom background color for editors, so we
      // don't overwrite it.
  ...
  private JSyntaxTextArea(boolean dummy) { disableUndo = dummy; }
  ...
  public JSyntaxTextArea(int rows, int cols, boolean disableUndo) {
    ...
    if (USER_FONT_FAMILY != null) {
      ...
      setFont(
          JMeterUIDefaults.createFont(USER_FONT_FAMILY, Font.PLAIN, fontSize));
  ...
  public void setLanguage(String language) {
    if (language == null) {
      // TODO: Log a message?
      // But how to find the name of the offending GUI element in the case of a
      // TestBean?
   ...
   * See <a href="https://github.com/bobbylight/RSyntaxTextArea/issues/19">Issue
   * 19 on RSyntaxTextArea</a>
   */
  @Override
