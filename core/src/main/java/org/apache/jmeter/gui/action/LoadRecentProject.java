...
public class LoadRecentProject extends Load {
  ...
  static { commands.add(ActionNames.OPEN_RECENT); }

  private static final Preferences prefs =
      Preferences.userNodeForPackage(LoadRecentProject.class);
  // Note: Windows user preferences are stored relative to:
  // HKEY_CURRENT_USER\Software\JavaSoft\Prefs

  public LoadRecentProject() { super(); }
   ...
   *
   * @return a List of JMenuItem, representing recent files. JMenuItem may not
   *     be visible
   */
  ...
  public static void updateRecentFileMenuItems(List<JComponent> menuItems,
    ...
    // Get the preference for the recent files
    Deque<String> newRecentFiles =
        IntStream.range(0, NUMBER_OF_MENU_ITEMS)
  ...
  private static String getMenuItemDisplayName(File file) {
    ...
    if (menuText.length() > maxLength) {
      menuText = "..." + menuText.substring(menuText.length() - maxLength,
                                            menuText.length()); //$NON-NLS-1$
    }
    return menuText;
  }
  ...
  hasVisibleMenuItem(List<? extends JComponent> fileLoadRecentFiles) {
