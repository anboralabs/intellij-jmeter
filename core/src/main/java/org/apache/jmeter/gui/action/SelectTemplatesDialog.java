 ...
 * @since 2.10
 */
public class SelectTemplatesDialog extends JDialog
    implements ChangeListener, ActionListener,
               HyperlinkListener { // NOSONAR Ignore inheritence warning
  ...
  private static final Logger log =
      LoggerFactory.getLogger(SelectTemplatesDialog.class);

  private final JLabeledChoice templateList = new JLabeledChoice(
      JMeterUtils.getResString("template_choose"), false); //$NON-NLS-1$

  private final HtmlPane helpDoc = new HtmlPane();

  private final JButton reloadTemplateButton =
      new JButton(JMeterUtils.getResString("template_reload")); //$NON-NLS-1$

  private final JButton applyTemplateButton = new JButton();

  private final JButton cancelButton =
      new JButton(JMeterUtils.getResString("cancel")); //$NON-NLS-1$

  private final JButton previous =
      new JButton(JMeterUtils.getResString("previous")); //$NON-NLS-1$

  private final JButton validateButton = new JButton();

  private final Map<String, JLabeledTextField> parametersTextFields =
      new LinkedHashMap<>();

  private final JPanel actionBtnBar = new JPanel(new FlowLayout());

  public SelectTemplatesDialog() {
    super((JFrame)null, JMeterUtils.getResString("template_title"),
          true); //$NON-NLS-1$
    init();
  }
  ...
  private void checkDirtyAndLoad(final ActionEvent actionEvent)
    ...
    templateList.setValues(
        TemplateManager.getInstance()
            .getTemplateNames()); // reload the templates before loading
    ...
    ActionRouter.getInstance().doActionNow(new ActionEvent(
        actionEvent.getSource(), actionEvent.getID(), ActionNames.STOP_THREAD));
    final File parent = template.getParent();
    File fileToCopy = parent != null ? new File(parent, template.getFileName())
                                     : new File(JMeterUtils.getJMeterHome(),
                                                template.getFileName());
    replaceTemplateParametersAndLoad(actionEvent, template, isTestPlan,
                                     fileToCopy);
  }
    ...
    try {
      ...
      if (template.getParameters() != null &&
        ...
        try {
          ...
          TemplateUtil.processTemplate(jmxFile, temporaryGeneratedFile,
                                       templateCfg, userParameters);
        } catch (IOException | TemplateException ex) {
          log.error("Error generating output file {} from template {}",
                    temporaryGeneratedFile, jmxFile, ex);
          return;
        }
      }
      ...
      if (temporaryGeneratedFile != null && !temporaryGeneratedFile.delete()) {
        log.warn("Could not delete generated output file {} from template {}",
                 temporaryGeneratedFile, templateFile);
      }
    }
  }
  ...
  private static boolean checkDirty(final ActionEvent actionEvent) {
    ...
    if (guiPackage.isDirty()) {
      ...
      if (response == JOptionPane.YES_OPTION) {
        ActionRouter.getInstance().doActionNow(new ActionEvent(
            actionEvent.getSource(), actionEvent.getID(), ActionNames.SAVE));
        return true;
      }
  ...
  private Map<String, String> getUserParameters() {
    ...
    for (Map.Entry<String, JLabeledTextField> entry :
         parametersTextFields.entrySet()) {
  ...
  private void init() { // WARNING: called from ctor so must not be overridden
    ...
    ComponentUtil.centerComponentInWindow(
        this, 50); // center position and 50% of screen size
  ...
  private static boolean hasParameters(Template template) {
    return !(template.getParameters() == null ||
             template.getParameters().isEmpty());
  }
  ...
  private void resetJDialog(boolean reloadTemplates) {
    ...
    templateList.setValues(
        TemplateManager.getInstance().getTemplateNames()); // reload templates
    this.setContentPane(templateSelectionPanel());
    this.revalidate();
  }
  ...
  private JPanel configureParametersPanel(Map<String, String> parameters) {
    ...
    JLabel label =
        new JLabel(JMeterUtils.getResString("template_fill_parameters"));
