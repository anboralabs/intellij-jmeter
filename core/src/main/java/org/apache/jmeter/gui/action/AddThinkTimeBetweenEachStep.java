...
public class AddThinkTimeBetweenEachStep extends AbstractAction {
  private static final Logger log =
      LoggerFactory.getLogger(AddThinkTimeBetweenEachStep.class);
  ...
  static { commands.add(ActionNames.ADD_THINK_TIME_BETWEEN_EACH_STEP); }

  /**
   *
   */
  public AddThinkTimeBetweenEachStep() { super(); }
        ...
        if (insertThinkTime) {
          ...
          if (nodes.length != 2) {
            throw new IllegalArgumentException(
                "Invalid Think Time, expected 2 nodes, got:" + nodes.length);
          }
          index++;
   ...
   * @throws ReflectiveOperationException when class instantiation for {@value
   *     #DEFAULT_IMPLEMENTATION} fails
   * @throws IllegalUserActionException when {@link
   *     ThinkTimeCreator#createThinkTime(GuiPackage, JMeterTreeNode)} throws
   *     this
   */
  private static JMeterTreeNode[] createThinkTime(GuiPackage guiPackage,
                                                  JMeterTreeNode parentNode)
      throws ReflectiveOperationException, IllegalUserActionException {
    Class<?> clazz = Class.forName(DEFAULT_IMPLEMENTATION);
    ThinkTimeCreator thinkTimeCreator =
        (ThinkTimeCreator)clazz.getDeclaredConstructor().newInstance();
