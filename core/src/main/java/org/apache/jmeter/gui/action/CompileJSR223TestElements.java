...
public class CompileJSR223TestElements
    extends AbstractAction implements MenuCreator {
  ...
  private static final MessageFormat MESSAGE_FORMAT = new MessageFormat(
      JMeterUtils.getResString("compilation_errors")); // //$NON-NLS-1$
  /**
   *
   */
  private static class JSR223TestElementCompilerVisitor
    ...
    public JSR223TestElementCompilerVisitor() { super(); }
    @Override
  ...
  static { commands.add(ActionNames.COMPILE_JSR223); }

  public CompileJSR223TestElements() { super(); }
  ...
  public void doAction(ActionEvent e) {
    ...
    JSR223TestElementCompilerVisitor visitor =
        new JSR223TestElementCompilerVisitor();
    wholeTree.traverse(visitor);
    GuiPackage.getInstance().getMainFrame().repaint();
    if (visitor.getElementsWithCompilationErrors() > 0) {
      JMeterUtils.reportErrorToUser(MESSAGE_FORMAT.format(
          new Object[] {visitor.getElementsWithCompilationErrors()}));
    }
  }
  ...
  public JMenuItem[] getMenuItemsAtLocation(MENU_LOCATION location) {
    if (location == MENU_LOCATION.TOOLS) {
      ...
      JMenuItem menuItemIC =
          new JMenuItem(JMeterUtils.getResString(ActionNames.COMPILE_JSR223),
