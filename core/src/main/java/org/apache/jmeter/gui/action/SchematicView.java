...
public class SchematicView extends AbstractAction implements MenuCreator {
  ...
  private static final MessageFormat GENERATION_SUCCESS_MSG = new MessageFormat(
      JMeterUtils.getResString("schematic_view_generation_ok"));

  @FunctionalInterface
  public interface SchematicViewGenerator {
    void generate(HashTree testPlan, File testPlanFile,
  ...
  private static final class XslSchematicViewGenerator
    ...
    public void generate(HashTree testPlan, File testPlanFile,
      ...
      if (!StringUtils.isEmpty(DEFAULT_XSL_FILE)) {
        log.info("Will use file {} for Schematic View generation",
                 DEFAULT_XSL_FILE);
        xslt = new StreamSource(new File(DEFAULT_XSL_FILE));
      } else {
        xslt = new StreamSource(SchematicView.class.getResourceAsStream(
            "/org/apache/jmeter/gui/action/schematic.xsl"));
      }
  ...
  static { commands.add(ActionNames.SCHEMATIC_VIEW); }

  public SchematicView() { super(); }
  ...
  public JMenuItem[] getMenuItemsAtLocation(MENU_LOCATION location) {
    if (location == MENU_LOCATION.TOOLS) {
      ...
      JMenuItem menuItem =
          new JMenuItem(JMeterUtils.getResString(ActionNames.SCHEMATIC_VIEW),
