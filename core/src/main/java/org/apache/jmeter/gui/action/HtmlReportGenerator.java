...
public class HtmlReportGenerator {
  ...
  private static final long COMMAND_TIMEOUT = JMeterUtils.getPropDefault(
      "generate_report_ui.generation_timeout", 300_000L);

  private final String csvFilePath;
  private final String userPropertiesFilePath;
  private final String outputDirectoryPath;

  public HtmlReportGenerator(String csvFilePath, String userPropertiesFilePath,
                             String outputDirectoryPath) {
    this.csvFilePath = csvFilePath;
    this.userPropertiesFilePath = userPropertiesFilePath;
    if (outputDirectoryPath == null) {
      this.outputDirectoryPath =
          JMeterUtils.getJMeterBinDir() + "/report-output/";
    } else {
  ...
  public List<String> run() {
    ...
    try {
      SystemCommand sc = new SystemCommand(
          new File(JMeterUtils.getJMeterBinDir()), COMMAND_TIMEOUT, 100, null,
          null, commandExecutionOutput, null);
      LOGGER.debug("Running report generation");
      resultCode = sc.run(generationCommand);
      if (resultCode != 0) {
        errorMessageList.add(
            commandExecutionOutput.toString(Charset.defaultCharset().name()));
      ...
      errorMessageList.add(MessageFormat.format(
          JMeterUtils.getResString(
              "generate_report_ui.html_report_timeout_error"),
          COMMAND_TIMEOUT, e.getMessage(), commandExecutionOutput));
      LOGGER.error(
          "Report generation took more time than configured timeout (Property {}={}, command output=[{}])",
          "generate_report_ui.generation_timeout", COMMAND_TIMEOUT,
          commandExecutionOutput, e);
    } catch (InterruptedException | IOException e) {
      errorMessageList.add(MessageFormat.format(
          JMeterUtils.getResString(
              "generate_report_ui.html_report_unknown_error"),
    ...
    LOGGER.debug("SystemCommand ran: {}  returned: {}", generationCommand,
                 resultCode);
    return errorMessageList;
  }
  ...
  List<String> checkArguments() {
    ...
    if (csvError != null) {
      errors.add(JMeterUtils.getResString("generate_report_ui.csv_file") +
                 csvError);
    }
  ...
  private static String checkFile(File fileToCheck) {
    if (fileToCheck.exists() && fileToCheck.canRead() && fileToCheck.isFile()) {
      ...
      return MessageFormat.format(JMeterUtils.getResString(NO_FILE),
                                  fileToCheck);
    }
  }
    ...
    if (directoryToCheck.exists()) {
      ...
      if (files != null && files.length > 0) {
        return MessageFormat.format(
            JMeterUtils.getResString(NOT_EMPTY_DIRECTORY), directoryToCheck);
      } else {
        return null;
      }
    } else {
      ...
      if (parentDirectory != null && parentDirectory.exists() &&
        ...
        if (directoryToCheck.mkdir()) {
          ...
          return MessageFormat.format(
              JMeterUtils.getResString(CANNOT_CREATE_DIRECTORY),
              directoryToCheck);
        }
      } else {
        return MessageFormat.format(
