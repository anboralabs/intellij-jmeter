...
/**
 * Generates Configure pop-up dialogue for Listeners from all methods in
 * SampleSaveConfiguration with the signature "boolean saveXXX()". There must be
 * a corresponding "void setXXX(boolean)" method, and a property save_XXX which
 * is used to name the field on the dialogue.
 *
 */
public class SavePropertyDialog extends JDialog implements ActionListener {

  private static final Logger log =
      LoggerFactory.getLogger(SavePropertyDialog.class);
  ...
  private static final String RESOURCE_PREFIX =
      "save_"; // $NON-NLS-1$ e.g. save_XXX property
  ...
  private void initDialog() {
    ...
    final int configCount =
        (SampleSaveConfiguration.SAVE_CONFIG_NAMES.size() / 3) + 1;
    ...
    for (final String name : SampleSaveConfiguration.SAVE_CONFIG_NAMES) {
      try {
        ...
        final String actionCommand =
            SampleSaveConfiguration.setterName(name); // $NON-NLS-1$
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    f.invoke(saveConfig,
             new Object[] {((JCheckBox)e.getSource()).isSelected()});
  }

  private boolean getSaveState(String methodName)
