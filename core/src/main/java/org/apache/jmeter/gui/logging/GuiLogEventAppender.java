 ...
 * @since 3.2
 */
@Plugin(name = "GuiLogEvent", category = "Core", elementType = "appender",
        printObject = true)
public class GuiLogEventAppender extends AbstractAppender {

  protected GuiLogEventAppender(final String name, final Filter filter,
  ...
  public void append(LogEvent logEvent) {
    ...
    //       For example, if it fails to get GuiPackage#getInstance() due to
    //       static member initialization failure (e.g, accessing
    //       JMeterUtils#getPropDefault(...) without initializing application
    //       properties), the error log can be detected as "Recursive call to
    //       appender ..." by Log4j2 LoggerControl, resulting in no meaningful
    //       error logs in the log appender targets.
    GuiPackage instance = GuiPackage.getInstance();

    if (instance != null) {
      final String serializedString =
          getStringLayout().toSerializable(logEvent);

      if (serializedString != null && !serializedString.isEmpty()) {
        LogEventObject logEventObject =
            new LogEventObject(logEvent, serializedString);
  ...
  @PluginFactory
  public static GuiLogEventAppender
