...
public final class FileDialoger {
  ...
  /**
   * Prompts the user to choose a file or a directory from their filesystems for
   * our own devious uses. This method maintains the last directory the user
   * visited before dismissing the dialog. This does NOT imply they actually
   * chose a file from that directory, only that they closed the dialog there.
   * It is the caller's responsibility to check to see if the selected file is
   * non-null.
   *
   ...
   * used as the starting point for the returned JFileChooser.
   *
   * @return the JFileChooser that interacted with the user, after they are
   *         finished using it - null if no file was chosen
   */
  ...
  public static JFileChooser promptToOpenFile(String[] exts,
                                              String existingFileName,
                                              boolean onlyDirectories) {
    return promptToOpenFile(GuiPackage.getInstance().getMainFrame(), exts,
                            existingFileName, onlyDirectories);
  }
   ...
   *         finished using it - null if no file was chosen
   */
  public static JFileChooser promptToOpenFile(Component parentComponent,
                                              String[] exts,
                                              String existingFileName,
                                              boolean onlyDirectories) {
    if (onlyDirectories) {
    ...
    setCurrentDirOnJFC(existingFileName, lastJFCDirectory,
   ...
   */
  public static String getLastJFCDirectory() { return lastJFCDirectory; }

  /**
   *
   * @param lastJFCDirectory The last directory visited by the user while
   *     choosing Files
   */
