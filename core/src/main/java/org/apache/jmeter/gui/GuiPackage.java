...
public final class GuiPackage implements LocaleChangeListener, HistoryListener {
  ...
  private static final String SAVE_BEFORE_RUN_PROPERTY =
      "save_automatically_before_run"; // $NON-NLS-1$

  private static final boolean SAVE_BEFORE_RUN_PROPERTY_DEFAULT_VALUE = true;

  private static final Preferences PREFS =
      Preferences.userNodeForPackage(GuiPackage.class);
   ...
   * <p>This enables to associate {@link UnsharedComponent} UIs with their
   * {@link TestElement}.</p>
   */
  private final IdentityHashMap<TestElement, JMeterGUIComponent> nodesToGui =
      new IdentityHashMap<>();
   ...
   */
  private final Map<Class<?>, JMeterGUIComponent> testBeanGUIs =
      new HashMap<>();
  ...
  private final List<TestPlanListener> testPlanListeners =
   ...
   */
  private GuiPackage(JMeterTreeModel treeModel,
   ...
   * @return the GuiPackage instance (may be null, e.g in non-Gui mode)
   */
  public static void setInstance(GuiPackage instance) {
    guiPack = instance;
  }
   ...
   *            the model for JMeter's test tree
   */
  public static void initInstance(JMeterTreeListener listener,
   ...
   * @return the GUI component corresponding to the specified test element
   */
  public JMeterGUIComponent getGui(TestElement node, Class<?> guiClass,
                                   Class<?> testClass) {
    try {
  ...
  public JMeterGUIComponent getCurrentGui() {
    try {
      ...
      if (comp == null) {
        log.debug("No Component found for {}.",
                  treeListener.getCurrentNode().getName());
        return null;
      }
      comp.clearGui();
  ...
  public TestElement createTestElement(String objClass) {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(null, msg, "Missing jar? See log file.",
                                    JOptionPane.ERROR_MESSAGE);
      throw new RuntimeException(e.toString(), e); // Probably a missing jar
    } catch (ReflectiveOperationException e) {
      log.error("Problem retrieving gui for {}", objClass, e);
      throw new RuntimeException(e.toString(),
                                 e); // Programming error: bail out.
    }
  }
   ...
   */
  private JMeterGUIComponent getGuiFromCache(Class<?> guiClass,
                                             Class<?> testClass)
    ...
    if (guiClass == TestBeanGUI.class) {
      ...
      if (comp == null) {
        comp =
            (JMeterGUIComponent)guiClass.getDeclaredConstructor().newInstance();
   ...
   * Various {@link org.apache.jmeter.gui.action.Command} actions set this
   * property when components are modified/created/saved.
   *
   * @param dirty
   ...
   */
  public JMeterTreeModel getTreeModel() { return treeModel; }
  ...
  public ValueReplacer getReplacer() {
    return new ValueReplacer(
        (TestPlan)((JMeterTreeNode)getTreeModel().getTestPlan().getArray()[0])
            .getTestElement());
  }
   ...
   */
  public void setMainFrame(MainFrame newMainFrame) { mainFrame = newMainFrame; }
   ...
   */
  public JMeterTreeListener getTreeListener() { return treeListener; }
   ...
   */
  public void setMainToolbar(JToolBar newToolbar) { toolbar = newToolbar; }
   ...
   */
  public JToolBar getMainToolbar() { return toolbar; }
  ...
  public void displayPopUp(Component invoker, MouseEvent e, JPopupMenu popup) {
    if (popup != null) {
      log.debug("Showing pop up for {} at x,y = {},{}", invoker, e.getX(),
                e.getY());
   ...
   * <p>When Look and Feel (or zoom scaling) changes, only visible components
   * are updated. The hidden ones are updated as they are shown.</p>
   */
  ...
  public String getTestPlanFile() { return testPlanFile; }
   ...
   */
  public void register(Stoppable stoppable) { stoppables.add(stoppable); }
   ...
   */
  public LoggerPanel getLoggerPanel() { return loggerPanel; }

  /**
   * Navigate back through undo history
   */
  public void undo() { undoHistory.undo(); }

  /**
   * Navigate forward through undo history
   */
  public void redo() { undoHistory.redo(); }

  /**
   * @return true if history contains redo item
   */
  public boolean canRedo() { return undoHistory.canRedo(); }

  /**
   * @return true if history contains undo item
   */
  public boolean canUndo() { return undoHistory.canUndo(); }

  /**
   * Compute checksum of TestElement to detect changes
   * the method calculates properties checksum to detect testelement
   * modifications
   * TODO would be better to override hashCode for TestElement, but I decided
   * not to touch it
   *
      ...
      if (obj instanceof TestElementProperty) {
        ...
        if (stringValue != null) {
          ...
          if (log.isDebugEnabled()) {
            log.debug(
                "obj.getStringValue() returned null for test element:{} at property:{}",
  ...
  public void notifyChangeInHistory(UndoHistory history) {
    ((JMeterToolBar)toolbar)
        .updateUndoRedoIcons(history.canUndo(), history.canRedo());
  }
  ...
  public TreeNodeNamingPolicy getNamingPolicy() {
    if (namingPolicy == null) {
      final String namingPolicyImplementation = JMeterUtils.getPropDefault(
          "naming_policy.impl", //$NON-NLS-1$
      ...
      try {
        ...
        this.namingPolicy =
            (TreeNodeNamingPolicy)implementationClass.getDeclaredConstructor()
                .newInstance();

      } catch (Exception ex) {
        log.error(
            "Failed to create configured naming policy:{}, will use default one",
            namingPolicyImplementation, ex);
   ...
   * @return {@link GuiLogEventBus}
   */
  public GuiLogEventBus getLogEventBus() { return logEventBus; }

  /**
   * Begin a group of actions modeled as 1 undo
   */
  public void beginUndoTransaction() { undoHistory.beginUndoTransaction(); }

  /**
   * End a group of actions modeled as 1 undo
   */
  public void endUndoTransaction() { undoHistory.endUndoTransaction(); }
  ...
  public void setSaveBeforeRunByPreference(boolean saveBeforeRun) {
    PREFS.put(SBR_PREFS_KEY,
              Boolean.toString(saveBeforeRun)); // $NON-NLS-1$ // $NON-NLS-2$
  }
