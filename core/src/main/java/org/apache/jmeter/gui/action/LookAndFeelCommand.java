...
public class LookAndFeelCommand extends AbstractAction {
  ...
  public static class MenuItem {
    ...
    private static MenuItem of(String title, String lafClass) {
      return new MenuItem(title, ActionNames.LAF_PREFIX + lafClass, lafClass,
                          null);
    }
    ...
    private static MenuItem ofDarklafTheme(Theme theme) {
      return new MenuItem("Darklaf - " + theme.getName(),
                          ...
                          JMeterMenuBar.DARKLAF_LAF_CLASS, theme);
    }
  }
  ...
  static {
    if (System.getProperty("darklaf.decorations") == null) {
      ...
      // darklaf.allowNativeCode=true is required for darklaf.decorations=true
      // to work.
  ...
  public static String getJMeterLaf() {
    ...
    laf =
        JMeterUtils.getPropDefault(JMETER_LAF, JMeterMenuBar.DARCULA_LAF_CLASS);
    if (laf != null) {
      return checkLafName(laf);
    }
  ...
  public static boolean isDarklafTheme() {
    return "Darklaf".equalsIgnoreCase(
        UIManager.getLookAndFeel().getID()); // $NON-NLS-1$
  }
  ...
  public void doAction(ActionEvent ev) {
    try {
      ...
      int chosenOption = JOptionPane.showConfirmDialog(
          ...
          JMeterUtils.getResString("laf_quit_after_change"), // $NON-NLS-1$
          JMeterUtils.getResString("exit"),                  // $NON-NLS-1$
          JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
      if (chosenOption == JOptionPane.YES_OPTION) {
        ActionRouter.getInstance().doActionNow(
            new ActionEvent(ev.getSource(), ev.getID(), ActionNames.RESTART));
      }
