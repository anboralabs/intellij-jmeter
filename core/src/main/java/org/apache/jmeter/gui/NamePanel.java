...
public class NamePanel extends JPanel implements JMeterGUIComponent {
  ...
  private void init() { // WARNING: called from ctor so must not be overridden
    ...
    JLabel nameLabel =
        new JLabel(JMeterUtils.getResString("name")); // $NON-NLS-1$
  ...
  public String getName() {
    /*
     * Null check is needed because some implementations of JPanel end up
     * calling getName(). This is a bug, which has been reported as:
     ...
     */
    if (nameField != null) { // NOSONAR suppress warning as the null check is
                             // needed as per above
  ...
  public TestElement createTestElement() {
    ...
    org.apache.jmeter.testelement.WorkBench wb =
        new org.apache.jmeter.testelement.WorkBench();
  ...
  public void modifyTestElement(TestElement wb) {
    wb.setName(getName());
    wb.setProperty(
        new StringProperty(TestElement.GUI_CLASS, this.getClass().getName()));
    @SuppressWarnings("deprecation")
