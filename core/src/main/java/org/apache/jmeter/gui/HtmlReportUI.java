...
public class HtmlReportUI implements ActionListener {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(HtmlReportUI.class);
  ...
  private static final String GENERATE_REPORT_LABEL =
      JMeterUtils.getResString("generate_report_ui.html_report_request");
  private static final String GENERATING_REPORT_LABEL =
      JMeterUtils.getResString("generate_report_ui.html_report_processing");
  ...
  private final ImageIcon runningIcon = JMeterUtils.getImage(
      IMAGES_PREFIX + iconSize + "/task-recurring.png"); // $NON-NLS-1$
  private final ImageIcon inErrorIcon = JMeterUtils.getImage(
      IMAGES_PREFIX + iconSize + "/dialog-error-5.png"); // $NON-NLS-1$
  private final ImageIcon completedIcon = JMeterUtils.getImage(
      IMAGES_PREFIX + iconSize + "/task-complete.png"); // $NON-NLS-1$

  static { commands.add(ActionNames.HTML_REPORT); }

  public HtmlReportUI() { super(); }
  ...
  public void setupInputDialog(JFrame parent) {
    messageDialog = new EscapeDialog(
        parent, JMeterUtils.getResString("html_report"), false);
    setupContentPane();
  }
  ...
  private JPanel setupFileChooserPanel() {
    ...
    fileChooserPanel.add(
        new JLabel(JMeterUtils.getResString("generate_report_ui.csv_file")));
    ...
    this.userPropertiesFileButton =
        new JButton(JMeterUtils.getResString(BROWSE));
  ...
  private class ReportGenerationWorker
    ...
    protected List<String> doInBackground() throws Exception {
      HtmlReportGenerator htmlReportAction =
          new HtmlReportGenerator(csvFilePathTextField.getText(),
                                  userPropertiesFilePathTextField.getText(),
    ...
    protected void done() {
      try {
        ...
        if (LOGGER.isErrorEnabled()) {
          LOGGER.error("Error during html report generation: {}",
    ...
    case CREATE_REQUEST:
      ...
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug(
            ...
            csvFilePathTextField.getText(),
            userPropertiesFilePathTextField.getText(),
            outputDirectoryPathTextField.getText());
      }
      break;
    case BROWSE_USER_PROPERTIES:
      userPropertiesFilePathTextField.setText(showFileChooser(
          userPropertiesFileButton.getParent(), userPropertiesFilePathTextField,
          false, new String[] {".properties"}));
      break;
    case BROWSE_CSV:
      csvFilePathTextField.setText(
          showFileChooser(csvFileButton.getParent(), csvFilePathTextField,
                          false, new String[] {".jtl", ".csv"}));
      break;
    case BROWSE_OUTPUT:
      outputDirectoryPathTextField.setText(
          showFileChooser(outputDirectoryButton.getParent(),
                          outputDirectoryPathTextField, true, null));
      break;
    default:
      break;
    }
  ...
  void reportToUser(List<String> runErrors) {
    if (runErrors.isEmpty()) {
      addTextToReport(
          JMeterUtils.getResString(HtmlReportGenerator.HTML_REPORT_SUCCESS));
   ...
   */
  private String showFileChooser(Component component,
                                 JTextField locationTextField,
                                 boolean onlyDirectory, String[] extensions) {
