...
public class TextBoxDialoger implements ActionListener {
  ...
  private static final String SAVE_CLOSE_COMMAND =
      "save_close_dialog"; // $NON-NLS-1$
   ...
   */
  public TextBoxDialoger(String text) { init(text); }
  ...
  private void createDialogBox() {
    ...
    String title = editable ? JMeterUtils.getResString("textbox_title_edit")
    //$NON-NLS-1$ : JMeterUtils.getResString("textbox_title_view");
    //$NON-NLS-1$ dialog = new JDialog(mainFrame, title, true);  // modal dialog
    box
    ...
    d2.height) / 2); dialog.pack();*/
  }

  private void closeDialog() { dialog.setVisible(false); }
  ...
  public String getTextBox() { return textBox.getText(); }
  ...
  public static class TextBoxDoubleClick extends MouseAdapter {
    ...
    public void mouseClicked(MouseEvent e) {
      if (e.getClickCount() == 2) { // double click
        ...
        Object value =
            tm.getValueAt(table.getSelectedRow(), table.getSelectedColumn());
        new TextBoxDialoger(
            value.toString(),
            false); // view only NOSONAR this instantiation opens a popup
      }
    }
  }
      ...
      if (e.getClickCount() == 2) { // double (pressed) click
        ...
        if (value instanceof String) {
          if (table.getCellEditor() != null) {
            table.getCellEditor()
                .cancelCellEditing(); // in main table (evt mousePressed because
                                      // cell is editable)
          }
