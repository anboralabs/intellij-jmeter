...
public interface JMeterGUIComponent extends ClearGui {
   ...
   * getStaticLabel(). However where the resource name is not available or
   * required, getStaticLabel() may be overridden instead.</p>
   *
   ...
   *
   * <p>Since 5.6.3, the default implementation is as follows, and subclasses
   * should override
   ...
   *
   * <p>
   * Before 5.6.3 the canonical implementation was as follows, however, it is
   * recommended to avoid overriding {@link #createTestElement()} and override
   * {@link #makeTestElement()} instead. <pre> public TestElement
   * createTestElement() { TestElementXYZ el = new TestElementXYZ();
  ...
  default TestElement makeTestElement() {
    throw new UnsupportedOperationException(
        "Please override makeTestElement with creating the element you need: return new ....");
  }

  /**
   * Configures default values for element after its creation.
   * Plugin authors should call this once in their {@link #createTestElement()}
   * implementation.
   * @since 5.6.3
   ...
   *
   * <p>
   * If you override {@link AbstractJMeterGuiComponent}, you might want using
   * {@link AbstractJMeterGuiComponent#bindingGroup} instead of overriding
   * {@code modifyTestElement}.
   *
   * <p>
   ...
   *     super.modifyTestElement(element); // clear the element and assign basic
   * fields like name, gui class, test class
   *     // Using the element setters (preferred):
   *     // If the field is empty, you probably want to remove the property
   * instead of storing an empty string
   *     // See <a href="https://github.com/apache/jmeter/pull/6199">Streamline
   * binding of UI elements to TestElement properties</a>
   *     // for more details
   ...
   *     element.setProperty(TestElementXYZ.STATE,
   * StringUtils.defaultIfEmpty(guiState.getText(), null))
   *     element.setProperty(TestElementXYZ.CODE,
   * StringUtils.defaultIfEmpty(guiCode.getText(), null))
   *     ... other GUI fields ...
   * }
   * </pre>
   *
  ...
  default void modifyTestElement(TestElement element) {
    // TODO: should we keep .clear() here? It probably makes it easier to remove
    // all properties before populating
   ...
   *     ... other GUI fields ...
   *     // or using the element property names directly (do not use unless
   * there is no getter for the field)
   ...
   */
  default boolean canBeAdded() { return true; }
}
