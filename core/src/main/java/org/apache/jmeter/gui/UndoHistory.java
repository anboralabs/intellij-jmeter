...
public class UndoHistory implements TreeModelListener, Serializable {
  ...
  public void clear() {
    ...
    if (isTransaction()) {
      if (log.isWarnEnabled()) {
        log.warn("Clearing undo history with {} unfinished transactions",
                 transactions.size());
      }
      transactions.clear();
    }
  ...
  public void add(JMeterTreeModel treeModel, String comment) {
    ...
    // get test plan tree
    HashTree tree =
        treeModel.getCurrentSubTree((JMeterTreeNode)treeModel.getRoot());
    ...
    addEdit(
        new UndoHistoryItem(copy, comment, TreeState.from(guiPackage), dirty));
   ...
   */
  private void loadHistoricalTree(JMeterTreeModel acceptorModel,
                                  GuiPackage guiInstance, HashTree newModel) {
   ...
   */
  public boolean canRedo() { return manager.canRedo(); }

  /**
   * @return true if not at first element
   */
  public boolean canUndo() { return manager.canUndo(); }
  ...
  public void treeNodesChanged(TreeModelEvent tme) {
    String name =
        ((JMeterTreeNode)tme.getTreePath().getLastPathComponent()).getName();
   ...
   */
  public static boolean isEnabled() { return HISTORY_SIZE > 0; }
  ...
  private void addEdit(UndoHistoryItem item) {
    if (lastKnownState != null) {
      GlobalUndoableEdit edit =
          new GlobalUndoableEdit(item, lastKnownState, this::reload);
      addEdit(edit);
    } else {
  ...
  boolean isTransaction() { return !transactions.isEmpty(); }
