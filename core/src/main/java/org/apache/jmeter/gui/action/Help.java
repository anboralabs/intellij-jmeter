...
public class Help extends AbstractAction {
  ...
  private static final String HELP_COMPONENTS =
      HELP_DOCS + "component_reference.html"; // $NON-NLS-1$

  public static final String HELP_FUNCTIONS =
      HELP_DOCS + "functions.html"; // $NON-NLS-1$
  ...
  public void doAction(ActionEvent e) {
    if (USE_LOCAL_HELP) {
      ...
      if (e.getSource() instanceof String[]) {
        ...
        url.append(HELP_COMPONENTS)
            .append('#')
            .append(GuiPackage.getInstance()
                        .getTreeListener()
                        .getCurrentNode()
                        .getDocAnchor());
      }
      try {
        helpDoc.setPage(url.toString()); // N.B. this only reloads if necessary
                                         // (ignores the reference)
      } catch (IOException ioe) {
        log.error("Error setting page for url, {}", url, ioe);
        helpDoc.setText(
            "<html><head><title>Problem loading help page</title>"
            + "<style><!--"
            ...
            + "<div>Can't load url: &quot;<em>" + url.toString() +
            "</em>&quot;</div>"
      ...
      if (e.getSource() instanceof String[]) {
        ActionRouter.getInstance().doActionNow(new ActionEvent(
            e.getSource(), e.getID(), ActionNames.LINK_FUNC_REF));
      } else {
        String[] source =
            new String[] {ActionNames.LINK_COMP_REF, GuiPackage.getInstance()
                                                         .getTreeListener()
                                                         .getCurrentNode()
                                                         .getDocAnchor()};
        ActionRouter.getInstance().doActionNow(
            new ActionEvent(source, e.getID(), ActionNames.LINK_COMP_REF));
      }
    }
  }
  ...
  private static JDialog initHelpWindow() {
    if (helpWindow == null) {
      helpWindow =
          new EscapeDialog(new Frame(), // independent frame to
