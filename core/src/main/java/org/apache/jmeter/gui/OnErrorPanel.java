...
public class OnErrorPanel extends JPanel {
  ...
  private JPanel createOnErrorPanel() {
    ...
    continueBox =
        addRadioButton("sampler_on_error_continue", group, panel); //$NON-NLS-1$
    breakLoopBox = addRadioButton("sampler_on_error_break_loop", group,
                                  panel); //$NON-NLS-1$
    startNextThreadLoopBox = addRadioButton("sampler_on_error_start_next_loop",
                                            group, panel); //$NON-NLS-1$
    startNextIterationOfCurrentLoopBox =
        addRadioButton("sampler_on_error_start_next_iteration_current_loop",
                       group, panel); //$NON-NLS-1$
    stopTestBox = addRadioButton("sampler_on_error_stop_test", group,
                                 panel); //$NON-NLS-1$
    stopTestNowBox = addRadioButton("sampler_on_error_stop_test_now", group,
                                    panel); //$NON-NLS-1$
    stopThrdBox = addRadioButton("sampler_on_error_stop_thread", group,
                                 panel); //$NON-NLS-1$
  ...
  private static JRadioButton addRadioButton(String labelKey, ButtonGroup group,
                                             JPanel panel) {
    JRadioButton radioButton =
        new JRadioButton(JMeterUtils.getResString(labelKey));
  ...
  public void configure(int errorAction) {
    ...
    startNextThreadLoopBox.setSelected(
        errorAction == OnErrorTestElement.ON_ERROR_START_NEXT_THREAD_LOOP);
    startNextIterationOfCurrentLoopBox.setSelected(
        errorAction ==
        OnErrorTestElement.ON_ERROR_START_NEXT_ITERATION_OF_CURRENT_LOOP);
    ...
    breakLoopBox.setSelected(errorAction ==
                             OnErrorTestElement.ON_ERROR_BREAK_CURRENT_LOOP);
  }
