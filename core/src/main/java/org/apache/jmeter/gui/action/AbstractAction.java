...
public abstract class AbstractAction implements Command {
  ...
  private enum ActionOnFile { APPEND, DELETE, ASK }

  private static final ActionOnFile actionOnFile =
      ActionOnFile.valueOf(JMeterUtils.getPropDefault(
          "resultcollector.action_if_file_exists", ActionOnFile.ASK.name()));
  ...
  protected boolean popupCheckExistingFileListener(HashTree tree) {
    ...
    for (ResultCollector rc : resultListeners.getSearchResults()) {
      ...
      if (f.exists()) {
        ...
        case ASK:
          String[] option =
              new String[] {JMeterUtils.getResString("concat_result"),
          ...
          int response = JOptionPane.showOptionDialog(
              GuiPackage.getInstance().getMainFrame(), question,
          ...
          case JOptionPane.CANCEL_OPTION:
            ...
            if (f.delete()) {
              ...
              log.error("Could not delete existing file {}",
                        f.getAbsolutePath());
              return false;
            }
  ...
  comp); if (window instanceof JFrame) { parent = (JFrame) window;
              }
          }
      } else {
