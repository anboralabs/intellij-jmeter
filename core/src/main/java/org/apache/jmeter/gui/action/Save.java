...
public class Save extends AbstractAction {
  ...
  private static final String JMX_BACKUP_ON_SAVE =
      "jmeter.gui.action.save.backup_on_save"; // $NON-NLS-1$

  private static final String JMX_BACKUP_DIRECTORY =
      "jmeter.gui.action.save.backup_directory"; // $NON-NLS-1$

  private static final String JMX_BACKUP_MAX_HOURS =
      "jmeter.gui.action.save.keep_backup_max_hours"; // $NON-NLS-1$

  private static final String JMX_BACKUP_MAX_COUNT =
      "jmeter.gui.action.save.keep_backup_max_count"; // $NON-NLS-1$

  public static final String JMX_FILE_EXTENSION = ".jmx"; // $NON-NLS-1$

  private static final String DEFAULT_BACKUP_DIRECTORY =
      JMeterUtils.getJMeterHome() + "/backups"; //$NON-NLS-1$

  // Whether we should keep backups for save JMX files. Default is to enable
  // backup
  private static final boolean BACKUP_ENABLED =
      JMeterUtils.getPropDefault(JMX_BACKUP_ON_SAVE, true);

  // Path to the backup directory
  private static final String BACKUP_DIRECTORY = JMeterUtils.getPropDefault(
      JMX_BACKUP_DIRECTORY, DEFAULT_BACKUP_DIRECTORY);

  // Backup files expiration in hours. Default is to never expire (zero value).
  private static final int BACKUP_MAX_HOURS =
      JMeterUtils.getPropDefault(JMX_BACKUP_MAX_HOURS, 0);
  ...
  private static final DecimalFormat BACKUP_VERSION_FORMATER =
      new DecimalFormat("000000"); //$NON-NLS-1$
      ...
      if (checkAcceptableForTestFragment(nodes)) {
        ...
        JMeterUtils.reportErrorToUser(
            JMeterUtils.getResString(
                "save_as_test_fragment_error"),                 // $NON-NLS-1$
  ...
  private static HashTree createTestFragmentNode(JMeterTreeNode[] nodes) {
    TestElement element = GuiPackage.getInstance().createTestElement(
        TestFragmentControllerGui.class.getName());
    HashTree hashTree = new ListedHashTree();
    HashTree tfTree = hashTree.add(new JMeterTreeNode(element, null));
    for (JMeterTreeNode node : nodes) {
      // Clone deeply current node
      TreeCloner cloner = new TreeCloner(false);
      GuiPackage.getInstance().getTreeModel().getCurrentSubTree(node).traverse(
          cloner);
      // Add clone to tfTree
  ...
  private static String computeFileName() {
    ...
    if (f.exists()) {
      int response = JOptionPane.showConfirmDialog(
          ...
          JMeterUtils.getResString(
              "save_overwrite_existing_file"), // $NON-NLS-1$
   ...
   */
  void backupAndSave(ActionEvent e, HashTree subTree, boolean fullSave,
                     String newFile) throws IllegalUserActionException {
    //
    ...
    try (FileOutputStream ostream = new FileOutputStream(newFile)) {
      ...
      if (fullSave) { // Only update the stored copy of the tree for a full save
        ...
        subTree =
            GuiPackage.getInstance()
                .getTreeModel()
                .getTestPlan(); // refetch, because convertSubTree affects it
        ActionRouter.getInstance().doActionNow(
            new ActionEvent(subTree, e.getID(), ActionNames.SUB_TREE_SAVED));
      }
      ...
      throw new IllegalUserActionException(
          "Couldn't save test plan to file: " + newFile, ex);
    }
  }
  ...
  private static List<File> createBackupFile(File fileToBackup) {
    ...
    // {baseName}{versionSeparator}{version}{jmxExtension}  // NOSONAR
    String backupName = baseName + versionSeparator +
    ...
    try {
      ...
      log.error("Failed to backup file: {}", fileToBackup.getAbsolutePath(),
                e); //$NON-NLS-1$
      return EMPTY_FILE_LIST;
    }
  ...
  backupFilesToDelete(List<? extends File> backupFiles) {
    ...
    if (BACKUP_MAX_COUNT > 0 && backupFiles.size() > BACKUP_MAX_COUNT) {
      ...
      filesToDelete.addAll(
          backupFiles.subList(0, backupFiles.size() - BACKUP_MAX_COUNT));
    }
   ...
   */
  private static List<File>
  expiredBackupFiles(List<? extends File> backupFiles) {
    if (BACKUP_MAX_HOURS > 0) {
      final long expiryMillis =
          System.currentTimeMillis() - (1L * BACKUP_MAX_HOURS * MS_PER_HOUR);
      return backupFiles.stream()
  ...
  checkAcceptableForTestFragment(JMeterTreeNode[] nodes) {
    return Arrays.stream(nodes)
        ...
        .noneMatch(
            o -> o instanceof AbstractThreadGroup || o instanceof TestPlan);
  }
  ...
  void convertSubTree(HashTree tree) {
    for (Object o : new ArrayList<>(tree.list())) {
      ...
      TestElement testElement =
          item.getTestElement(); // requires JMeterTreeNode
    ...
    public PrivatePatternFileFilter(Pattern pattern) {
      if (pattern == null) {
        throw new IllegalArgumentException(
            "pattern cannot be null !"); //$NON-NLS-1$
      }
