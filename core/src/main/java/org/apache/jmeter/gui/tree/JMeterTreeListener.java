...
public class JMeterTreeListener
  ...
  private static final Logger log =
      LoggerFactory.getLogger(JMeterTreeListener.class);
   ...
   */
  public JMeterTreeListener(JMeterTreeModel model) { this.model = model; }
   ...
   */
  public void setModel(JMeterTreeModel m) { model = m; }
   ...
   */
  public void setActionHandler(ActionListener ah) { actionHandler = ah; }
   ...
   *            the new JTree value
   */
  public void setJTree(JTree tree) { this.tree = tree; }
  ...
  public void valueChanged(TreeSelectionEvent e) {
    ...
    actionHandler.actionPerformed(
        new ActionEvent(this, 3333, ActionNames.EDIT)); // $NON-NLS-1$
  }
  ...
  public void mousePressed(MouseEvent e) {
    ...
    if (closestPath == null) {
      log.debug("ClosestPathForLocation is not found for x={}, y={}", e.getX(),
                e.getY());
      return;
    }
    Rectangle bounds = tree.getPathBounds(closestPath);
    if (bounds == null || bounds.y > e.getY() ||
        e.getY() > bounds.y + bounds.height) {
      log.debug(
          "Mouse click was outside of node {}. bounds={}, event.x={}, event.y={}",
  ...
  private void displayPopUp(MouseEvent e) {
    ...
    // inactive node In other words: right-click different nodes and verify if
    // menu is shown every time. invokeLater seems to be required as long as
    // tree.requestFocusInWindow(); is used in valueChanged
