...
public class Top5ErrorsBySamplerConsumer
  ...
  public Top5ErrorsBySamplerConsumer() { super(false); }
   ...
   *     results
   *                  in which case we ignore Transaction Controller's
   * SampleResult
   */
  private void aggregateSample(Sample sample, Top5ErrorsSummaryData data,
                               boolean isOverall) {
  ...
  protected ListResultData createDataResult(String key,
    ...
    if (errors > 0 || key == null) {
      result.addResult(new ValueResultData(
          key != null
              ? key
              : JMeterUtils.getResString("reportgenerator_top5_total")));
  ...
  protected ListResultData createResultTitles() {
    ...
    titles.addResult(new ValueResultData(
        JMeterUtils.getResString("reportgenerator_top5_label")));
    titles.addResult(new ValueResultData(
        JMeterUtils.getResString("reportgenerator_top5_sample_count")));
    titles.addResult(new ValueResultData(
  ...
  /**
   * @param ignoreTCFromTop5ErrorsBySampler ignore transaction controller
   *     sampler results when computing top 5
   */
  public void
  setIgnoreTransactionController(boolean ignoreTCFromTop5ErrorsBySampler) {
