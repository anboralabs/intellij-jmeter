...
public class ReportGeneratorConfiguration {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(ReportGeneratorConfiguration.class);

  private static final String RANGE_DATE_FORMAT_DEFAULT =
  ...
  public static final String REPORT_GENERATOR_KEY_RANGE_DATE_FORMAT =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "date_format";

  public static final String REPORT_GENERATOR_GRAPH_KEY_PREFIX =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "graph";
  public static final String REPORT_GENERATOR_EXPORTER_KEY_PREFIX =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "exporter";

  // Temporary directory
  public static final String REPORT_GENERATOR_KEY_TEMP_DIR =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "temp_dir";
  private static final File REPORT_GENERATOR_KEY_TEMP_DIR_DEFAULT =
      new File("temp");

  // Apdex Satisfied Threshold
  private static final String REPORT_GENERATOR_KEY_APDEX_SATISFIED_THRESHOLD =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "apdex_satisfied_threshold";
  private static final Long
      REPORT_GENERATOR_KEY_APDEX_SATISFIED_THRESHOLD_DEFAULT = 500L;

  // Apdex Tolerated Threshold
  private static final String REPORT_GENERATOR_KEY_APDEX_TOLERATED_THRESHOLD =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "apdex_tolerated_threshold";
  private static final Long
      REPORT_GENERATOR_KEY_APDEX_TOLERATED_THRESHOLD_DEFAULT = 1500L;

  // Apdex per transaction Thresholds
  private static final String REPORT_GENERATOR_KEY_APDEX_PER_TRANSACTION =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "apdex_per_transaction";
  ...
  private static final String
      REPORT_GENERATOR_KEY_EXCLUDE_TC_FROM_TOP5_ERRORS_BY_SAMPLER =
          REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER +
          "exclude_tc_from_top5_errors_by_sampler";

  // Sample Filter
  private static final String REPORT_GENERATOR_KEY_SAMPLE_FILTER =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "sample_filter";

  // report title
  private static final String REPORT_GENERATOR_KEY_REPORT_TITLE =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "report_title";

  // start date for which report must be generated
  private static final String REPORT_GENERATOR_KEY_START_DATE =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "start_date";

  // end date for which report must be generated
  private static final String REPORT_GENERATOR_KEY_END_DATE =
      REPORT_GENERATOR_KEY_PREFIX + KEY_DELIMITER + "end_date";
  ...
  private static final class ExporterConfigurationFactory
    ...
    private ExporterConfigurationFactory(Props props) { this.props = props; }
    ...
    public void initialize(String exportId,
      ...
      String className = getRequiredProperty(
          props, getExporterPropertyKey(exportId, SUBCONF_KEY_CLASSNAME), "",
          String.class);
      ...
      boolean filtersOnlySampleSeries = getRequiredProperty(
          ...
          EXPORTER_KEY_FILTERS_ONLY_SAMPLE_SERIES_DEFAULT, Boolean.class);
      ...
      String seriesFilter = getRequiredProperty(
          props, getExporterPropertyKey(exportId, EXPORTER_KEY_SERIES_FILTER),
          EXPORTER_KEY_SERIES_FILTER_DEFAULT, String.class);
      ...
      boolean showControllerSeriesOnly = getRequiredProperty(
          props,
          getExporterPropertyKey(exportId, EXPORTER_KEY_SHOW_CONTROLLERS_ONLY),
          EXPORTER_KEY_SHOW_CONTROLLERS_ONLY_DEFAULT, Boolean.class);
      exportConfiguration.showControllerSeriesOnly(showControllerSeriesOnly);

      // Load graph extra properties
      Map<String, SubConfiguration> graphExtraConfigurations =
          exportConfiguration.getGraphExtraConfigurations();
      loadSubConfiguration(
          graphExtraConfigurations, props,
          getSubConfigurationPropertyKey(REPORT_GENERATOR_EXPORTER_KEY_PREFIX,
                                         exportId,
                                         EXPORTER_KEY_GRAPH_EXTRA_OPTIONS),
          true, new SubConfigurationFactory<SubConfiguration>() {
            @Override
  ...
  private static final class GraphConfigurationFactory
    ...
    private GraphConfigurationFactory(Props props) { this.props = props; }
    ...
    public void initialize(String graphId,
      ...
      boolean excludeControllers = getRequiredProperty(
          props, getGraphPropertyKey(graphId, GRAPH_KEY_EXCLUDE_CONTROLLERS),
      ...
      String title = getRequiredProperty(
          props, getGraphPropertyKey(graphId, GRAPH_KEY_TITLE),
          GRAPH_KEY_TITLE_DEFAULT, String.class);
      graphConfiguration.setTitle(title);

      // Get the property defining the class name
      String className = getRequiredProperty(
          props, getGraphPropertyKey(graphId, SUBCONF_KEY_CLASSNAME), "",
          String.class);
      log.debug("Using class:'{}' for graph:'{}' with id:'{}'", className,
                title, graphId);
  ...
  private final Map<String, ExporterConfiguration> exportConfigurations =
      new HashMap<>();
  private final Map<String, GraphConfiguration> graphConfigurations =
      new HashMap<>();
   ...
   */
  public final String getSampleFilter() { return sampleFilter; }
   ...
   */
  public final File getTempDirectory() { return tempDirectory; }
   ...
   */
  public static String getGraphPropertyKey(String graphId,
                                           String propertyName) {
    return getSubConfigurationPropertyPrefix(REPORT_GENERATOR_GRAPH_KEY_PREFIX,
                                             graphId) +
        KEY_DELIMITER + propertyName;
  }
   ...
   */
  private static <TProperty> TProperty
  getProperty(Props props, String key, TProperty defaultValue,
              @SuppressWarnings("BoundedWildcard") Class<TProperty> clazz)
      throws ConfigurationException {
    String value = props.getValue(key);
    if (value == null) {
      log.info("Property '{}' not found, using default value '{}' instead.",
               key, defaultValue);
      return defaultValue;
    }
  ...
  private static <TProperty> TProperty
  getOptionalProperty(Props props, String key, Class<TProperty> clazz)
      throws ConfigurationException {
  ...
  private static <TProperty> TProperty getRequiredProperty(
      Props props, String key, TProperty defaultValue, Class<TProperty> clazz)
      throws ConfigurationException {
   ...
   */
  private static <TSubConf extends SubConfiguration> void
  loadSubConfiguration(Map<String, TSubConf> subConfigurations, Props props,
                       String propertyPrefix, boolean noPropertyKey,
                       SubConfigurationFactory<TSubConf> factory)
      throws ConfigurationException {

    for (Map.Entry<String, Object> entry :
         props.innerMap(propertyPrefix).entrySet()) {
    ...
    for (Map.Entry<String, TSubConf> entry : subConfigurations.entrySet()) {
      ...
      Map<String, Object> extraKeys = props.innerMap(
          noPropertyKey
              ? getSubConfigurationPropertyPrefix(propertyPrefix, subConfId)
              : getSubConfigurationPropertyKey(propertyPrefix, subConfId,
                                               SUBCONF_KEY_PROPERTY));
   ...
   */
  public static ReportGeneratorConfiguration
  loadFromProperties(Properties properties) throws ConfigurationException {
    ...
    final File tempDirectory =
        getRequiredProperty(props, REPORT_GENERATOR_KEY_TEMP_DIR,
    ...
    final long apdexSatisfiedThreshold = getRequiredProperty(
        props, REPORT_GENERATOR_KEY_APDEX_SATISFIED_THRESHOLD,
        REPORT_GENERATOR_KEY_APDEX_SATISFIED_THRESHOLD_DEFAULT, long.class);
    configuration.setApdexSatisfiedThreshold(apdexSatisfiedThreshold);

    // Load apdex tolerated threshold
    final long apdexToleratedThreshold = getRequiredProperty(
        props, REPORT_GENERATOR_KEY_APDEX_TOLERATED_THRESHOLD,
        REPORT_GENERATOR_KEY_APDEX_TOLERATED_THRESHOLD_DEFAULT, long.class);
    configuration.setApdexToleratedThreshold(apdexToleratedThreshold);

    // Load apdex per transactions, overridden by user
    final String apdexPerTransaction = getOptionalProperty(
        props, REPORT_GENERATOR_KEY_APDEX_PER_TRANSACTION, String.class);
    configuration.setApdexPerTransaction(
        getApdexPerTransactionParts(apdexPerTransaction));

    final boolean ignoreTCFromTop5ErrorsBySampler = getRequiredProperty(
        props, REPORT_GENERATOR_KEY_EXCLUDE_TC_FROM_TOP5_ERRORS_BY_SAMPLER,
        Boolean.TRUE, Boolean.class);
    configuration.setIgnoreTCFromTop5ErrorsBySampler(
        ignoreTCFromTop5ErrorsBySampler);

    // Load sample filter
    final String sampleFilter = getOptionalProperty(
        props, REPORT_GENERATOR_KEY_SAMPLE_FILTER, String.class);
    configuration.setSampleFilter(sampleFilter);

    final String reportTitle = getOptionalProperty(
        props, REPORT_GENERATOR_KEY_REPORT_TITLE, String.class);
    configuration.setReportTitle(reportTitle);

    Date reportStartDate = null;
    Date reportEndDate = null;
    final String startDateValue = getOptionalProperty(
        props, REPORT_GENERATOR_KEY_START_DATE, String.class);
    final String endDateValue =
        getOptionalProperty(props, REPORT_GENERATOR_KEY_END_DATE, String.class);

    String rangeDateFormat = getOptionalProperty(
        props, REPORT_GENERATOR_KEY_RANGE_DATE_FORMAT, String.class);
    if (StringUtils.isEmpty(rangeDateFormat)) {
      rangeDateFormat = RANGE_DATE_FORMAT_DEFAULT;
    }
    SimpleDateFormat dateFormat =
        new SimpleDateFormat(rangeDateFormat, Locale.ENGLISH);
    ...
    try {
      ...
      log.error("Error parsing property {} with value: {} using format: {}",
                REPORT_GENERATOR_KEY_START_DATE, startDateValue,
                rangeDateFormat, e);
    }
    try {
      ...
      log.error("Error parsing property {} with value: {} using format: {}",
                REPORT_GENERATOR_KEY_END_DATE, endDateValue, rangeDateFormat,
                e);
    }

    log.info("Will use date range start date: {}, end date: {}", startDateValue,
             endDateValue);

    // Find graph identifiers and load a configuration for each
    final Map<String, GraphConfiguration> graphConfigurations =
        configuration.getGraphConfigurations();
    ...
    final Map<String, ExporterConfiguration> exportConfigurations =
        configuration.getExportConfigurations();
   ...
   * sample names as keys and an array of 2 longs [satisfied, tolerated] as
   * values. The sample name can be a regex supplied by the user.
   * @param apdexPerTransaction the string coming from properties
   * @return {@link Map} containing for each sample name or sample name regex an
   *     array of Long corresponding to satisfied and tolerated apdex
   *     thresholds.
   */
        ...
        if (colonSeparator == -1 || pipeSeparator == -1 ||
          ...
          log.error(
              "error parsing property apdex_per_transaction around chunk {}. "
                  +
                  "Wrong format, should have been: 'sample:satisfiedMs|toleratedMS', ignoring",
              chunk);
          continue;
        }
        String key = chunk.substring(0, colonSeparator).trim();
        Long satisfied = Long.valueOf(
            chunk.substring(colonSeparator + 1, pipeSeparator).trim());
        Long tolerated =
            Long.valueOf(chunk.substring(pipeSeparator + 1).trim());
  ...
  /**
   * @param ignoreTCFromTop5ErrorsBySampler the ignoreTCFromTop5ErrorsBySampler
   *     to set
   */
  public void
