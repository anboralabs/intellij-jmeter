...
public class ResponseTimePercentilesOverTimeGraphConsumer
  ...
  private static GroupInfo createPercentileGroupInfo(String propKey,
                                                     String label) {
    String seriesName = formatPercentile(label);
    double defaultValue =
        new BigDecimal(label).setScale(2, RoundingMode.CEILING).doubleValue();
  ...
  createGroupInfo(AggregatorFactory aggregationFactory,
                  StaticSeriesSelector seriesSelector) {
    return new GroupInfo(aggregationFactory, seriesSelector,
                         new SuccessfulElapsedTimeValueSelector(), false,
                         false);
  }
}
