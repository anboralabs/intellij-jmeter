...
public class ApdexSummaryConsumer
  ...
  public ApdexSummaryConsumer() { super(true); }
  ...
  protected void updateData(SummaryInfo info, Sample sample) {
    ...
    if (data == null) {
      data =
          new ApdexSummaryData(getThresholdSelector().select(sample.getName()));
      info.setData(data);
    }
    ...
    if (sample.getSuccess()) {
      ...
      ApdexThresholdsInfo overallThresholdsInfo =
          overallData.getApdexThresholdInfo();
  ...
  protected ListResultData createResultTitles() {
    ...
    titles.addResult(new ValueResultData(
        JMeterUtils.getResString("reportgenerator_summary_apdex_apdex")));
    titles.addResult(new ValueResultData(
        JMeterUtils.getResString("reportgenerator_summary_apdex_satisfied")));
    titles.addResult(new ValueResultData(
        JMeterUtils.getResString("reportgenerator_summary_apdex_tolerated")));
    titles.addResult(new ValueResultData(
        JMeterUtils.getResString("reportgenerator_summary_apdex_samplers")));
    return titles;
  }
}
