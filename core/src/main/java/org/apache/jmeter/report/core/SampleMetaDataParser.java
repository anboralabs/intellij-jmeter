...
public class SampleMetaDataParser {
  ...
  public SampleMetaDataParser(char separator) { this.separator = separator; }
      ...
      if (matcher.matches()) {
        ...
        if (guessedSep.length() != 1) {
          throw new IllegalArgumentException(
              "We guessed a delimiter of '" + guessedSep +
              "', but we support only one-character-separators");
        }
        useSep = guessedSep.charAt(0);
        logger.warn(
            "Use guessed delimiter '{}' instead of configured '{}'. "
                +
                "Please configure the property 'jmeter.save.saveservice.default_delimiter={}'",
            useSep, separator, useSep);
      }
    }
