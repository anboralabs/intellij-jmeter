...
public class ReportGenerator {
  ...
  private static final boolean CSV_OUTPUT_FORMAT =
      "csv".equalsIgnoreCase(JMeterUtils.getPropDefault(
  ...
  private static final String INVALID_CLASS_FMT =
  ...
  public static final String DATE_RANGE_FILTER_CONSUMER_NAME =
      "dateRangeFilter";
  ...
  public static final String STATISTICS_SUMMARY_CONSUMER_NAME =
      "statisticsSummary";
  public static final String TOP5_ERRORS_BY_SAMPLER_CONSUMER_NAME =
      "top5ErrorsBySampler";
  public static final String START_INTERVAL_CONTROLLER_FILTER_CONSUMER_NAME =
      "startIntervalControlerFilter";

  private static final Pattern POTENTIAL_CAMEL_CASE_PATTERN =
      Pattern.compile("_(.)");
   ...
   * @param resultCollector Can be null, used if generation occurs at end of
   *     test
  ...
  public ReportGenerator(String resultsFile, ResultCollector resultCollector)
    ...
    log.info("ReportGenerator will use for Parsing the separator: '{}'",
    ...
    if (resultCollector == null) {
      if (!(file.isFile() && file.canRead())) {
        throw new IllegalArgumentException(
            String.format("Cannot read test results file : %s", file));
      }
      log.info("Will only generate report from results file: {}", resultsFile);
    } else {
      if (file.exists() && file.length() > 0) {
        throw new IllegalArgumentException("Results file:" + resultsFile +
                                           " is not empty");
      }
    ...
    File rgp =
        new File(JMeterUtils.getJMeterBinDir(), REPORTGENERATOR_PROPERTIES);
    if (log.isInfoEnabled()) {
      log.info("Reading report generator properties from: {}",
               rgp.getAbsolutePath());
    }
  ...
  private static String getSetterName(String propertyKey) {
    ...
    StringBuffer buffer = new StringBuffer(); // NOSONAR Unfortunately Matcher
                                              // does not support StringBuilder
    while (matcher.find()) {
  ...
  public void generate() throws GenerationException {
    ...
    SampleSource source =
        new CsvFileSampleSource(testFile, CSV_DEFAULT_SEPARATOR);
    ...
    Map<String, GraphConfiguration> graphConfigurations =
    ...
    try {
      ...
      throw new GenerationException(
          "Error while processing samples: " + ex.getMessage(), ex);
    }
    ...
    for (Map.Entry<String, ExporterConfiguration> entry :
         configuration.getExportConfigurations().entrySet()) {
      key = entry.getKey();
      value = entry.getValue();
      if (log.isInfoEnabled()) {
        log.info("Exporting data using exporter:'{}' of className:'{}'", key,
                 value.getClassName());
      }
  ...
  private static void removeTempDir(File tmpDir, boolean tmpDirCreated) {
    if (tmpDirCreated) {
      try {
        ...
        log.warn("Cannot delete created temporary directory, '{}'.", tmpDir,
                 ex);
      }
    }
  }
  ...
  private static boolean createTempDir(File tmpDir) throws GenerationException {
    ...
    if (!tmpDirCreated) {
      String message = String.format(
          "Cannot create temporary directory \"%s\", check property \"%s\"",
  ...
  private static void addGraphConsumer(
      FilterConsumer nameFilter, FilterConsumer excludeControllerFilter,
    ...
    try {
      ...
      for (Map.Entry<String, String> entryProperty :
           graphConfiguration.getProperties().entrySet()) {
        String propertyName = entryProperty.getKey();
        String propertyValue = entryProperty.getValue();
        String setterName = getSetterName(propertyName);

        setProperty(className, obj, methods, propertyName, propertyValue,
                    setterName);
      }
      graph.initialize();

      // Choose which entry point to use to plug the graph
      AbstractSampleConsumer entryPoint =
          graphConfiguration.excludesControllers() ? excludeControllerFilter
                                                   : nameFilter;
      entryPoint.addSampleConsumer(graph);
    } catch (ClassNotFoundException ex) {
      log.warn(
          "Unable to add class:{} as consumer for HTML report generation, "
              +
              "check class name or that the plugin that contains it is on classpath",
          className, ex);
  ...
  private void exportData(SampleContext sampleContext, String exporterName,
    ...
    try {
      ...
      String error = String.format(
          "Data exporter \"%s\" is unable to export data.", exporterName);
      throw new GenerationException(error, ex);
    }
  }
  ...
  private static FilterConsumer createExcludeControllerFilter() {
    ...
    excludeControllerFilter.setName(
  ...
  private static SampleConsumer
  createTop5ErrorsConsumer(ReportGeneratorConfiguration configuration) {
    Top5ErrorsBySamplerConsumer top5ErrorsBySamplerConsumer =
        new Top5ErrorsBySamplerConsumer();
  ...
  private static StatisticsSummaryConsumer createStatisticsSummaryConsumer() {
    StatisticsSummaryConsumer statisticsSummaryConsumer =
        new StatisticsSummaryConsumer();
  ...
  private static RequestsSummaryConsumer createRequestsSummaryConsumer() {
    RequestsSummaryConsumer requestsSummaryConsumer =
        new RequestsSummaryConsumer();
  ...
  private ApdexSummaryConsumer createApdexSummaryConsumer() {
    ...
    apdexSummaryConsumer.setThresholdSelector(sampleName -> {
      ...
      for (Map.Entry<String, Long[]> entry :
           configuration.getApdexPerTransaction().entrySet()) {
        if (isMatching(sampleName, entry.getKey())) {
          Long satisfied = entry.getValue()[0];
          Long tolerated = entry.getValue()[1];
          if (log.isDebugEnabled()) {
            log.debug(
                "Found match for sampleName:{}, Regex:{}, satisfied value:{}, tolerated value:{}",
  ...
  private static boolean isMatching(String sampleName, String keyName) {
    ...
    org.apache.oro.text.regex.Pattern regex =
        JMeterUtils.getPatternCache().getPattern(keyName);
  ...
  private FilterConsumer createNameFilter() {
    ...
    nameFilter.setSamplePredicate(sample -> {
      ...
      Pattern filteredSamplesPattern =
          configuration.getFilteredSamplesPattern();
   ...
   * @param methods       methods of the object to be searched for the property
   *     setter
   * @param propertyName  name of the property to be set
   * @param propertyValue value to be set
   * @param setterName    name of the property setter that should be used to set
   *     the
   *                      property
   * @throws IllegalAccessException if reflection throws an
   *     IllegalAccessException
   * @throws GenerationException    if conversion of the property value fails or
   *     reflection
   *                                throws an InvocationTargetException
   */
  private static void setProperty(String className, Object obj,
                                  Method[] methods, String propertyName,
                                  String propertyValue, String setterName)
    ...
    try {
        ...
        if (method.getName().equals(setterName)) {
          ...
          if (parameterTypes.length == 1) {
            ...
            if (parameterType.isAssignableFrom(String.class)) {
              ...
              if (converter == null) {
                throw new GenerationException(
                    String.format("Not supported conversion to \"%s\"",
      ...
      log.warn("'{}' is not a valid property for class '{}', skip it",
