...
public abstract class AbstractGraphConsumer extends AbstractSampleConsumer {
  ...
  private String aggregatedKeysSeriesFormat =
   ...
   */
  public final boolean rendersPercentiles() { return renderPercentiles; }
   ...
   */
  protected final GraphKeysSelector getKeysSelector() { return keysSelector; }
   ...
   */
  public final String getOverallSeriesFormat() { return overallSeriesFormat; }
   ...
   *
   * @param aggregatedKeysSeriesFormat the format for the name of aggregated
   *     keys seriesData to set
   */
  public final void
  ...
  private static void setMinResult(MapResultData result, String name,
                                   Double value) {
  ...
  private static void setMaxResult(MapResultData result, String name,
                                   Double value) {
    ValueResultData valueResult = (ValueResultData)result.getResult(name);
   ...
   * @param aggregated
   */
  private void addKeyData(MapResultData result,
                          @SuppressWarnings("unused") String group,
                          String series, SeriesData seriesData,
                          boolean aggregated) {
    ...
    ListResultData dataResult =
        (ListResultData)seriesResult.getResult(RESULT_SERIES_DATA);
      ...
      for (String seriesName : groupInfo.getSeriesSelector().select(sample)) {
        ...
        if (seriesData == null) {
          boolean isControllersSeries =
              groupInfo.getSeriesSelector().allowsControllersDiscrimination() &&
              sample.isController();

          seriesData = new SeriesData(factory, aggregatedKeysSeries,
                                      isControllersSeries, false);
  ...
  public void stopConsuming() {
    ...
    for (Map.Entry<String, GroupInfo> groupEntry : groupInfos.entrySet()) {
      ...
      for (Map.Entry<String, SeriesData> seriesEntry :
           groupData.getSeriesInfo().entrySet()) {
      ...
      if (overallSeries) {
        ...
        String overallSeriesName =
            String.format(overallSeriesFormat, groupName);
