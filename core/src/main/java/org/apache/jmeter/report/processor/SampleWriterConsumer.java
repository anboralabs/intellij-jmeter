...
public class SampleWriterConsumer extends AbstractSampleConsumer {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(SampleWriterConsumer.class);
  ...
  public void startConsuming() {
    if (outputFile == null) {
      ...
      if (LOG.isInfoEnabled()) {
        LOG.info(
            "startConsuming(): No output file set, writing to work directory :" +
            wd.getAbsolutePath());
      }
    ...
    for (int i = 0; i < channelsCount; i++) {
      csvWriters[i] =
          new CsvSampleWriter(getOutputFile(i), getConsumedMetadata(i));
