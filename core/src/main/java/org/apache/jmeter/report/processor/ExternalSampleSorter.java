...
public class ExternalSampleSorter extends AbstractSampleConsumer {
   ...
   */
  public boolean isParallelize() { return parallelize; }
  ...
  public void sort(CsvFile inputFile, File outputFile, boolean writeHeader) {
    ...
    try (CsvSampleReader csvReader =
             new CsvSampleReader(inputFile, inputFile.getSeparator(), false)) {
   ...
   *            The CSV metadata : header information + separator (must not be
   * {@code null})
   * @param inputFile
  ...
  public void sort(SampleMetadata sampleMetadata, File inputFile,
    ...
    try (CsvSampleReader csvReader =
             new CsvSampleReader(inputFile, sampleMetadata)) {
      sort(csvReader, outputFile, writeHeader);
    }
  }
   ...
   *
   * @param csvReader
   *            The reader that provides the samples to be sorted (must not be
   * {@code null})
   * @param output
  ...
  private File sortAndDump(final List<Sample> samples,
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("sortAndDump(): in " +
                (System.currentTimeMillis() - start) / 1000f + " s. Sorted  " +
  ...
  private boolean leftBeforeRight(Sample leftSample, Sample rightSample) {
    return !revertedSort &&
        ...
        revertedSort && sampleComparator.compare(leftSample, rightSample) >= 0;
  }
  ...
  private File getChunkFile() {
    ...
    File out =
        new File(getWorkingDirectory(),
                 "chunk-" + df.format(sequence.incrementAndGet()) + ".csv");
    out.deleteOnExit();
    return out;
  }
   ...
   */
  public final boolean isRevertedSort() { return revertedSort; }
