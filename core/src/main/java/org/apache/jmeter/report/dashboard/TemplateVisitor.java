...
public class TemplateVisitor extends SimpleFileVisitor<Path> {
   ...
   *            the data to inject
   */
  public TemplateVisitor(Path source, Path target, Configuration configuration,
                         DataContext data) {
    this.source = source;
    this.target = target;
  ...
  public FileVisitResult preVisitDirectory(Path file, BasicFileAttributes attrs)
    ...
    try {
      ...
      LOGGER.info(
          "Copying folder from '{}' to '{}', got message: "
              +
  ...
  public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
    ...
    if (TEMPLATED_FILE_EXT.equalsIgnoreCase(extension)) {
      ...
      Path newPath =
          target.resolve(FilenameUtils.removeExtension(templatePath));
