...
public class HtmlTemplateExporter extends AbstractDataExporter {
   ...
   */
  private static void addToContext(String key, Object value,
                                   DataContext context) {
  ...
  private static class ExtraOptionsResultCustomizer
    ...
    public ResultData customizeResult(ResultData result) {
      ...
      extraOptions.getProperties().forEach(
          (key,
           value) -> extraResult.setResult(key, new ValueResultData(value)));
  ...
  private static class EmptyGraphChecker implements ResultChecker {
    ...
    public final void setGraphId(String graphId) { this.graphId = graphId; }
    ...
    public EmptyGraphChecker(boolean filtersOnlySampleSeries,
                             boolean showControllerSeriesOnly,
    ...
    public boolean checkResult(DataContext dataContext, ResultData result) {
      ...
      ResultData seriesResult =
          findData(AbstractGraphConsumer.RESULT_SERIES, result);
      ...
      while (index < count && !matches) {
        ...
        if (currentResult instanceof MapResultData) {
          ...
          String name =
              findValue(String.class, AbstractGraphConsumer.RESULT_SERIES_NAME,
                        seriesData);

          // Is the current series a controller series ?
          boolean isController = findValue(
              Boolean.class, AbstractGraphConsumer.RESULT_SERIES_IS_CONTROLLER,
              seriesData);
          ...
          if (matches) {
            ...
            if (log.isDebugEnabled()) {
              log.debug(
                  "name:{} does not match pattern:{}, filtersOnlySampleSeries:{},"
                      + " supportsControllerDiscrimination:{}",
                  name, filterPattern.pattern(), filtersOnlySampleSeries,
                  supportsControllerDiscrimination);
            }
          }
        }
  ...
  private static boolean htmlReportFileFilter(File file) {
    ...
    boolean isContentOrAdmin =
        fileName.equals("content") || fileName.startsWith("sbadmin2-");
  ...
  public void export(SampleContext context, File file,
    ...
    File templateDirectory = getPropertyFromConfig(
        exportCfg, TEMPLATE_DIR,
        new File(JMeterUtils.getJMeterBinDir(), TEMPLATE_DIR_NAME_DEFAULT),
        File.class);
    if (!templateDirectory.isDirectory()) {
      String message = String.format("\"%s\" is not a valid template directory",
    ...
    File outputDir = getPropertyFromConfig(
        exportCfg, OUTPUT_DIR,
        new File(JMeterUtils.getJMeterBinDir(), OUTPUT_DIR_NAME_DEFAULT),
        File.class);
    ...
    JOrphanUtils.canSafelyWriteToFolder(
        outputDir, HtmlTemplateExporter::htmlReportFileFilter);
    ...
    addToContext(DATA_CTX_FILTERS_ONLY_SAMPLE_SERIES, filtersOnlySampleSeries,
                 dataContext);
    ...
    if (StringUtils.isNotBlank(seriesFilter)) {
      try {
        ...
        log.error("Invalid series filter: '{}', {}", seriesFilter,
                  ex.getDescription());
      }
    }
    ...
    final boolean showControllerSeriesOnly =
        exportCfg.showControllerSeriesOnly();
    addToContext(DATA_CTX_SHOW_CONTROLLERS_ONLY, showControllerSeriesOnly,
                 dataContext);
    ...
    addResultToContext(ReportGenerator.APDEX_SUMMARY_CONSUMER_NAME, storedData,
                       dataContext, jsonizer);

    // Add errors summary consumer result to the data context
    addResultToContext(ReportGenerator.ERRORS_SUMMARY_CONSUMER_NAME, storedData,
                       dataContext, jsonizer);
    ...
    ExtraOptionsResultCustomizer customizer =
        new ExtraOptionsResultCustomizer();
    EmptyGraphChecker checker = new EmptyGraphChecker(
        filtersOnlySampleSeries, showControllerSeriesOnly, filterPattern);
    Map<String, GraphConfiguration> mapConfiguration = new HashMap<>();
    DataContext customGraphs = new DataContext();

    for (Map.Entry<String, GraphConfiguration> graphEntry :
         configuration.getGraphConfigurations().entrySet()) {
    ...
    // timestamp
    long oldTimestamp =
        formatTimestamp(ReportGenerator.BEGIN_DATE_CONSUMER_NAME, dataContext);
    ...
    addToContext(DATA_CTX_TIMEZONE_OFFSET, timezone.getOffset(oldTimestamp),
                 dataContext);
    ...
    try {
      ...
      if (log.isInfoEnabled()) {
        log.info("Report will be generated in: {}, creating folder structure",
                 outputDir.getAbsolutePath());
      }
      FileUtils.forceMkdir(outputDir);
      TemplateVisitor visitor =
          new TemplateVisitor(templateDirectory.toPath(), outputDir.toPath(),
  ...
  private static <T> void addResultToContext(String resultKey,
  ...
  addResultToContext(String resultKey, Map<String, Object> storage,
                     DataContext dataContext, ResultDataVisitor<T> visitor,
                     ResultCustomizer customizer, ResultChecker checker) {
