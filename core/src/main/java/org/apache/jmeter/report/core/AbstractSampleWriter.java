...
public abstract class AbstractSampleWriter extends SampleWriter {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(AbstractSampleWriter.class);

  /** output writer to write samples to */
  protected PrintWriter writer;

  /**
   * Set the new writer on which samples will be written by this sample
   * writer.<br> If any writer exists on the sample writer, it is flushed and
   * closed before being replaced by the new one.
   *
   * @param newWriter
  ...
  public void setWriter(Writer newWriter) {
    ...
    this.writer =
        new PrintWriter(new BufferedWriter(newWriter, BUF_SIZE), false);
  }
