...
public class DistributedRunner {
  private static final String HOST_NOT_FOUND_MESSAGE =
      "Host not found in list of active engines: {}";

  private static final Logger log =
      LoggerFactory.getLogger(DistributedRunner.class);

  public static final String RETRIES_NUMBER = "client.tries"; // $NON-NLS-1$
  public static final String RETRIES_DELAY =
      "client.retries_delay"; // $NON-NLS-1$
  public static final String CONTINUE_ON_FAIL =
      "client.continue_on_fail"; // $NON-NLS-1$
  ...
  public DistributedRunner() { this(new Properties()); }
    ...
    for (int tryNo = 0; tryNo < retriesNumber; tryNo++) {
      if (tryNo > 0) {
        println("Following remote engines will retry configuring: " + addrs +
                ", pausing before retry for " + retriesDelay + "ms");
        try {
          Thread.sleep(retriesDelay);
        } catch (InterruptedException e) { // NOSONAR
          throw new IllegalStateException(
              "Interrupted while initializing remote engines:" + addrs, e);
        }
      }
  ...
  public void start(List<String> addresses) {
    ...
    println("Starting distributed test with remote engines: " + addresses +
            " @ " + formatLikeDate(now) + " (" + now.toEpochMilli() + ')');
      ...
      try {
        if (engine != null) {
          ...
          failedEngines.add(address);
               ...
               JMeterEngineException e) { // NOSONAR already reported to user
        failedEngines.add(address);
        JMeterUtils.reportErrorToUser(
            e.getMessage(),
            JMeterUtils.getResString("remote_error_starting")); // $NON-NLS-1$
      }
    }
  ...
  private JMeterEngine getClientEngine(String address, HashTree testTree) {
    ...
    try {
      ...
      JMeterUtils.reportErrorToUser(
          ex.getMessage(),
          JMeterUtils.getResString("remote_error_init") + ": " +
              address); // $NON-NLS-1$ $NON-NLS-2$
      return null;
    }
  }
