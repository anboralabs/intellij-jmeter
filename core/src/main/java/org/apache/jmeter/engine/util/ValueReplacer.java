...
public class ValueReplacer {
  private static final Logger log =
      LoggerFactory.getLogger(ValueReplacer.class);
  ...
  /**
   * Constructor which couples the given {@link TestPlan} to this by means of
  ...
  boolean containsKey(String k) { return variables.containsKey(k); }
   ...
   * @throws InvalidVariableException when transforming of the variables goes
   *     awry and
  ...
  public void replaceValues(TestElement el) throws InvalidVariableException {
    Collection<JMeterProperty> newProps = replaceValues(
        el.propertyIterator(),
  ...
  private static void
  setProperties(TestElement el, Collection<? extends JMeterProperty> newProps) {
    el.clear();
   ...
   * @param el {@link TestElement} in which the we will look for strings, that
   *     can be replaced by variable references
   ...
   * regexMatch is <code>true</code>
   * @param el {@link TestElement} in which the we will look for strings, that
   *     can be replaced by variable references
   * @param regexMatch when <code>true</code> variable substitution will be done
   *     in regexp matching mode
   * @throws InvalidVariableException  when transforming of the strings goes
   *     awry and
   * the used transformer throws an {@link InvalidVariableException}
   */
  ...
  public void undoReverseReplace(TestElement el)
    ...
    Collection<JMeterProperty> newProps =
        replaceValues(el.propertyIterator(),
   ...
   */
  public void addVariables(Map<String, String> vars) { variables.putAll(vars); }

  /**
   * Replaces a {@link StringProperty} containing functions with their Function
   * properties equivalent. <p>For example:
   ...
   * @param iter the {@link PropertyIterator} over all properties, in which the
   *     values should be replaced
   * @param transform the {@link PropertyTransformer}, that should do
   *     transformation
   * @return a new {@link Collection} with all the transformed {@link
   *     JMeterProperty}s
   */
  private static Collection<JMeterProperty>
  replaceValues(PropertyIterator iter, PropertyTransformer transform) {
