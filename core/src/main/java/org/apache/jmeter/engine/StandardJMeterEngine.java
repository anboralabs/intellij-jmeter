...
public class StandardJMeterEngine implements JMeterEngine, Runnable {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(StandardJMeterEngine.class);

  // Should we exit at end of the test? (only applies to server, because host is
  // non-null)
  ...
  private static final boolean REMOTE_SYSTEM_EXIT =
      JMeterUtils.getPropDefault("jmeterengine.remote.system.exit", false);

  /** Whether to call System.exit(1) if threads won't stop */
  private static final boolean SYSTEM_EXIT_ON_STOP_FAIL =
      JMeterUtils.getPropDefault("jmeterengine.stopfail.system.exit", true);
   ...
   * test". The use of {@link ExecutorService} allows propagating the exception
   * from the threads. Thread keepalive time is set to 1 second, so threads are
   * released early, so the application can shut down faster.
   */
  private static final ExecutorService EXECUTOR_SERVICE =
      new ThreadPoolExecutor(
          ...
          (runnable)
              -> new Thread(runnable, "StandardJMeterEngine-" +
  ...
  public StandardJMeterEngine() { this(null); }
   ...
   */
  private static void
  initSingletonEngine(StandardJMeterEngine standardJMeterEngine) {
  ...
  private static void removeThreadGroups(List<?> elements) {
    ...
    while (
        iter.hasNext()) { // Can't use for loop here because we remove elements
      Object item = iter.next();
      if (item instanceof AbstractThreadGroup ||
          !(item instanceof TestElement)) {
        iter.remove();
      }
    }
  }
  ...
  private void notifyTestListenersOfStart(
      SearchByClass<? extends TestStateListener> testListeners) {
    for (TestStateListener tl : testListeners.getSearchResults()) {
      try {
        ...
        // TODO: we should not be logging the exceptions multiple times,
        // however, currently GUI does not
        //   monitor if the running test fails, so we log the exception for the
        //   users to see in the logs
        log.error("Unable to execute testStarted({}) for test element {}", host,
                  tl, e);
        throw new IllegalStateException("Unable to execute testStarted(" +
                                            host + ") for test element " + tl,
                                        e);
      }
    }
  }
  ...
  private class StopTest implements Runnable {
    ...
    private StopTest(boolean b) { now = b; }
    ...
    /**
     * For each thread group, invoke {@link
     * AbstractThreadGroup#tellThreadsToStop()}
     */
    ...
    private void resetSingletonEngine() {
      StandardJMeterEngine.engine =
          null; // NOSONAR We cannot make the method static here
    }
        ...
        if (!stopped) { // we totally failed to stop the test
          if (JMeter.isNonGUI()) {
            ...
            if (SYSTEM_EXIT_ON_STOP_FAIL) { // default is true
              log.error("Exiting");
              System.out.println(
                  "Fatal error, could not stop test, exiting"); // NOSONAR
                                                                // Intentional
              System.exit(1); // NOSONAR Intentional
            } else {
              System.out.println(
                  "Fatal error, could not stop test"); // NOSONAR Intentional
            }
          } else {
  ...
  public void run() {
    ...
    try {
      ...
      JMeterUtils.reportErrorToUser(
          "Error occurred compiling the tree: - see log file", e);
    ...
    SearchByClass<TestStateListener> testListeners =
        new SearchByClass<>(TestStateListener.class); // TL - S&E
    ...
    SearchByClass<SetupThreadGroup> setupSearcher =
        new SearchByClass<>(SetupThreadGroup.class);
    SearchByClass<AbstractThreadGroup> searcher =
        new SearchByClass<>(AbstractThreadGroup.class);
    SearchByClass<PostThreadGroup> postSearcher =
        new SearchByClass<>(PostThreadGroup.class);
    ...
    Iterator<SetupThreadGroup> setupIter =
        setupSearcher.getSearchResults().iterator();
    Iterator<AbstractThreadGroup> iter = searcher.getSearchResults().iterator();
    Iterator<PostThreadGroup> postIter =
        postSearcher.getSearchResults().iterator();
    ...
    if (setupIter.hasNext()) {
      ...
      while (running && setupIter.hasNext()) { // for each setup thread group
        ...
        startThreadGroup(group, groupCount, setupSearcher, testLevelElements,
                         notifier);
        if (serialized && setupIter.hasNext()) {
          log.info(
              "Waiting for setup thread group: {} to finish before starting next setup group",
              groupName);
    ...
    boolean mainGroups =
        running; // still running at this point, i.e. setUp was not cancelled
    while (running && iter.hasNext()) { // for each thread group
      AbstractThreadGroup group = iter.next();
      // ignore Setup and Post here.  We could have filtered the searcher. but
      // then future Thread Group objects wouldn't execute.
      if (group instanceof SetupThreadGroup || group instanceof
                                                   PostThreadGroup) {
        continue;
      }
      groupCount++;
      ...
      if (serialized && iter.hasNext()) {
        log.info(
            "Waiting for thread group: {} to finish before starting next group",
            groupName);
        group.waitThreadsStopped();
      }
    ...
    if (postIter.hasNext()) {
      ...
      if (mainGroups &&
          !running) { // i.e. shutdown/stopped during main thread groups
      ...
      while (running && postIter.hasNext()) { // for each setup thread group
        ...
        log.info("Starting tearDown ThreadGroup: {} : {}", groupCount,
                 groupName);
        startThreadGroup(group, groupCount, postSearcher, testLevelElements,
                         notifier);
        if (serialized && postIter.hasNext()) {
          log.info(
              "Waiting for post thread group: {} to finish before starting next post group",
              groupName);
          group.waitThreadsStopped();
        }
      }
  ...
  @SuppressWarnings("FutureReturnValueIgnored")
  public void exit() {
    ClientJMeterEngine.tidyRMI(
        log);                 // This should be enough to allow server to exit.
