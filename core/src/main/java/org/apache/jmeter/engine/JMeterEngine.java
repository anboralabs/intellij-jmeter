...
public interface JMeterEngine {
  ...
  /**
   *
   * @param now boolean that tell whether stop is immediate (interrupt) or not
   *     (wait for current sample end)
   */
  ...
  /**
   * @return boolean Flag to show whether engine is active (true when test is
   *     running). Set to false at end of test
   */
  boolean isActive();
}
