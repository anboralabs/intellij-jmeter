...
/**
 * Converts the Remoteable Test and Sample Listeners in the test tree by
 * wrapping them with RemoteSampleListeners so that the samples are returned to
 * the client.
 *
...
public class ConvertListeners implements HashTreeTraverser {
  private static final Logger log =
      LoggerFactory.getLogger(ConvertListeners.class);
  ...
  public void addNode(Object node, HashTree subTree) {
    for (Object item : subTree.list()) {
      if (item instanceof AbstractThreadGroup && log.isDebugEnabled()) {
        log.debug("num threads = {}",
                  ((AbstractThreadGroup)item).getNumThreads());
      }
      if (item instanceof Remoteable) {
        if (item instanceof RemoteThreadsListenerTestElement) {
          // Used for remote notification of threads start/stop,see BUG 54152
          try {
            RemoteThreadsListenerWrapper wrapper =
                new RemoteThreadsListenerWrapper(
                    new RemoteThreadsListenerImpl());
            subTree.replaceKey(item, wrapper);
          } catch (RemoteException e) {
            log.error("Error replacing {} by wrapper: {}",
                      RemoteThreadsListenerTestElement.class,
        ...
        try {
          ...
          if (item instanceof TestStateListener &&
              item instanceof SampleListener) { // TL - all
          ...
          log.error("RemoteException occurred while replacing Remotable item.",
                    e); // $NON-NLS-1$
        }
      }
    }
