...
public final class RemoteJMeterEngineImpl
    extends java.rmi.server.UnicastRemoteObject implements RemoteJMeterEngine {
  private static final long serialVersionUID = 242L;

  private static final Logger log =
      LoggerFactory.getLogger(RemoteJMeterEngineImpl.class);
  ...
  private RemoteJMeterEngineImpl(int localPort, int rmiRegistryPort)
  ...
  public static void startServer(int rmiRegistryPort) throws RemoteException {
    RemoteJMeterEngineImpl engine = new RemoteJMeterEngineImpl(
        RmiUtils.DEFAULT_LOCAL_PORT, rmiRegistryPort);
    engine.init();
  }
  ...
  private void init() throws RemoteException {
    ...
    if (localHost.isSiteLocalAddress()) {
      // should perhaps be log.warn, but this causes the client-server test to
      // fail
      log.info(
    ...
    if (CREATE_SERVER) {
      ...
      try {
        reg = LocateRegistry.createRegistry(
            this.rmiRegistryPort, RmiUtils.createClientSocketFactory(),
            RmiUtils.createServerSocketFactory());
    ...
    try {
      if (reg == null) {
        ...
        reg = LocateRegistry.getRegistry(RmiUtils.getRmiHost().getHostName(),
   ...
   */
  @Override
  public void rconfigure(HashTree testTree, String hostAndPort, File jmxBase,
    ...
    synchronized (
        LOCK) { // close window where another remote client might jump in
  ...
  public void rreset() throws RemoteException {
    // Mail on userlist reported NPE here - looks like only happens if there are
    // network errors, but check anyway
  ...
  public void rexit() throws RemoteException {
    ...
    Registry reg = LocateRegistry.getRegistry(
        RmiUtils.getRmiHost().getHostName(), this.rmiRegistryPort,
        RmiUtils.createClientSocketFactory());
    try {
  ...
  public void rsetProperties(HashMap<String, String> map)
      throws RemoteException { // NOSONAR
  ...
  private void checkOwner(String methodName) {
    if (ownerThread != null && ownerThread != Thread.currentThread()) {
      String msg =
          "The engine is not owned by this thread - cannot call " + methodName;
      log.warn(msg);
