...
public class ClientJMeterEngine implements JMeterEngine {
  ...
  private static RemoteJMeterEngine getEngine(String hostAndPort)
    ...
    final String name = RemoteJMeterEngineImpl
                            .JMETER_ENGINE_RMI_NAME; // $NON-NLS-1$ $NON-NLS-2$
    ...
    Registry registry = LocateRegistry.getRegistry(
        host, port, RmiUtils.createClientSocketFactory());
    ...
    if (remobj instanceof RemoteJMeterEngine) {
      ...
      if (remobj instanceof RemoteObject) {
        ...
        System.out.println("Using remote object: " +
                           robj.getRef().remoteToString()); // NOSONAR
      }
      return rje;
    }
  ...
  public ClientJMeterEngine(String hostAndPort)
  ...
  public void stopTest(boolean now) {
    log.info("About to {} remote test on {}", now ? "stop" : "shutdown",
             hostAndPort);
    try {
  ...
  public void runTest() throws JMeterEngineException {
    ...
    synchronized (testTree) {
      ...
      testTree.traverse(
          compiler); // limit the changes to client only test elements
      JMeterContextService.initClientSideVariables(
          compiler.getClientSideVariables());
    ...
    try {
      ...
      log.info("sent test to {} basedir='{}'", hostAndPort,
               baseDirRelative); // $NON-NLS-1$
      if (savep == null) {
  ...
  toHashMapOfString(Properties properties) {
    return new HashMap<>(
  ...
  public static void tidyRMI(Logger logger) {
    String reaperRE =
        JMeterUtils.getPropDefault("rmi.thread.name", "^RMI Reaper$");
   ...
   */
  public String getHost() { return hostAndPort; }
