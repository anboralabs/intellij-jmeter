...
class FunctionParser {
  ...
  ArrayList<Object> compileString(String value)
    ...
    try {
      while (reader.read(current) == 1) {
        if (current[0] == '\\') { // Handle escapes
          ...
          // N.B. This method is used to parse function parameters, so must
          // treat ',' as special
      ...
      while (reader.read(current) == 1) {
        if (current[0] == '\\') {
          ...
          if (function instanceof Function) {
            ...
            if (firstNonSpace(reader, '#') != '}') {
              ...
              throw new InvalidVariableException(
                  "Expected } after " + funcName + " function call in " +
          ...
          if (function instanceof
              Function) { // ensure that setParameters() is called.
  ...
  private static char firstNonSpace(StringReader reader, char defaultResult)
      throws IOException {
    char[] current = new char[1];
    while (reader.read(current) == 1) {
   ...
   * <li></li>
   * </ul>
   * @param reader a StringReader pointing to the current input location, just
   *     after "("
   * @return a list of CompoundVariable elements
   * @throws InvalidVariableException when evaluation of variables fail
   */
  @SuppressWarnings("NonApiType")
  ArrayList<CompoundVariable> parseParams(StringReader reader)
      throws InvalidVariableException {
      ...
      while (reader.read(current) == 1) {
        if (current[0] == '\\') {    // Process escaped characters
          ...
          result.add(param);
                   ...
                   parenRecursion == 0) {
          // Detect functionName() so this does not generate empty string as the
          // parameter
