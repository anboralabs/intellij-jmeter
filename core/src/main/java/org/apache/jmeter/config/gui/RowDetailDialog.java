...
public class RowDetailDialog
    extends JDialog implements ActionListener, DocumentListener, ItemListener {
  ...
  private void init() { // WARNING: called from ctor so must not be overridden
    ...
    for (int column = 0; column < columnCount; column++) {
      JLabel dataLabel = new JLabel(
          JMeterUtils.getResString(tableModel.getColumnName(column)));
      dataPanel.add(dataLabel);
      JComponent component =
          createComponentForRow(columnCount, linesPerTextArea, column,
    ...
    JButton updateButton =
        new JButton(JMeterUtils.getResString("update")); //$NON-NLS-1$
  ...
  private JComponent createComponentForRow(final int columnCount,
                                           final int linesPerTextArea,
    ...
    if (column > 0 || columnCount == 1) {
      if (tableModel.getColumnClass(column).equals(Boolean.class)) {
        ...
        JSyntaxTextArea dataArea =
            JSyntaxTextArea.getInstance(linesPerTextArea, 80);
  ...
  private void setValues(int selectedRow) {
    for (int i = 0; i < tableModel.getColumnCount(); i++) {
      ...
      if (component instanceof JTextComponent) {
        ...
        ((JCheckBox)component)
            .setSelected((Boolean)tableModel.getValueAt(selectedRow, i));
      }
    }
    textChanged = false;
  }
  ...
  protected void doUpdate(ActionEvent actionEvent) {
    for (int i = 0; i < tableModel.getColumnCount(); i++) {
      final JComponent component = dataComponents.get(i);
      if (component instanceof JTextComponent) {
        tableModel.setValueAt(((JTextComponent)component).getText(),
                              selectedRow, i);
      } else if (component instanceof JCheckBox) {
        tableModel.setValueAt(((JCheckBox)component).isSelected(), selectedRow,
                              i);
      }
    }
