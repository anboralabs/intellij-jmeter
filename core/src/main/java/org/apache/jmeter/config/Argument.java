...
public class Argument extends AbstractTestElement implements Serializable {
   ...
   */
  public Argument() { this(null, null, null, null); }
   ...
   *            the argument value
   */
  public Argument(String name, String value) { this(name, value, null, null); }
  ...
  @Override
  public PropertiesAccessor<? extends Argument, ? extends ArgumentSchema>
  getProps() {
   ...
   */
  public String getValue() { return get(getSchema().getValue()); }
   ...
   */
  public String getDescription() { return get(getSchema().getDescription()); }
   ...
   * @return the MetaData value
   */
  public String getMetaData() { return get(getSchema().getMetadata()); }
  ...
  public boolean isSkippable(String parameterName) {
    if (JOrphanUtils.isBlank(parameterName)) {
      return true; // Skip parameters with a blank name (allows use of optional
                   // variables in parameter lists)
    }
    // TODO: improve this test
    if (parameterName.trim().startsWith("${") &&
        parameterName.endsWith("}")) { // $NON-NLS-1$ $NON-NLS-2$
