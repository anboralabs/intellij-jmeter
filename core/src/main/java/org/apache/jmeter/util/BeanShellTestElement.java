...
public abstract class BeanShellTestElement extends AbstractTestElement
  ...
  private static final Logger log =
      LoggerFactory.getLogger(BeanShellTestElement.class);
  ...
  protected BeanShellInterpreter getBeanShellInterpreter() {
    if (isResetInterpreter()) {
      try {
        ...
        log.error("Cannot reset BeanShell: {}",
   ...
   *
   * @param bsh          the interpreter, not {@code null}
   * @param sampleResult sampler result to set {@code setSamplerData} or {@code
   *     null}
   * @return the result of the script, may be {@code null}
   * @throws JMeterException when working with the bsh fails
   */
  ...
  public void setParameters(String s) { parameters = s; }
  ...
  public boolean isResetInterpreter() { return resetInterpreter; }

  public void setResetInterpreter(boolean b) { resetInterpreter = b; }
}
