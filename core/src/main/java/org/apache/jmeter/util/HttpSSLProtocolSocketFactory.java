...
public class HttpSSLProtocolSocketFactory
  ...
  private static final Logger log =
      LoggerFactory.getLogger(HttpSSLProtocolSocketFactory.class);

  private static final String PROTOCOL_LIST = JMeterUtils.getPropDefault(
      "https.socket.protocols", ""); // $NON-NLS-1$ $NON-NLS-2$

  private static final String[] protocols =
      PROTOCOL_LIST.split(" "); // $NON-NLS-1$

  private static final String CIPHER_LIST = JMeterUtils.getPropDefault(
      "https.cipherSuites", ""); // $NON-NLS-1$ $NON-NLS-2$

  private static final String[] ciphers =
      CIPHER_LIST.split(", *"); // $NON-NLS-1$

  static {
    if (!PROTOCOL_LIST.isEmpty()) {
      log.info("Using protocol list:{} and cipher list: {}", PROTOCOL_LIST,
               CIPHER_LIST);
    }
  }
  ...
  public HttpSSLProtocolSocketFactory() { this(0); }

  public HttpSSLProtocolSocketFactory(int cps) { this.cps = cps; }
  ...
  private static SSLSocketFactory getSSLSocketFactory() throws IOException {
    try {
      SSLContext sslContext =
          ((JsseSSLManager)SSLManager.getInstance()).getContext();
