...
public abstract class SSLManager {
  ...
  private static final String SSL_TRUST_STORE =
      "javax.net.ssl.trustStore"; // $NON-NLS-1$

  private static final String KEY_STORE_PASSWORD =
      "javax.net.ssl.keyStorePassword"; // $NON-NLS-1$ NOSONAR no hard coded
                                        // password

  public static final String JAVAX_NET_SSL_KEY_STORE =
      "javax.net.ssl.keyStore"; // $NON-NLS-1$

  private static final String JAVAX_NET_SSL_KEY_STORE_TYPE =
      "javax.net.ssl.keyStoreType"; // $NON-NLS-1$
   ...
   */
  public static synchronized void reset() { SSLManager.manager = null; }
  ...
  protected synchronized JmeterKeyStore getKeyStore() {
    if (null == this.keyStore) {
      ...
      String fileType = System.getProperty(
          JAVAX_NET_SSL_KEY_STORE_TYPE, // use the system property to determine
                                        // the type
          fileName.toLowerCase(Locale.ENGLISH).endsWith(".p12")
              ? PKCS12
              : "JKS"); // otherwise use the name
      log.info("JmeterKeyStore Location: {} type {}", fileName, fileType);
      try {
        this.keyStore = JmeterKeyStore.getInstance(
            fileType, keystoreAliasStartIndex, keystoreAliasEndIndex,
            clientCertAliasVarName);
        log.info("KeyStore created OK");
      } catch (Exception e) {
        this.keyStore = null;
        throw new IllegalArgumentException(
            "Could not create keystore: " + e.getMessage(), e);
      }
      ...
      try {
        ...
        if ("NONE".equalsIgnoreCase(fileName)) {
          ...
          log.info("Total of {} aliases loaded OK from PKCS11",
        ...
        log.error(
            "Problem finding the correct algorithm while loading keys from keystore '{}'",
            fileName, e);
      } catch (CertificateException e) {
        log.error("Problem with one of the certificates/keys in keystore '{}'",
                  fileName, e);
  ...
  private void retryLoadKeys(File initStore, boolean allowEmptyPassword)
    ...
    for (int i = 0; i < 3; i++) {
      ...
      try {
        ...
        log.warn("Could not load keystore '{}'. Wrong password for keystore?",
                 initStore, e);
      }
      this.defaultpw = null;
    }
  ...
  /*
   * The password can be defined as a property; this dialogue is provided to
   * allow it to be entered at run-time.
   */
  private String getPassword() {
    ...
    if (null == password) {
      ...
      if (guiInstance != null) {
        JPanel panel =
            new JPanel(new MigLayout("fillx, wrap 2", "[][fill, grow]"));
        ...
        int choice = JOptionPane.showConfirmDialog(
            guiInstance.getMainFrame(), panel,
   ...
   * </ul>
   * If the KeyStore object cannot be created, then this is currently treated
   * the same as if no truststore name was provided.
   *
   * @return
   *         {@code null} when Java truststore should be used.
   *         Otherwise the truststore, which may be empty if the file could not
   * be loaded.
   *
   */
    ...
    if (!truststoreLoaded) {
      ...
      try {
        ...
        throw new RuntimeException(
            "Problem creating truststore: " + e.getMessage(), e);
      }
      ...
      try {
        ...
        throw new RuntimeException("Can't load TrustStore: " + e.getMessage(),
                                   e);
      }
    }
   ...
   */
  public static boolean isSSLSupported() { return SSLManager.IS_SSL_SUPPORTED; }
   ...
   *            as default key
   */
  public synchronized void configureKeystore(boolean preload, int startIndex,
                                             int endIndex,
   ...
   * Destroy Keystore
   */
  public synchronized void destroyKeystore() { keyStore = null; }
}
