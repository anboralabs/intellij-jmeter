...
public class BeanShellInterpreter {
  ...
  private final String
      initFile; // Script file to initialize the Interpreter with

  private final Logger
      logger; // Logger to use during initialization and script run
  ...
  // Called from ctor, so must be private (or final, but it does not seem useful
  // elsewhere)
  private void init() throws ClassNotFoundException {
    ...
    if (StringUtils.isNotBlank(initFile)) {
      ...
      if (!in.exists()) { // Cannot find the file locally, so try the bin
                          // directory
        fileToUse = JMeterUtils.getJMeterHome() + File.separator +
                    "bin" // $NON-NLS-1$
   ...
   */
  public void reset() throws ClassNotFoundException { init(); }

  private Object bshInvoke(Method m, Object[] o, boolean shouldLog)
    ...
    try {
      r = m.invoke(bshInstance, o);
             ...
             IllegalAccessException e) { // Programming error
  ...
  public static boolean isInterpreterPresent() { return bshClass != null; }
}
