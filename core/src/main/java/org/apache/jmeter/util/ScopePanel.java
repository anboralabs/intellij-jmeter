...
public class ScopePanel
  ...
  public ScopePanel(boolean enableVariableButton,
    ...
    parentButton = new JRadioButton(
        JMeterUtils.getResString("sample_scope_parent")); //$NON-NLS-1$
    if (enableParentAndSubsamples) {
      allButton = new JRadioButton(
          JMeterUtils.getResString("sample_scope_all")); //$NON-NLS-1$
    } else {
      allButton = null;
    }
    if (enableSubsamplesOnly) {
      childButton = new JRadioButton(
          JMeterUtils.getResString("sample_scope_children")); //$NON-NLS-1$
    } else {
      childButton = null;
    }
    if (enableVariableButton) {
      variableButton = new JRadioButton(
          JMeterUtils.getResString("sample_scope_variable")); //$NON-NLS-1$
  ...
  private void init() { // WARNING: called from ctor so must not be overridden
    ...
    setBorder(BorderFactory.createTitledBorder(
        JMeterUtils.getResString("sample_scope"))); //$NON-NLS-1$
  ...
  public void clearGui() { parentButton.setSelected(true); }
  ...
  public void setScopeAll() { setScopeAll(false); }
  ...
  public void setScopeChildren() { setScopeChildren(false); }
  ...
  public void setScopeParent() { setScopeParent(false); }
  ...
  public boolean isScopeParent() { return parentButton.isSelected(); }
  ...
  public String getVariable() { return variableName.getText(); }
