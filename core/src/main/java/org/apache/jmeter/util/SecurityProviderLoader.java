...
public class SecurityProviderLoader {
  ...
  private static final Pattern CONFIGURATION_REGEX = Pattern.compile(
      "^(?<classname>[^:]+)(:(?<position>\\d+)(:(?<config>.+))?)?$");
  ...
  public static void addSecurityProvider(Properties properties) {
    properties.keySet()
        ...
        .forEach(key -> addSecurityProvider(properties.get(key).toString()));
  }
    ...
    if (matcher.matches()) {
      ...
      try {
        ...
        String message = String.format(
            "Security Provider '%s' could not be installed.", classname);
  ...
  tryConstructorWithString(Class<Provider> providerClass, final String config)
