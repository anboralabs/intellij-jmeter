...
public class JsseSSLManager extends SSLManager {
  private static final Logger log =
      LoggerFactory.getLogger(JsseSSLManager.class);

  // Temporary fix to allow default protocol to be changed
  private static final String DEFAULT_SSL_PROTOCOL = JMeterUtils.getPropDefault(
      "https.default.protocol", "TLS"); // $NON-NLS-1$ // $NON-NLS-2$

  // Allow reversion to original shared session context
  private static final boolean SHARED_SESSION_CONTEXT =
      JMeterUtils.getPropDefault("https.sessioncontext.shared",
                                 false); // $NON-NLS-1$

  /**
   * Characters per second, used to slow down sockets
   */
  public static final int CPS = JMeterUtils.getPropDefault(
      "httpclient.socket.https.cps", 0); // $NON-NLS-1$

  static {
    if (log.isInfoEnabled()) {
      log.info("Using default SSL protocol: {}", DEFAULT_SSL_PROTOCOL);
      log.info("SSL session context: {}",
               SHARED_SESSION_CONTEXT ? "shared" : "per-thread");
  ...
  public JsseSSLManager(Provider provider) {
    ...
    try {
      ...
      HttpsURLConnection.setDefaultSSLSocketFactory(
          new HttpSSLProtocolSocketFactory(CPS));
      HttpsURLConnection.setDefaultHostnameVerifier(
          (hostname, session)
              -> true); // NOSONAR JMeter is a pentest and perf testing tool
  ...
  public void setContext(HttpURLConnection conn) {
    ...
    if (!(conn instanceof HttpsURLConnection)) {
      if (log.isWarnEnabled()) {
        log.warn("Unexpected HttpURLConnection class: {}",
  ...
  private SSLContext createContext() throws GeneralSecurityException {
    ...
    for (int i = 0; i < trustmanagers.length; i++) {
      if (trustmanagers[i] instanceof X509TrustManager) {
        trustmanagers[i] =
            new CustomX509TrustManager((X509TrustManager)trustmanagers[i]);
      }
    }
  ...
  private static class WrappedX509KeyManager extends X509ExtendedKeyManager {
     ...
     * This is used for the methods {@link #getServerAliases(String,
     * Principal[])} and {@link #chooseServerAlias(String, Principal[], Socket)}
     */
    ...
    public String[] getClientAliases(String keyType, Principal[] issuers) {
      ...
      // implementation moved to JmeterKeystore as only that has the keyType
      // info
     ...
     * @param issuers the list of acceptable CA issuer subject names or null if
     *     it does not matter which issuers are used.
     * @param socket the socket to be used for this connection.
     *     This parameter can be null, which indicates that implementations are
     * free to select an alias applicable to any socket.
     *
     ...
     * in JMeter.
     *
     * @see javax.net.ssl.X509KeyManager#chooseServerAlias(String, Principal[],
     *     Socket)
     */
    @Override
