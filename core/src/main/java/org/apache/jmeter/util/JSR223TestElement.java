...
public abstract class JSR223TestElement
    extends ScriptingTestElement implements Serializable, TestStateListener {
   ...
   */
  private static final Cache<ScriptCacheKey, CompiledScript>
      COMPILED_SCRIPT_CACHE =
          Caffeine.newBuilder()
              .maximumSize(JMeterUtils.getPropDefault(
                  "jsr223.compiled_scripts_cache_size", 100))
              .build();

  /**
   * Lambdas can't throw checked exceptions, so we wrap cache loading failure
   * with a runtime one.
   */
  ...
  protected JSR223TestElement() { super(); }

  /**
   * @return {@link ScriptEngine} for language defaulting to groovy if language
   *     is not set
  ...
  protected ScriptEngine getScriptEngine() throws ScriptException {
    ...
    if (scriptEngine == null) {
      throw new ScriptException(
          "Cannot find engine named: '" + lang +
          "', ensure you set language field in JSR223 Test Element: " +
          getName());
    }
  ...
  protected void populateBindings(Bindings bindings) {
    ...
    bindings.put("Parameters",
                 scriptParameters); // $NON-NLS-1$ (this name is fixed)
  ...
  /**
   * This method will run inline script or file script with special behaviour
   * for file script:
  ...
  protected Object processFileOrScript(ScriptEngine scriptEngine,
    ...
    boolean supportsCompilable =
        ...
        !"bsh.engine.BshScriptEngine".equals(
            scriptEngine.getClass().getName()); // NOSONAR // $NON-NLS-1$
    try {
      if (!StringUtils.isEmpty(filename)) {
        if (!scriptFile.isFile()) {
          throw new ScriptException(
              "Script file '" + scriptFile.getAbsolutePath() +
        ...
        ScriptCacheKey newCacheKey = ScriptCacheKey.ofFile(
            getScriptLanguage(), scriptFile.getAbsolutePath(),
      ...
      if (!StringUtils.isEmpty(script)) {
        ...
        throw new ScriptException(
            "Both script file and script text are empty for element:" +
            getName());
      }
  ...
  private static <T extends ScriptCacheKey> CompiledScript getCompiledScript(
    ...
    try {
      CompiledScript compiledScript =
          COMPILED_SCRIPT_CACHE.get(newCacheKey, compiler);
      if (compiledScript == null) {
        throw new ScriptException("Script compilation returned null: " +
                                  newCacheKey);
      }
      ...
      if (cause instanceof IOException) {
        cause.addSuppressed(new IllegalStateException(
            "Unable to compile script " + newCacheKey));
  ...
  public boolean compile() throws ScriptException, IOException {
    ...
    if (!StringUtils.isEmpty(getScript())) {
      try {
        ...
        logger.error("Error compiling script for test element {}, error:{}",
  ...
  public void setScriptLanguage(String s) { scriptLanguage = s; }
}
