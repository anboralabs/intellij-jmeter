...
public class SSLRMIServerSocketFactory
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(SSLRMIServerSocketFactory.class);
  ...
  public ServerSocket createServerSocket(int port) throws IOException {
    ...
    try {
      socket =
          (SSLServerSocket)factory.createServerSocket(port, 0, localAddress);
    } catch (BindException e) {
      throw new IOException(
          "Could not bind to " + localAddress + " using port " + port, e);
    }
  ...
  public String toString() {
    return "SSLRMIServerSocketFactory(host=" + localAddress +
        ", keyStoreLocation=" + this.keyStoreLocation +
