...
public final class RmiUtils {
  ...
  public static final int DEFAULT_RMI_REGISTRY_PORT =
      JMeterUtils.getPropDefault("server_port", 0); // $NON-NLS-1$
  ...
  private static final String KEYSTORE_TYPE =
      JMeterUtils.getPropDefault("server.rmi.ssl.keystore.type", "JKS");

  private static final String KEYSTORE_FILE = JMeterUtils.getPropDefault(
      "server.rmi.ssl.keystore.file", "rmi_keystore.jks");

  private static final String KEYSTORE_PASSWORD = JMeterUtils.getPropDefault(
      "server.rmi.ssl.keystore.password", "changeit");

  private static final String KEYSTORE_ALIAS =
      JMeterUtils.getPropDefault("server.rmi.ssl.keystore.alias", "rmi");

  private static final String TRUSTSTORE_TYPE = JMeterUtils.getPropDefault(
      "server.rmi.ssl.truststore.type", KEYSTORE_TYPE);

  private static final String TRUSTSTORE_FILE = JMeterUtils.getPropDefault(
      "server.rmi.ssl.truststore.file", KEYSTORE_FILE);

  private static final String TRUSTSTORE_PASSWORD = JMeterUtils.getPropDefault(
      "server.rmi.ssl.truststore.password", KEYSTORE_PASSWORD);

  private static final boolean SSL_DISABLED =
      JMeterUtils.getPropDefault("server.rmi.ssl.disable", false);
  ...
  private RmiUtils() { super(); }

  public static RMIClientSocketFactory createClientSocketFactory() {
    if (SSL_DISABLED) {
      log.info(
          "Disabling SSL for RMI as server.rmi.ssl.disable is set to 'true'");
      return null;
    }
    ...
    factory.setTruststore(TRUSTSTORE_FILE, TRUSTSTORE_TYPE,
                          TRUSTSTORE_PASSWORD);
    return factory;
  }

  public static RMIServerSocketFactory createServerSocketFactory()
    ...
    SSLRMIServerSocketFactory factory =
        new SSLRMIServerSocketFactory(getRmiHost());
    factory.setAlias(KEYSTORE_ALIAS);
  ...
  public static InetAddress getRmiHost() throws RemoteException {
    ...
    String host = System.getProperties().getProperty(
        "java.rmi.server.hostname"); // $NON-NLS-1$
    try {
      if (host == null) {
        log.info(
        ...
        log.info(
            "Resolving by name the value of System property 'java.rmi.server.hostname': {}",
            host);
        localHost = InetAddress.getByName(host);
      }
    } catch (UnknownHostException e) {
      throw new RemoteException(
          "Cannot start. Unable to get local host IP address.", e);
    }
  ...
  public static int getRmiRegistryPort() {
    return DEFAULT_RMI_REGISTRY_PORT == 0 ? RmiUtils.DEFAULT_RMI_PORT
