 ...
 * Note that the {@link SummariserRunningSample} start and end times relate to
 * the samples, not the reporting interval.
 *
...
public class Summariser extends AbstractTestElement
   ...
   * share the same instance as the sampleOccurred() methods, so the
   * testStarted() method can fetch the Totals accumulator object for the
   * samples to be stored in.
   */
  ...
  private static final boolean IGNORE_TC_GENERATED_SAMPLERESULT =
   ...
   * totals.
   */
  private static final Map<String, Totals> ACCUMULATORS =
  ...
  public Summariser() {
    // TODO: is it needed to reset static field instanceCount in the instance
    // constructor?
    synchronized (LOCK) {
      ACCUMULATORS.clear();
      instanceCount = 0;
  ...
  public void sampleOccurred(SampleEvent e) {
    ...
    if (IGNORE_TC_GENERATED_SAMPLERESULT &&
        TransactionController.isFromTransactionController(s)) {
      return;
    }
    ...
    synchronized (myTotals) {
      ...
      if ((now > myTotals.last + INTERVAL_WINDOW) &&
    ...
    if (reportNow) {
      ...
      if (myTotal != null && myDelta != null &&
          myTotal.getNumSamples() != myDelta.getNumSamples()) { // NOSONAR
  ...
  private static StringBuilder doubleToSb(DecimalFormat dfDouble,
                                          StringBuilder sb, double d, int len,
                                          int frac) {
    sb.setLength(0);
   ...
   * test.
   *
   * However, when a test is run on multiple nodes, there is no guarantee that
   * all the testStarted() methods will be called before all the threadStart()
   * or sampleOccurred() methods for other threads - nor that testEnded() will
   * only be called after all sampleOccurred() calls. The ordering is only
   * guaranteed within a single test.
   *
   */
  ...
  /**
   * Called from a different thread as testStarted() but using the same
   * instance. So synch is needed to fetch the accumulator, and the myName field
   * will already be set up. <p>
   * {@inheritDoc}
   */
  @Override
  ...
  private static void
  formatAndWriteToLog(String name,
                      SummariserRunningSample summariserRunningSample,
                      String type) {
  ...
  private static String format(String name,
    ...
    if (elapsed > 0) {
      sb.append(
          doubleToSb(dfDouble, tmp, summariserRunningSample.getRate(), 6, 1));
    } else {
