...
public class TestPlan
    extends AbstractTestElement implements Serializable, TestStateListener {
  ...
  public TestPlan() { super(); }

  public TestPlan(String name) { setName(name); }
  ...
  @Override
  public PropertiesAccessor<? extends TestPlan, ? extends TestPlanSchema>
  getProps() {
  ...
  public void prepareForPreCompile() { getVariables().setRunningVersion(true); }
  ...
  public String getBasedir() { return getPropertyAsString(BASEDIR); }

  // Does not appear to be used yet
  public void setBasedir(String b) { setProperty(BASEDIR, b); }

  public Arguments getArguments() { return getVariables(); }
  ...
  private Arguments getVariables() {
    return getSchema().getUserDefinedVariables().getOrCreate(this,
                                                             Arguments::new);
  }
   ...
   * @return mode
   */
  public static boolean getFunctionalMode() { return functionalMode; }
  ...
  public void testStarted() {
    if (getBasedir() != null && getBasedir().length() > 0) {
      try {
        FileServer.getFileServer().setBasedir(
            FileServer.getFileServer().getBaseDir() + getBasedir());
      } catch (IllegalStateException e) {
        log.error("Failed to set file server base dir with {}", getBasedir(),
                  e);
      }
    }
