...
public class FileServer {
  ...
  private static final String BASE_PREFIX = JMeterUtils.getPropDefault(
      "jmeter.save.saveservice.base_prefix", // $NON-NLS-1$
      BASE_PREFIX_DEFAULT);
   ...
   */
  public static FileServer getFileServer() { return server; }
   ...
   * path. If the path does not refer to an existing directory, then its parent
   * is used. Normally the provided path is a file, so using the parent
  ...
  private void checkForOpenFiles() throws IllegalStateException {
    if (filesOpen()) { // checks for entries in use
      throw new IllegalStateException(
          "Files are still open, cannot change base directory");
    }
    files.clear(); // tidy up any unused entries
  }

  public synchronized String getBaseDir() { return base.getAbsolutePath(); }

  public static String getDefaultBase() { return DEFAULT_BASE; }
   ...
   *     null)
   */
  public void reserveFile(String filename) { reserveFile(filename, null); }
   ...
   *     null or empty
   */
  public synchronized String reserveFile(String filename, String charsetName,
                                         String alias, boolean hasHeader) {
    ...
    if (fileEntry == null) {
      fileEntry =
          new FileEntry(resolveFileFromPath(filename), null, charsetName);
      ...
      if (hasHeader) {
        try {
          ...
          if (fileEntry.headerLine == null) {
            fileEntry.exception =
                new EOFException("File is empty: " + fileEntry.file);
          }
    ...
    if (hasHeader && fileEntry.headerLine == null) {
      throw new IllegalArgumentException(
          "Could not read file header line for file " + filename,
          fileEntry.exception);
    }
   ...
   * @param ignoreFirstLine whether the file contains a file header which will
   *     be ignored
   ...
   */
  public synchronized String[] getParsedLine(String alias, boolean recycle,
                                             boolean ignoreFirstLine,
   ...
   */
  private BufferedReader getReader(String alias, boolean recycle,
  ...
  private static BufferedReader createBufferedReader(FileEntry fileEntry)
      throws IOException {
    if (!fileEntry.file.canRead() || !fileEntry.file.isFile()) {
      throw new IllegalArgumentException("File " + fileEntry.file.getName() +
  ...
  public synchronized void write(String filename, String value)
      throws IOException {
    FileEntry fileEntry = files.get(filename);
    if (fileEntry != null) {
      if (fileEntry.inputOutputObject == null) {
  ...
  private static BufferedWriter createBufferedWriter(FileEntry fileEntry)
      throws IOException {
    ...
    if (!JOrphanUtils.isBlank(charsetName)) {
      ...
      @SuppressWarnings("DefaultCharset")
      final OutputStreamWriter withPlatformEncoding =
  ...
  private static void closeFile(String name, FileEntry fileEntry)
      throws IOException {
  ...
  boolean filesOpen() { // package access for test code only
    return files.values().stream().anyMatch(
        fileEntry -> fileEntry.inputOutputObject != null);
  }
   ...
   * FileServer is for thread safe File access relative to current test's base
   * directory.
   *
   * @param basedir    name of the directory in which the files can be found
   * @param extensions array of allowed extensions, if <code>null</code> is
   *     given,
   *                   any file be allowed
