...
public class TestPlanGui extends AbstractJMeterGuiComponent {
  ...
  private final JBooleanPropertyEditor functionalMode =
      new JBooleanPropertyEditor(TestPlanSchema.INSTANCE.getFunctionalMode(),
  ...
  public TestPlanGui() {
    ...
    init();
    bindingGroup.addAll(
        Arrays.asList(functionalMode, serializedMode, tearDownOnShutdown));
  }
  ...
  public JPopupMenu createPopupMenu() {
    ...
    addMenu.add(
        MenuFactory.makeMenu(MenuFactory.CONFIG_ELEMENTS, ActionNames.ADD));
    ...
    addMenu.addSeparator();
    addMenu.add(
        MenuFactory.makeMenu(MenuFactory.PRE_PROCESSORS, ActionNames.ADD));
    addMenu.add(
        MenuFactory.makeMenu(MenuFactory.POST_PROCESSORS, ActionNames.ADD));
    ...
    addMenu.add(
        MenuFactory.makeMenu(MenuFactory.NON_TEST_ELEMENTS, ActionNames.ADD));
    pop.add(addMenu);
  ...
  private void init() { // WARNING: called from ctor so must not be overridden
    ...
    JComponent explain = new JLabel(
        JMeterUtils.getResString("functional_mode_explanation")); // $NON-NLS-1$
