...
public class WhileController
  ...
  private static final Logger log =
      LoggerFactory.getLogger(WhileController.class);

  private static final long serialVersionUID = 233L;

  private boolean breakLoop;

  public WhileController() { super(); }
  ...
  @Override
  public PropertiesAccessor<? extends WhileController, ?
                                extends WhileControllerSchema>
  getProps() {
  ...
  private boolean endOfLoop(boolean loopEnd) {
    ...
    if ((loopEnd && cnd.isEmpty()) ||
        "LAST".equalsIgnoreCase(cnd)) { // $NON-NLS-1$
      JMeterVariables threadVars =
          JMeterContextService.getContext().getVariables();
      res = "false".equalsIgnoreCase(
          threadVars.get(JMeterThread.LAST_SAMPLE_OK)); // $NON-NLS-1$
    } else {
  ...
  protected void resetLoopCount() { resetIterCount(); }
