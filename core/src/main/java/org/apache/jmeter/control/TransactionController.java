...
public class TransactionController extends GenericController
   ...
   */
  static final String NUMBER_OF_SAMPLES_IN_TRANSACTION_PREFIX =
   ...
   */
  public TransactionController() { lnf = new ListenerNotifier(); }
  ...
  @Override
  public PropertiesAccessor<? extends TransactionController, ?
                                extends TransactionControllerSchema>
  getProps() {
  ...
  protected Sampler nextIsAController(Controller controller)
    ...
    if (sampler == null) {
      ...
      // We need to call the super.next, instead of this.next, which is done in
      // GenericController, because if we call this.next(), it will return the
      // TransactionSampler, and we do not want that. We need to get the next
      // real sampler or controller
  ...
  private Sampler nextWithoutTransactionSampler() {
    ...
    if (returnValue == null && isLast) // Must be the end of the controller
    {
      if (res != null) {
        // See BUG 55816
        if (!isIncludeTimers()) {
          long processingTimeOfLastChild =
              res.currentTimeInMillis() - prevEndTime;
  ...
  public void triggerEndOfLoop() {
    if (!isGenerateParentSample()) {
      if (res != null) {
        ...
        res.sampleEnd();
        res.setSuccessful(
            TRUE.equals(JMeterContextService.getContext().getVariables().get(
  ...
  protected void notifyListeners() {
    ...
    SamplePackage pack =
        (SamplePackage)threadVars.getObject(JMeterThread.PACKAGE_OBJECT);
    if (pack == null) {
      // If child of TransactionController is a ThroughputController and TPC
      // does not sample its children, then we will have this
      // TODO Should this be at warn level ?
      log.warn("Could not fetch SamplePackage");
    } else {
      SampleEvent event = new SampleEvent(
          res, threadContext.getThreadGroup().getName(), threadVars, true);
      // We must set res to null now, before sending the event for the
      // transaction, so that we can ignore that event in our sampleOccurred
      // method
      res = null;
    ...
    if (!isGenerateParentSample()) {
      ...
      if (res != null && !se.isTransactionSampleEvent()) {
        ...
        if (!isIncludeTimers()) { // Accumulate waiting time for later
          pauseTime +=
              sampleResult.getEndTime() - sampleResult.getTime() - prevEndTime;
   ...
   * @param includeTimers Flag whether timers and pre/post processor should be
