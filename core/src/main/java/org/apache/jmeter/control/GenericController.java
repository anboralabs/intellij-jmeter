 ...
 * </p>
 */
public class GenericController extends AbstractTestElement
    implements Controller, Serializable, TestCompilerHelper {

  private static final long serialVersionUID = 235L;

  private static final Logger log =
      LoggerFactory.getLogger(GenericController.class);

  static final String INDEX_VAR_NAME_SUFFIX = "__idx";

  private transient Deque<LoopIterationListener> iterationListeners =
      new ArrayDeque<>();

  // Only create the map if it is required
  private transient IdentityHashMap<TestElement, Object> children =
      new IdentityHashMap<>();
  ...
  @Override
  public PropertiesAccessor<? extends GenericController, ?
                                extends GenericControllerSchema>
  getProps() {
   ...
   * <li>recoverRunningVersion() to set the controller back to the initial
   * state</li>
   * </ul>
   *
   */
   ...
   * #nextIsASampler(Sampler)}, otherwise calls {@link
   * #nextIsAController(Controller)}
   * </p>
   *
   * <p>
   * If any of the called methods throws {@link NextIsNullException}, returns
   * <code>null</code>, otherwise the value obtained above is returned.
   * </p>
   *
   ...
   */
  protected boolean isFirst() { return first; }

  /**
   * If b is true, it means first is reset which means Controller has executed
   * all its children
  ...
  protected Sampler nextIsAController(Controller controller)
      throws NextIsNullException { // NOSONAR false positive , throws is
  ...
  /**
   * Called by {@link #next()} when {@link #getCurrentElement()} returns
   * <code>null</code>. Reinitialises the controller.
   *
  ...
  /**
   * <p>
   * Gets the element indicated by the <code>current</code> index, if one
   * exists, from the <code>subControllersAndSamplers</code> list.
   * </p>
   * <p>
   ...
   */
  protected void incrementCurrent() { current++; }

  protected void resetCurrent() { current = 0; }
  ...
  protected int getIterCount() { return iterCount; }

  protected void incrementIterCount() { iterCount++; }

  protected void resetIterCount() { iterCount = 0; }
