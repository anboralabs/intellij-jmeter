 ...
 * reading CSV files.
 *
 * WARNING: the class assumes that column names are unique, but does not enforce
 * this.
 *
 */
public class Data implements Serializable {
   ...
   *            position to set to.
   */
  public void setCurrentPos(int r) { currentPos = r; }
   ...
   */
  public void sort(String column) { sortData(column, 0, size); }
   ...
   */
  public int size() { return size; } // end method
   ...
   */
  public boolean hasHeader(String column) { return data.containsKey(column); }
   ...
   */
  public boolean next() { return ++currentPos < size; }
   ...
   * @return True if there is another row. False if there are no more rows.
   */
  public boolean previous() { return --currentPos >= 0; }

  /**
   * Resets the current position of the data set to just before the first
   * element.
   */
  public void reset() { currentPos = -1; }
   ...
   *            these must be distinct - duplicates will cause incorrect
   * behaviour
   */
  ...
  public int getHeaderCount() { return header.size(); }
