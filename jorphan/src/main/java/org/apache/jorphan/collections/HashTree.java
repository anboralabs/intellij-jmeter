...
public class HashTree
   ...
   */
  protected HashTree(Map<Object, HashTree> _map) { this(_map, null); }
   ...
   */
  public HashTree(Object key) { this(new IdentityHashMap<>(), key); }
  ...
  private HashTree(Map<Object, ? extends HashTree> _map, Object key) {
    if (_map != null) {
      if (_map instanceof IdentityHashMap) {
        ...
        IdentityHashMap<Object, HashTree> identityMap =
            new IdentityHashMap<>(_map);
        data = identityMap;
      }
    } else {
   ...
   */
  public void add(Object key, HashTree subTree) { add(key).add(subTree); }
   ...
   */
  public void set(Object key, HashTree t) { data.put(key, t); }
   ...
   */
  public HashTree add(Object key, Object value) { return add(key).add(value); }
   ...
   *            array of objects to be added as keys in the secondary node
   */
  public void add(Object key, Object[] values) { add(key).add(values); }
   ...
   *            node
   */
  public void add(Object key, Collection<?> values) { add(key).add(values); }
   ...
   */
  public HashTree getTree(Object key) { return data.get(key); }
   ...
   * @return HashTree
   */
  protected HashTree createNewTree() { return new HashTree(); }
   ...
   */
  protected HashTree createNewTree(Object key) { return new HashTree(key); }
   ...
   */
  public Collection<Object> list() { return data.keySet(); }
   ...
   */
  public Object[] getArray() { return data.keySet().toArray(); }
  ...
  public int hashCode() {
    ...
    int sum = 0;
    int mul = 0;
    // Unordered hash function which uses reference equality for keys, and
    // object equality for values We use several commutative functions to reduce
    // the possibility of collisions. Note: IdentityHashMap.hashCode uses
    // reference equality for both keys and values, so we can't use it.
  ...
  private static class TreeSearcher implements HashTreeTraverser {
    ...
    public TreeSearcher(Object t) { target = t; }

    public HashTree getResult() { return result; }
