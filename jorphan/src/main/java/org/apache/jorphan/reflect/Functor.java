 ...
 * instance) to be omitted from the constructor. <p> The class name takes
 * precedence over the instance. <p> If a functor is created with a particular
 * instance, then that is used for all future calls; if an object is provided,
 * it is ignored. This allows easy override of the table model behaviour. <p> If
 * an argument list is provided in the constructor, then that is ignored in
 * subsequent invoke() calls. <p> Usage:
 *
 * <pre>
...
public class Functor {
   ...
   * it is set once by doCreateMethod(), which must be the only method to access
   * it.
   */
  ...
  private Functor(Class<?> _clazz, Object _invokee, String _methodName,
    ...
    if (_clazz != null && _invokee != null) {
      throw new IllegalArgumentException(
          "Cannot provide both Class and Object");
    }
    if (_args != null && _types != null) {
      throw new IllegalArgumentException(
          "Cannot provide both arguments and argument types");
    }
    // If class not provided, default to invokee class, else null
    this.clazz = _clazz != null
                     ? _clazz
                     : (_invokee != null ? _invokee.getClass() : null);
    this.invokee = _invokee;
    this.methodName = _methodName;
    this.args = _args;
    // If types not provided, default to argument types, else null
    this.types =
        _types != null ? _types : (_args != null ? _getTypes(_args) : null);
  }
  ...
  private Object doInvoke(Class<?> _class, Object _invokee, Object[] _args) {
    ...
    try {
      ...
      if (method == null) {
        final String message = "Can't find method " + _class.getName() + "#" +
                               methodName + typesToString(argTypes);
        log.error(message, new Throwable());
        throw new JMeterError(message);
      }
      return method.invoke(_invokee, _args);
    } catch (Exception e) {
      final String message = "Trouble functing: " + _class.getName() + "." +
                             methodName + "(...) : "
                             + " invokee: " + _invokee + " " + e.getMessage();
      log.warn(message, e);
  ...
  /**
   * Invoke a Functor, which must have been created with either a class name or
   * object.
   *
  ...
  public Object invoke() {
    if (invokee == null) {
      throw new IllegalStateException(
          "Cannot call invoke() - invokee not known");
    }
  ...
  public Object invoke(Object[] p_args) {
    if (invokee == null) {
      throw new IllegalStateException(
          "Invokee was not provided in constructor");
    }
    // If invokee was provided, then clazz has been set up
   ...
   * time.
   *
   * If the invokee was provided at construction time, then this invokee will be
   * ignored. If actual arguments were provided at construction time, then
  ...
  public Object invoke(Object p_invokee, Object[] p_args) {
    return doInvoke(
        clazz != null
            ? clazz
            : p_invokee.getClass(), // Use constructor class if present
   ...
   * defined. Synchronized to protect access to methodToInvoke.
   */
  private synchronized Method doCreateMethod(Class<?> p_class,
                                             Class<?>[] p_types) {
    if (log.isDebugEnabled()) {
      log.debug("doCreateMethod() using " + this.toString() + "class=" +
                p_class.getName() + " types: " + Arrays.asList(p_types));
    }
      ...
      try {
        ...
        for (int i = 0; i < p_types.length; i++) {
          ...
          for (Class<?> anInterface : interfaces) {
            methodToInvoke =
                doCreateMethod(p_class, getNewArray(i, anInterface, p_types));
            if (methodToInvoke != null) {
              return methodToInvoke;
            }
          }
  ...
  private static Class<?>[] getNewArray(int i, Class<?> replacement,
                                        Class<?>[] orig) {
