...
public final class ClassFinder {
  ...
  private static final ThreadLocal<Boolean>
      SKIP_JARS_WITH_JMETER_SKIP_ATTRIBUTE = new ThreadLocal<>();

  public static final String JMETER_SKIP_CLASS_SCANNING_ATTRIBUTE =
      "JMeter-Skip-Class-Scanning";
  ...
  public static boolean getSkipJarsWithJmeterSkipClassScanningAttribute() {
    return Objects.equals(SKIP_JARS_WITH_JMETER_SKIP_ATTRIBUTE.get(),
                          Boolean.TRUE);
  }

  /**
   * Configures if {@link ClassFinder} should skip jar files that have {@code
   * JMeter-Skip-Class-Scanning: true} manifest attribute. JMeter will skip such
   * jars when it uses both {@link ServiceLoader} and {@link ClassFinder}.
   * However, {@link ClassFinder} was public, so it was possible that custom
   * plugins could use it, and they should be able to find the implementations
   * even if they are in jars with {@code JMeter-Skip-Class-Scanning: true}. <p>
   * Sample usage:
   * <pre>
   * List&lt;String&gt; classNames;
   * try (ClassFinder.Closeable ignored =
   * ClassFinder.skipJarsWithJmeterSkipClassScanningAttribute()) {
   *   // findClassesThatExtend will not skip jars with
   * JMeter-Skip-Class-Scanning: true manifest attribute classNames =
   * ClassFinder.findClassesThatExtend(...);
   * </pre>
   *
   * @return closeable that will reset "skip jar files with manifest entry" flag
   *     when closed. Use it in try-with-resources
   */
   ...
   */
  public static <S> Collection<S>
  loadServices(@SuppressWarnings("BoundedWildcard") Class<S> service,
    ...
    while (it.hasNext()) {
      try {
        // This can't be for-each loop because we need to catch exceptions from
        // next()
        result.add(it.next());
      } catch (ServiceConfigurationError e) {
        // Java does not expose class name of the problematic class in question,
        // so we extract it from the message
        ...
        if (message.startsWith(service.getName())) {
          if (message.endsWith(" Unable to get public no-arg constructor")) {
            className = message.substring(
                ...
                message.length() -
                    " Unable to get public no-arg constructor".length());
          } else if (message.endsWith(" not a subtype")) {
            className =
                message.substring(service.getName().length() + ": ".length(),
  ...
  private static class ExtendsClassFilter implements ClassFilter {
    ...
    private static boolean isChildOf(Class<?>[] parentClasses,
      ...
      try {
        Class<?> targetClass =
            Class.forName(strClassName, false, contextClassLoader);
  ...
  private static class AnnoClassFilter implements ClassFilter {
    ...
    private final Class<? extends Annotation>[]
        annotations; // annotation classes to check
    private final ClassLoader contextClassLoader =
        Thread.currentThread()
            .getContextClassLoader(); // Potentially expensive; do it once
    ...
    public boolean accept(String className) {
      if (!className.contains("$") || inner) { // $NON-NLS-1$
        return hasAnnotationOnMethod(annotations, className,
                                     contextClassLoader);
      }
      return false;
    }
    ...
    hasAnnotationOnMethod(Class<? extends Annotation>[] annotations,
      ...
      try {
        ...
        return Arrays.stream(c.getMethods())
            .anyMatch(method
               ...
               UnsupportedClassVersionError | VerifyError ignored) {
   ...
   * @deprecated use {@link #loadServices(Class, ServiceLoader,
   *     ServiceLoadExceptionHandler)} or {@code
  ...
  private static Set<File> addJarsInPath(String[] paths) {
    ...
    for (final String path : paths) {
      ...
      if (dir.exists() && dir.isDirectory()) {
        File[] jars =
            dir.listFiles(f -> f.isFile() && f.getName().endsWith(DOT_JAR));
        if (jars != null) {
  ...
  findClassesThatExtend(String[] strPathsOrJars, final Class<?>[] superClasses,
                        final boolean innerClasses) throws IOException {
    return findClassesThatExtend(strPathsOrJars, superClasses, innerClasses,
                                 null, null);
  }
  ...
  public static List<String>
  findClassesThatExtend(String[] strPathsOrJars, final Class<?>[] superClasses,
                        final boolean innerClasses, String contains,
  ...
  findAnnotatedClasses(String[] strPathsOrJars,
                       ...
                       final boolean innerClasses) throws IOException {
    return findClassesThatExtend(strPathsOrJars, annotations, innerClasses,
                                 null, null, true);
  }
  ...
  findAnnotatedClasses(String[] strPathsOrJars,
                       final Class<? extends Annotation>[] annotations)
      throws IOException {
    return findClassesThatExtend(strPathsOrJars, annotations, false, null, null,
                                 true);
  }
  ...
  @Deprecated
  public static List<String>
  findClassesThatExtend(String[] searchPathsOrJars, final Class<?>[] classNames,
                        final boolean innerClasses, String contains,
                        String notContains, boolean annotations)
      throws IOException {
    if (log.isDebugEnabled()) {
      log.debug(
          "findClassesThatExtend with searchPathsOrJars : {}, superclass : {}"
              +
              " innerClasses : {} annotations: {} contains: {}, notContains: {}",
          Arrays.toString(searchPathsOrJars), Arrays.toString(classNames),
          innerClasses, annotations, contains, notContains);
    }
    ...
    if (annotations) {
      ...
      final Class<? extends Annotation>[] annoclassNames =
          (Class<? extends Annotation>[])classNames;
      filter = new AnnoClassFilter(annoclassNames, innerClasses);
    } else {
      filter = new ExtendsClassFilter(classNames, innerClasses, contains,
                                      notContains);
    }
   ...
   * @param filter            {@link ClassFilter} that the classes in the jars
   *     should
   *                          conform to
  ...
  public static List<String> findClasses(String[] searchPathsOrJars,
    ...
    // Some of the jars might be out of classpath, however java.class.path does
    // not represent the actual ClassLoader in use. For instance, NewDriver
    // builds its own classpath
  ...
  private static String fixClassName(String strClassName) {
    String fixedClassName =
        strClassName.replace('\\', '.'); // $NON-NLS-1$ // $NON-NLS-2$
    fixedClassName =
        fixedClassName.replace('/', '.'); // $NON-NLS-1$ // $NON-NLS-2$
    // remove ".class"
    fixedClassName =
        fixedClassName.substring(0, fixedClassName.length() - DOT_CLASS_LEN);
    return fixedClassName;
  }

  private static void findClassesInOnePath(File file,
                                           Set<? super String> listClasses,
                                           ClassFilter filter) {
    if (file.isDirectory()) {
      findClassesInPathsDir(file.getAbsolutePath(), file, listClasses, filter);
    } else if (file.exists()) {
      if (getSkipJarsWithJmeterSkipClassScanningAttribute() &&
          file.getName().endsWith(DOT_JAR)) {
        // Ignore jars with JMeter-Skip-Class-Scanning attribute
        try (JarFile jar = new JarFile(file)) {
          String value = jar.getManifest().getMainAttributes().getValue(
      ...
      try (ZipFile zipFile = new ZipFile(file);
        ...
        log.warn("Can not open the jar {}, message: {}", file.getAbsolutePath(),
  ...
  private static void findClassesInPathsDir(String strPathElement, File dir,
    ...
    for (File file : list) {
      ...
      } else if (file.getPath().endsWith(DOT_CLASS) && file.exists() &&
                 (file.length() != 0)) {
        final String path = file.getPath();
        String className =
            path.substring(strPathElement.length() + 1,
  ...
  /**
   * Run {@link ClassFilter#accept(String)} on className and add to listClasses
   * if accept returns true In case of Throwable, className will not be added
