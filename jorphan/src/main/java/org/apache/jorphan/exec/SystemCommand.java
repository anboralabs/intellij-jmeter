...
public class SystemCommand {
  /**
   * @deprecated does not make sense anymore. Superseded by {@link
   *     Process#wait(long)}
   */
  @Deprecated public static final int POLL_INTERVAL = 100;
  ...
  public SystemCommand(File directory, Map<String, String> env) {
    this(directory, 0L, 0, env, (InputStream)null, (OutputStream)null,
         (OutputStream)null);
  }
   ...
   * @param stdin File name that will contain data to be input to process (may
   *     be null)
   ...
   */
  public SystemCommand(File directory, long timeoutMillis, int pollInterval,
                       Map<String, String> env, InputStream stdin,
  ...
  private int run(List<String> arguments, InputStream in, OutputStream out,
    ...
    this.executionEnvironment =
        Collections.unmodifiableMap(procBuild.environment());
    ...
    try {
      ...
      if (in != null) {
        ...
        swin = null;
        procOut.close(); // ensure the application does not hang if it requests
                         // input
      }
   ...
   * @throws IOException when I/O error occurs while execution
   */
  public int run(List<String> arguments1, List<String> arguments2)
      throws InterruptedException, IOException, TimeoutException {
    ByteArrayOutputStream out =
        new ByteArrayOutputStream(); // capture the intermediate output
    int exitCode = run(arguments1, stdin, out, stderr);
    if (exitCode == 0) {
      exitCode = run(arguments2, new ByteArrayInputStream(out.toByteArray()),
                     stdout, stderr);
    }
    return exitCode;
  }
  ...
  /**
   * Wait for end of proc execution or timeout if timeoutInMillis is greater
   * than 0
   * @param proc Process
   ...
   * @throws TimeoutException when timeout is reached while execution
   */
  private static int waitForEndWithTimeout(Process proc, long timeoutInMillis)
    ...
    throw new TimeoutException("Process timeout reached after " +
