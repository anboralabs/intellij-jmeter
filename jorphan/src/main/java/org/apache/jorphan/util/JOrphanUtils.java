...
public final class JOrphanUtils {
  ...
  /**
   * This is <em>almost</em> equivalent to the {@link String#split(String)}
   * method in JDK 1.4. It is here to enable us to support earlier JDKs. <p>
   * Note that unlike JDK1.4 split(), it optionally ignores leading split
   * Characters, and the splitChar parameter is not a Regular expression <p>
   ...
   * @param splitChar Character(s) to split the string on, these are treated as
   *     a single unit
   * @param truncate  Should adjacent and leading/trailing splitChars be
   *     removed?
   * @return Array of all the tokens; empty if the input string is {@code null}
   *     or the splitChar is {@code null}
  ...
  public static String[] split(String splittee, String splitChar,
    ...
    if (truncate) {
      while ((spot = splittee.indexOf(adjacentSplit)) != -1) {
        splittee =
            splittee.substring(0, spot + splitLength) +
   ...
   *                 nothing between them. If null, then ignore omitted
   * elements.
   ...
   * @return the output string
   */
  public static String replaceAllChars(String source, char search,
                                       String replace) {
   ...
   */
  public static String substitute(final String input, final String pattern,
                                  final String sub) {
  ...
  // N.B. Commons IO IOUtils has equivalent methods; these were added before IO
  // was included
   ...
   * Mainly designed for use at the end of a non-GUI test to check for threads
   * that might prevent the JVM from exiting.
   *
  ...
  /**
   * Returns {@code null} if input is empty, {@code null} or contains spaces
   * only
   *
   * @param input String
   ...
   *
   * @param value Value
   * @return {@code true} if the String is not empty (""), not {@code null} and
   *     not whitespace only.
   */
  ...
  public static String formatDuration(long elapsedSec) {
    return String.format("%02d:%02d:%02d", elapsedSec / 3600,
                         (elapsedSec % 3600) / 60, elapsedSec % 60);
  }
   ...
   * either: <ul> <li>Because it exists but is not a folder</li> <li>Because it
   * exists but is not empty</li> <li>Because it does not exist but cannot be
   * created</li>
   * </ul>
   *
  ...
  /**
   * Check whether we can write to a folder.
   * A folder can be written to if folder.listFiles(exporterFileFilter) does not
   * return any file or folder. Throw {@link IllegalArgumentException} if folder
   * cannot be written to either: <ul> <li>Because it exists but is not a
   * folder</li> <li>Because it exists but is not empty using
   * folder.listFiles(exporterFileFilter)</li> <li>Because it does not exist but
  ...
  /**
   * Check whether we can write to a folder. If {@code deleteFolderContent} is
   * {@code true} the folder or file with the same name will be emptied or
   * deleted.
   *
   * @param folder              to check
   * @param deleteFolderContent flag whether the folder should be emptied or a
   *     file with the same name deleted
   * @throws IllegalArgumentException when folder can't be written to
   *                                  Throw IllegalArgumentException if folder
   * cannot be written
   */
  public static void canSafelyWriteToFolder(File folder,
   ...
   * Check whether we can write to a folder.
   *
   * @param folder               which should be checked for writability and
   *     emptiness
   * @param deleteFolderIfExists flag whether the folder should be emptied or a
   *     file with the same name deleted
   ...
   *                                   <li>it does not exist but cannot be
   * created</li>
   *                                  </ul>
   */
  public static void canSafelyWriteToFolder(File folder,
        ...
        if (deleteFolderIfExists) {
          if (!folder.delete()) {
            throw new IllegalArgumentException(
                "Cannot write to '" + folder.getAbsolutePath() +
                "' as it is an existing file and delete failed");
          }
        } else {
          throw new IllegalArgumentException("Cannot write to '" +
                                             folder.getAbsolutePath() +
                                             "' as it is an existing file");
        }
      } else {
        ...
        if (listedFiles != null && listedFiles.length > 0) {
          if (deleteFolderIfExists) {
            try {
              ...
              throw new IllegalArgumentException(
                  "Cannot write to '" + folder.getAbsolutePath() +
                      "' as folder is not empty and cleanup failed with error:" +
                      ex.getMessage(),
                  ex);
            }
            if (!folder.mkdir()) {
   ...
   * @return array of Object where first row is the replaced text, second row is
   *     the number of replacement that occurred
   */
  @SuppressWarnings("JdkObsolete")
  public static Object[] replaceAllWithRegex(String source, String regex,
                                             String replacement,
                                             boolean caseSensitive) {
    java.util.regex.Pattern pattern =
        caseSensitive ? java.util.regex.Pattern.compile(regex)
    ...
    StringBuffer result =
        new StringBuffer(); // NOSONAR Matcher#appendReplacement needs a
                            // StringBuffer
    while (matcher.find()) {
  ...
  /**
   * Replace all occurrences of {@code regex} in {@code value} by {@code
   * replaceBy} if {@code value} is not blank. The replaced text is fed into the
   * {@code setter}.
   *
   * @param regex         Regular expression that is used for the search
   * @param replaceBy     value that is used for replacement
   * @param caseSensitive flag whether the regex should be applied case
   *     sensitive
   ...
   */
  public static int replaceValue(String regex, String replaceBy,
                                 boolean caseSensitive, String value,
                                 Consumer<? super String> setter) {
    if (StringUtils.isBlank(value)) {
      return 0;
    }
    Object[] result =
        replaceAllWithRegex(value, regex, replaceBy, caseSensitive);
  ...
  public static String generateRandomAlphanumericPassword(int length) {
    ...
    RandomStringGenerator pwdGenerator =
