...
public class StringWrap {
  ...
  private final BreakCursor wordCursor =
      new BreakCursor(BreakIterator.getLineInstance());
  private final BreakCursor charCursor =
      new BreakCursor(BreakIterator.getCharacterInstance());
  ...
  private static class BreakCursor {
    ...
    BreakCursor(BreakIterator iterator) { this.iterator = iterator; }
     ...
     * @return true if the next break is detected within startWrap..endWrap
     *     boundaries
     */
  ...
  public int getMinWrap() { return minWrap; }

  public int getMaxWrap() { return maxWrap; }
   ...
   * @return modified text with added soft wraps, or input if wraps are not
   *     needed
   */
  public String wrap(String input, String delimiter) {
    ...
    StringBuilder sb = new StringBuilder(
        input.length() + input.length() / minWrap * delimiter.length());
    ...
    while (input.length() - pos > maxWrap) {
      ...
      if (wordCursor.advance(startWrap, endWrap)) {
        ...
        // If char advances at least once, add it with the break even if it
        // exceeds maxWrap Note: single "char break" might consume multiple Java
        // chars in case like emojis.
