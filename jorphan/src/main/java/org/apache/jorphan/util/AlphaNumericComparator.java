...
/**
 * Comparator for Objects, that compares based on their <em>converted</em>
 * values. The objects will be converted to a String value using the given
 * {@link Function}. That value will be compared in a human readable fashion by
 * trying to parse numbers that appear in the keys as integers and compare
 * those, too.<p> Heavily influenced by
...
public class AlphaNumericComparator<T> implements Comparator<T> {
  public static final Comparator<?> TO_STRING_COMPARATOR =
      new AlphaNumericComparator<>(Object::toString);

  private final Function<? super T, String> converter;

  /**
   * Constructs a comparator with a converter function
   * @param converter that generates a String value from the arguments given to
  ...
  public int compare(T o1, T o2) {
    ...
    while (m1.find() && m2.find()) {
      int compareCharGroup =
          m1.group(ALPHA_PART).compareTo(m2.group(ALPHA_PART));
  ...
  private static int compareOneEmptyPart(String numberPart1,
                                         String numberPart2) {
    ...
    throw new IllegalArgumentException(
        "At least one of the parameters have to be empty");
  }
