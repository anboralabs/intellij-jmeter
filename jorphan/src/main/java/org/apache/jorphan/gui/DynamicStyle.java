 ...
 * <p>{@code DynamicStyle} enables to augment the components (e.g. border, font,
 * color) as LaF changes</p>
 */
...
public class DynamicStyle {
   ...
   */
  @API(since = "5.3", status = API.Status.EXPERIMENTAL)
  public <T extends JComponent> T withDynamic(T component,
                                              Consumer<? super T> onUpdateUi) {
  ...
  public <T extends JComponent> T withBorder(T component, String resource) {
    return withDynamic(component,
                       c -> { c.setBorder(UIManager.getBorder(resource)); });
  }

  @API(since = "5.3", status = API.Status.EXPERIMENTAL)
  public <T extends JComponent> T withFont(T component, String resource) {
    return withDynamic(component,
                       c -> { c.setFont(UIManager.getFont(resource)); });
  }
  ...
  public void updateLaf(String className) {
    try {
      ...
      // 1) Nimbus LaF caches styles (e.g. Label.font), so if we want apply new
      // zoom scale,
      //    then we need to invalidate the cache
      // 2) We need to generate "property changed lookAndFeel" UI event.
      // Unfortunately,
      ...
      throw new IllegalStateException(
          "Unable to update look and feel to " + className, e); // $NON-NLS-1$
    }
  ...
  private void updateComponentTreeUI(Component root,
  ...
  private static void collectComponents(Component root,
                                        List<? super Component> components) {
    if (root == null) {
    ...
    if (root instanceof JComponent) {
      ...
      // Note: updateUI might alter the component tree, so we should call it
      // before we try iterating over the children
      jc.updateUI();
