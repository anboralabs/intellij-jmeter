...
public class ObjectTableModel extends DefaultTableModel {
  private static final Logger log =
      LoggerFactory.getLogger(ObjectTableModel.class);
   ...
   */
  public ObjectTableModel(String[] headers, Class<?> _objClass,
   ...
   * @param writeFunctors - used to set the values
   * @param editorClasses - class for each column
   * @param cellEditable - if cell must editable (false to allow double click on
   *     cell)
   */
  public ObjectTableModel(String[] headers, Class<?> _objClass,
                          Functor[] readFunctors, Functor[] writeFunctors,
                          Class<?>[] editorClasses, boolean cellEditable) {
    this(headers, readFunctors, writeFunctors, editorClasses);
    this.objectClass = _objClass;
    ...
    if (objectClass != null) {
      ...
      if (!objectClass.isAssignableFrom(valueClass)) {
        throw new IllegalArgumentException(
            "Trying to add class: " + valueClass.getName() +
  ...
  public boolean checkFunctors(Object _value, Class<?> caller) {
    ...
    if (_value == null && objectClass != null) {
      try {
        ...
        log.error("Cannot create instance of class {}", objectClass.getName(),
                  e);
        return false;
      }
    } else {
    ...
    for (int i = 0; i < getColumnCount(); i++) {
      ...
      if (setMethod != null &&
        ...
        status = false;
        log.warn("{} is attempting to use nonexistent {}", caller.getName(),
                 setMethod);
      }
   ...
   */
  public List<Object> getObjectListAsList() { return objects; }
