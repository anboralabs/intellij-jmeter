...
public class ObjectTableSorter extends RowSorter<ObjectTableModel> {
  ...
  public class Row {
    ...
    protected Row(int index) { this.index = index; }
   ...
   */
  public Comparator<Row> getPrimaryComparator() { return primaryComparator; }
  ...
  /**
   * Returns Comparator if all sorted columns matches. Defaults to model index
   * comparison.
   * @return Comparator if all sorted columns matches. Defaults to model index
   *     comparison.
   */
  public Comparator<Row> getFallbackComparator() { return fallbackComparator; }
   ...
   */
  public ObjectTableSorter
  setPrimaryComparator(Comparator<Row> primaryComparator) {
    invalidate();
  ...
  /**
   * Sets {@link #getPrimaryComparator() primary comparator} to one that don't
   * sort last row.
   * @return <code>this</code>
   */
  ...
  /**
   * Assign comparator to given column, if <code>null</code> a
   * getDefaultComparator(int) default one is used instead.
   * @param column Model column index.
   * @param comparator Column value comparator.
   * @return <code>this</code>
   */
  public ObjectTableSorter setValueComparator(int column,
                                              Comparator<?> comparator) {
    invalidate();
    valueComparators[column] =
        ObjectUtils.defaultIfNull(comparator, getDefaultComparator(column));
    return this;
  }

  /**
   * Builds a default comparator based on model column class. {@link
   * Collator#getInstance()} for {@link String},
   * {@link Comparator#naturalOrder() natural order} for {@link Comparable}, no
   * sort support for others.
   * @param column Model column index.
  ...
  public ObjectTableSorter setFallbackComparator(Comparator<Row> comparator) {
    invalidate();
    fallbackComparator = ObjectUtils.defaultIfNull(
    ...
    if (sortkey != null) {
      ...
      if (valueComparators[column] == null) {
        throw new IllegalArgumentException(format(
            "Can't sort column %s, it is mapped to type %s and this one have no natural order. So an explicit one must be specified",
  ...
  public List<? extends SortKey> getSortKeys() {
    return isSorted() ? Collections.singletonList(sortkey)
  ...
  protected void rowsChanged(int firstRow, int endRow, boolean deleted,
                             boolean inserted) {
    invalidate();
  }
  ...
  protected boolean isSorted(int column) {
    return isSorted() && sortkey.getColumn() == column &&
        sortkey.getSortOrder() != SortOrder.UNSORTED;
  }

  protected boolean isSorted() { return sortkey != null; }
  ...
  protected Comparator<Row> getComparatorFromSortKey(SortKey sortkey) {
    ...
    Function<Row, Object> getValueAt =
        (Row row) -> row.getValueAt(sortkey.getColumn());
      ...
      comparator =
          Stream
              .concat(Stream.concat(
                          getPrimaryComparator() != null
                              ? Stream.of(getPrimaryComparator())
                              : Stream.<Comparator<Row>>empty(),
                          getSortKeys()
                              .stream()
                              .filter(sk
                                      -> sk != null && sk.getSortOrder() !=
                                                           SortOrder.UNSORTED)
                              .map(this::getComparatorFromSortKey)),
                      Stream.of(getFallbackComparator()))
              .reduce(comparator,
                      (result, current)
                          -> result != null ? result.thenComparing(current)
                                            : current);
    }
