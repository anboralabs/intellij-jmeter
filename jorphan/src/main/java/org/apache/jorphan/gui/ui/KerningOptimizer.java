...
/**
 * Text rendering might be slow for long lines when kerning is enabled, so it is
 * worth disabling kerning for long texts.
 */
@API(since = "5.5", status = API.Status.INTERNAL)
public class KerningOptimizer {
  private final static Logger log =
      LoggerFactory.getLogger(KerningOptimizer.class);
  ...
  /**
   * Cache to avoid repeated calls to {@link Font#getAttributes()} since it
   * copies the map every time.
   */
  static class FontKerningCache {
    private static final String CLIENT_PROPERTY_KEY =
        "[jmeter]FontKerningCache";
    ...
    static Boolean kerningOf(JComponent component) {
      ...
      FontKerningCache cache =
          (FontKerningCache)component.getClientProperty(CLIENT_PROPERTY_KEY);
      if (cache == null) {
      ...
      if (cache.font == null || !font.equals(cache.font.get())) {
        ...
        cache.kerning = TextAttribute.KERNING_ON.equals(
            font.getAttributes().get(TextAttribute.KERNING));
      }
  ...
  public int getMaxTextLengthWithKerning() { return maxLengthWithKerning; }

  /**
   * Configures text kerning according to the expected document length. This
   * might be useful before setting the document so the kerning is disabled
  ...
  public void configureKerning(JComponent component, int documentLength) {
    ...
    if (kerning != desiredKerning) {
      if (log.isDebugEnabled()) {
        log.info(
            "Updating kerning (old: {}, new: {}), documentLength={}, component {}, ",
            kerning, desiredKerning, documentLength, component);
      }
      Font font = component.getFont();
      Font newFont = font.deriveFont(Collections.singletonMap(
          TextAttribute.KERNING,
          desiredKerning ? TextAttribute.KERNING_ON : 0));
      SwingUtilities.invokeLater(() -> component.setFont(newFont));
    }
  }

  /**
   * Adds a listener that disables kerning if text length reaches a certain
   * threshold.
   *
  ...
  public void installKerningListener(JTextComponent textComponent) {
    ...
    textComponent.addPropertyChangeListener(
        "document", new DisableKerningForLargeTexts(textComponent));
  }

  /**
   * Removes the listener that disables kerning if text length reaches a certain
   * threshold.
   *
   * @param textComponent text component for kerning configuration
   */
  public void uninstallKerningListener(JTextComponent textComponent) {
    ...
    for (PropertyChangeListener listener :
         textComponent.getPropertyChangeListeners("document")) {
  ...
  static class DisableKerningForLargeTexts
    ...
    private void configureKerning(Document e) {
      // RSyntaxTextArea and other implementations do not expect setFont called
      // from document change listeners It looks like invokeLater fixes that
      Boolean kerning = FontKerningCache.kerningOf(component);
      if (kerning == null) {
        return;
      }
      boolean desiredKerning =
          e.getLength() <= INSTANCE.getMaxTextLengthWithKerning();
      if (kerning != desiredKerning) {
        SwingUtilities.invokeLater(
            () -> INSTANCE.configureKerning(component, e.getLength()));
      }
    }
