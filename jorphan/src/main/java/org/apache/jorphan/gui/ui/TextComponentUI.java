...
public class TextComponentUI {
  ...
  public void installUndo(JTextComponent component) {
    // JMeter reuses Swing JComponents, so when user switches to another
    // component, JComponent#name is updated. However, we don't want user to be
    // able to "undo" that So when tree selection is changed, we increase
    // undoEpoch. That enables UndoManagers to treat that as "end of undo
    // history"
    UndoManager manager = new DefaultUndoManager(undoEpoch);
    manager.setLimit(200);
    component.addPropertyChangeListener(
        "document", new AddUndoableEditListenerPropertyChangeListener(manager));
    ...
    KeyStroke shiftCommandZ = KeyStroke.getKeyStroke(
        KeyEvent.VK_Z, COMMAND_KEY | InputEvent.SHIFT_DOWN_MASK);
    component.getInputMap().put(shiftCommandZ, "redo");
  }

  /**
   * Removes the default undo manager.
   * By default, JMeter installs undo manager to all text fields via {@code
   * Swing -> createUI}, however, undo is not always needed (e.g. log panel), so
   * here's an API to remove it.
   * @param component JTextField or JTextArea
   */
  ...
  public static void uninstallUndo(JTextComponent component) {
    ...
    for (PropertyChangeListener listener :
         component.getPropertyChangeListeners("document")) {
