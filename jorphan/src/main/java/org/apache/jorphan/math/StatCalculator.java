 ...
 * of values. It is not threadsafe.
 *
...
public abstract class StatCalculator<T extends Number & Comparable<? super T>> {
  ...
  /**
   * This constructor is used to set up particular values for the generic class
   * instance.
   *
   * @param zero - value to return for Median and PercentPoint if there are no
   *     values
   ...
   */
  public void addBytes(long newValue) { bytes += newValue; }

  /**
   * Add to sent bytes
   * @param newValue number of newly sent bytes
   */
  public void addSentBytes(long newValue) { sentBytes += newValue; }
  ...
  public T getMedian() { return getPercentPoint(0.5); }

  public long getTotalBytes() { return bytes; }

  public long getTotalSentBytes() { return sentBytes; }
   ...
   *     array containing the key and the [Integer] count.
  ...
  public double getStandardDeviation() { return deviation; }
   ...
   */
  public void addValue(T val) { addValue(val, 1L); }
