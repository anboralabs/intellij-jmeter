...
public final class CLArgsParser {
   ...
   */
  public final String[] getUnparsedArgs() { return this.unparsedArgs; }
   ...
   */
  public final List<CLOption> getArguments() { return this.options; }
   ...
   */
  public final String getErrorString() { return this.errorMessage; }
  ...
  private void checkIncompatible(final List<CLOption> arguments,
                                 final int[] incompatible, final int original)
      throws ParseException {
    final int size = arguments.size();
    ...
    for (int i = 0; i < size; i++) {
      ...
      for (int anIncompatible : incompatible) {
        if (id == anIncompatible) {
          ...
          if (id == originalId) {
            message = "Duplicate options for " +
                      describeDualOption(originalId) + " found.";
          } else {
            message = "Incompatible options -" + describeDualOption(id) +
                      " and " + describeDualOption(originalId) + " found.";
          }
   ...
   */
  private static String[] subArray(final String[] array, final int index,
                                   final int charIndex) {
  ...
  private void parse() throws ParseException {
    ...
    while (true) {
      ...
      if (null != this.control && this.control.isFinished(this.lastOptionId)) {
        ...
        this.unparsedArgs =
            subArray(this.args, this.argIndex, this.stringIndex);
        return;
      }
    ...
    if (this.option != null) {
      if (STATE_OPTIONAL_ARG == this.state) {
        ...
        final CLOptionDescriptor descriptor =
            getDescriptorFor(this.option.getDescriptor().getId());
        ...
        throw new ParseException(
            "IllegalState " + this.state + ": " + this.option, 0);
      }
    }
  }
  ...
  private void parseOption(final CLOptionDescriptor descriptor,
                           final String optionString) throws ParseException {
    if (null == descriptor) {
  ...
  private void parseArguments() throws ParseException {
    if (STATE_REQUIRE_ARG == this.state) {
      ...
      if (this.isLong &&
          '=' != this.tokesep) { // Long optional arg must have = as separator
        addOption(this.option);
        this.state = STATE_NORMAL;
        return;
      }
