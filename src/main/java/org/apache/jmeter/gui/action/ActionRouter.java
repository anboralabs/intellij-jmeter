...
public final class ActionRouter implements ActionListener {
  ...
  // New action will clear undo, no point in having a transaction. Same with
  // open EMI: XXX: Commands could also have an annotation to sigal the Undo
  // preference
  private final List<String> NO_TRANSACTION_ACTIONS = Arrays.asList(
      ActionNames.CLOSE, ActionNames.OPEN, ActionNames.OPEN_RECENT);
    ...
    try {
      try {
        ...
        JMeterUtils.reportErrorToUser(
            "Problem updating GUI - see log file for details");
      }
      if (!commands.isEmpty()) {
   ...
   */
  public void doActionNow(ActionEvent e) { performAction(e); }
   ...
   */
  private static void removeActionListener(
      Class<?> action, ActionListener listener,
      Map<? super String, Set<ActionListener>> actionListeners) {
    if (action != null) {
   ...
   */
  private static void
  addActionListener(Class<?> action, ActionListener listener,
                    Map<? super String, Set<ActionListener>> actionListeners) {
    if (action != null) {
      Set<ActionListener> set = actionListeners.get(action.getName());
      if (set == null) {
   ...
   */
  private static void
  actionPerformed(Class<? extends Command> action, ActionEvent e,
                  Map<String, ? extends Set<ActionListener>> actionListeners) {
    if (action != null) {
   ...
   * See <a href="https://bz.apache.org/bugzilla/show_bug.cgi?id=58790">Bug
   * 58790</a>
   */
   ...
   */
  public static ActionRouter getInstance() { return INSTANCE; }
}
