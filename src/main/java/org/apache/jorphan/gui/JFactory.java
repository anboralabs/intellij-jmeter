 ...
 * <p>The class provides APIs to configure Swing components so the look is
 * consistent, and it updates the components when Look and Feel changes</p>
 * <p>Swing API provides no standard components and colors for: small, big
 * components; warning, error styles for labels; and so on</p> <p>Note: by
 * default {@link JTable} comes with fixed {@code rowHeight} which does not work
 * when the fonts are scaled. So you need to call {@link
 * #singleLineRowHeight(JTable)} or configure {@code rowHeight} manually</p>
 */
@API(since = "5.3", status = API.Status.EXPERIMENTAL)
public class JFactory {
  private static final String SIZE_VARIANT =
      "JComponent.sizeVariant"; // $NON-NLS-1$
  ...
  public static JTextArea tabMovesFocus(JTextArea textArea) {
    return STYLE.withDynamic(textArea, c -> {
      FocusActions.bind(
          c, KeyStroke.getKeyStroke(KeyEvent.VK_TAB, 0),
   ...
   * There's no Swing property for {@code rowHeight}, so each table should be
  ...
  /**
   * Attaches a configuration action that is executed when Look and Feel
   * changes. <p>Note: the action is executed when {@code withDynamic} is
   * called, and the action is executed even if the new and the old LaFs are the
   * same.</p>
   * @param component component to update
   * @param onUpdateUi action to run (immediately and when look and feel
   *     changes)
  ...
  public static <T extends JComponent>
      T withDynamic(T component, Consumer<? super T> onUpdateUi) {
    return STYLE.withDynamic(component, onUpdateUi);
  }

  public static void updateUi(JComponent c) { STYLE.updateComponentTreeUI(c); }
}
