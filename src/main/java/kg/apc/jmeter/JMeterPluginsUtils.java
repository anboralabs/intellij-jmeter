...
public abstract class JMeterPluginsUtils {
  private static final Logger log =
      LoggerFactory.getLogger(JMeterPluginsUtils.class);
  ...
  public static final String ENCODING = JMeterUtils.getPropDefault(
      "sampleresult.default.encoding", SampleResult.DEFAULT_HTTP_ENCODING);
  ...
  static {
    String prefixPluginsCfg =
        JMeterUtils.getProperty("jmeterPlugin.prefixPlugins");
    if (prefixPluginsCfg != null) {
      JMeterPluginsUtils.prefixPlugins =
          "true".equalsIgnoreCase(prefixPluginsCfg.trim());
    }
  }

  public static CollectionProperty
  tableModelRowsToCollectionProperty(PowerTableModel model, String propname) {
    CollectionProperty rows =
        new CollectionProperty(propname, new ArrayList<>());
  ...
  public static CollectionProperty
  tableModelRowsToCollectionPropertyEval(PowerTableModel model,
                                         String propname) {
    CollectionProperty rows =
        new CollectionProperty(propname, new ArrayList<>());
    for (int row = 0; row < model.getRowCount(); row++) {
  ...
  public static void collectionPropertyToTableModelRows(CollectionProperty prop,
                                                        PowerTableModel model) {
    model.clearData();
    for (int rowN = 0; rowN < prop.size(); rowN++) {
      ArrayList<String> rowObject =
          (ArrayList<String>)prop.get(rowN).getObjectValue();
  ...
  public static void collectionPropertyToTableModelRows(CollectionProperty prop,
    ...
    model.clearData();
    for (int rowN = 0; rowN < prop.size(); rowN++) {
      ArrayList<StringProperty> rowStrings =
          (ArrayList<StringProperty>)prop.get(rowN).getObjectValue();
  ...
  public static String buildHelpPageUrl(String helpPage) {
    try {
      if (helpPage.matches("[hH][tT][tT][pP][sS]?://.*")) {
        log.debug("Help page URL found, skipping building link to " +
                  WIKI_BASE);
        return helpPage;
      }
    ...
    return WIKI_BASE + helpPage +
        "/?utm_source=jmeter&utm_medium=helplink&utm_campaign=" + helpPage;
  }
  ...
  public static Component addHelpLinkToPanel(Container panel, String helpPage) {
    ...
    icon.setIcon(new ImageIcon(
        JMeterPluginsUtils.class.getResource("vizualizers/information.png")));
    ...
    Container innerPanel =
        findComponentWithBorder((JComponent)panel, EtchedBorder.class);
  ...
  private static class URIOpener extends MouseAdapter {
    ...
    public URIOpener(String aURI) { uri = aURI; }
  ...
  public static void prepareJMeterEnv(String homeDir) {
    ...
    while (dir.exists() && dir.getName().equals("ext") &&
    dir.getParentFile().getName().equals("lib")) { dir = dir.getParentFile();
    }

    if (!dir.exists()) {
        throw new IllegalArgumentException("CMDRunner.jar must be placed in
  ...
  private static boolean isJMeterHome(String homeDir) {
    File f =
        new File(homeDir + File.separator + "lib" + File.separator + "ext");
  ...
  public static String getShortHostname(String host) {
    ...
    try {
      ...
      String hostnameRegex = JMeterUtils.getPropDefault(
          "jmeterPlugin.perfmon.label.useHostname.pattern", defaultRegex);
      ...
      log.warn("getShortHostname exception: " + e.getClass().getName() +
               " :: " + e.getMessage());
