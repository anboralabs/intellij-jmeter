package co.anbora.labs.jmeter.ide.settings

object Releases {
    val Versions = listOf(
        JMeterRelease(
            url = "https://archive.apache.org/dist/jmeter/binaries/apache-jmeter-5.6.3.zip",
            name = "v5.6.3",
            destinationFolder = "apache-jmeter-5.6.3"
        ),
        JMeterRelease(
            url = "https://archive.apache.org/dist/jmeter/binaries/apache-jmeter-5.6.2.zip",
            name = "v5.6.2",
            destinationFolder = "apache-jmeter-5.6.2"
        ),
        JMeterRelease(
            url = "https://archive.apache.org/dist/jmeter/binaries/apache-jmeter-5.6.1.zip",
            name = "v5.6.1",
            destinationFolder = "apache-jmeter-5.6.1"
        ),
        JMeterRelease(
            url = "https://archive.apache.org/dist/jmeter/binaries/apache-jmeter-5.6.zip",
            name = "v5.6",
            destinationFolder = "apache-jmeter-5.6"
        ),
        JMeterRelease(
            url = "https://archive.apache.org/dist/jmeter/binaries/apache-jmeter-5.5.zip",
            name = "v5.5",
            destinationFolder = "apache-jmeter-5.5"
        )
    )
}